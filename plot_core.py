...
def draw_system_sketch(L_total, supports_x, loads, transparent_bg=False):
    ...
    for sx in supports_x:
        ax.add_patch(patches.Polygon(
            [[sx-0.075, beam_y-0.15], [sx+0.075, beam_y-0.15], [sx, beam_y]], 
            closed=True, fill=False, edgecolor='#00FF00', linewidth=0.5
        ))
    ...
    for idx, ld in enumerate(loads):
        if ld['type'] in ['linear', 'Trapezoidal']:
            ...
            ax.add_patch(patches.Polygon(
                [[ld['x1'], beam_y], [ld['x1'], h1], [ld['x2'], h2], [ld['x2'], beam_y]], 
                closed=True, fill=False, hatch='||', edgecolor='blue', linewidth=0.5
            ))
    ...
    fig.savefig(img_stream, format='png', bbox_inches='tight', dpi=150, transparent=transparent_bg)
    plt.close(fig)
...
def generate_acrow_diagrams(section_name, L_total, supports_x, loads, E, I, Mall, Qall, Rall=None, transparent_bg=False):
    ...
    def add_side_table(row_idx, title, max_v, max_x, min_v, min_x, allow_val=None):
        ...
        # 💡 الجراحة البرمجية: إخفاء صف الـ Allowable وقرار الـ SAFE من جداول الريأكشن تماماً
        if title != "Reaction":
        ...
        if title != "Reaction" and allow_val is not None:
            is_safe = max(abs(max_v), abs(min_v)) <= allow_val
            color = "#198754" if is_safe else "#dc3545" 
            # 💡 الاعتماد على الـ length الخاص بالـ cell_text عشان البرنامج ميضربش Index Error 
            cell = tab[len(cell_text), 2] 
            cell.get_text().set_color(color)
            cell.get_text().set_fontweight('bold')
            
        for (r, c), cell_obj in tab.get_celld().items():
            # 💡 حماية الـ Loop من ضرب اللون الأخضر في المكان الغلط للريأكشن
            if not (title != "Reaction" and allow_val is not None and r == len(cell_text) and c == 2): 
                if transparent_bg:
...
def draw_tilting_diagrams(H, struts, w_dist, bkt_data=None, transparent_bg=False):
    ...
    fig1.savefig(img1_stream, format='png', bbox_inches='tight', dpi=150, transparent=transparent_bg)
    plt.close(fig1)
    ...
    fig2.savefig(img2_stream, format='png', bbox_inches='tight', dpi=150, transparent=transparent_bg)
    plt.close(fig2)
    ...
    fig3.savefig(img3_stream, format='png', bbox_inches='tight', dpi=150, transparent=transparent_bg)
    plt.close(fig3)
