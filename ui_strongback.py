...
def render_strongback_ui(i, w_tot, h_static, m_spc_val):
    ...
    with st.container(border=True):
        ...
        with c1: 
            # 💡 تحديث: ربط الخانة بنظام الذاكرة لاسترجاع المسافة المحفوظة
        ...
        with col_ld2:
            ...
            # 💡 استرجاع جدول الأحمال المحفوظ 
            saved_sbdf = st.session_state.get(f"sb_loads_{i}", [{"Load Type": "Linear", "WA (kN/m) or P (kN)": round(w1_base, 2), "WB (kN/m)": round(w1_base, 2), "LA (m) or X (m)": 0.0, "LB (m)": round(h_static, 2)}])
