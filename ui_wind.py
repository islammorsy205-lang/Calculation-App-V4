...
def render_wind_tilting_ui(i, h_panel, w_tot):
    ...
    with st.container(border=True):
        ...
        with cw1:
            exp_opts = ["B", "C", "D"]
            idx_exp = exp_opts.index(st.session_state.get(f"exp_{i}", "B")) if st.session_state.get(f"exp_{i}") in exp_opts else 0
            exp_cat = st.selectbox("Exposure Category", exp_opts, index=idx_exp, key=f"exp_{i}")
            
            v_wind = st.number_input("Wind Speed V (m/s)", value=float(st.session_state.get(f"vw_{i}", 47.0)), step=1.0, key=f"vw_{i}")
            
        with cw2:
            z_height = st.number_input("Height above ground, z (m)", value=float(st.session_state.get(f"z_ht_{i}", 20.0)), step=0.05, key=f"z_ht_{i}")
            kz_wind = get_kz(z_height, exp_cat)
            st.info(f"**Kz Factor** = {kz_wind:.2f}")
            
        with cw3: 
            h_panel = st.number_input("Total Wall/Panel Height (m)", key=f"hp_wind_{i}", step=0.05, value=float(current_h))
            
        with cw4: 
            wp_panel = st.number_input("Push-pull Spacing (m)", value=float(st.session_state.get(f"wp_{i}", 2.20)), step=0.05, key=f"wp_{i}")
        ...
        if st.toggle("Include Detailed Strut & Anchor Checks", value=bool(st.session_state.get(f"inc_tilt_{i}", False)), key=f"inc_tilt_{i}"):
            ts_1, ts_2 = st.columns([1.0, 2.0])
            
            with ts_1:
                inc_bracket = st.toggle("Include Access Bracket", value=bool(st.session_state.get(f"inc_bkt_{i}", False)), key=f"inc_bkt_{i}")
                ...
                if inc_bracket:
                    ...
                    acc_L1 = cb1.number_input("L1 (m)", value=float(st.session_state.get(f"acc_l1_{i}", 0.90)), step=0.05, key=f"acc_l1_{i}")
                    acc_L2 = cb2.number_input("L2 (m)", value=float(st.session_state.get(f"acc_l2_{i}", 1.00)), step=0.05, key=f"acc_l2_{i}")
                    acc_LL = cb3.number_input("LL (kN/m²)", value=float(st.session_state.get(f"acc_ll_{i}", 1.50)), step=0.1, key=f"acc_ll_{i}")
                
                np_opts = [2, 3]
                idx_np = np_opts.index(int(st.session_state.get(f"rb_num_props_{i}", 2))) if int(st.session_state.get(f"rb_num_props_{i}", 2)) in np_opts else 0
                num_props = st.radio("Number of Push-Pulls:", np_opts, index=idx_np, key=f"rb_num_props_{i}", horizontal=True)
                ...
                with col_g1:
                    y1 = st.number_input("Y1 (m)", value=float(st.session_state.get(f"y1_{i}", 0.50)), key=f"y1_{i}", step=0.05)
                    y2 = st.number_input("Y2 (m)", value=float(st.session_state.get(f"y2_{i}", 4.00)), key=f"y2_{i}", step=0.05)
                    if num_props == 3:
                        y3 = st.number_input("Y3 (m)", value=float(st.session_state.get(f"y3_{i}", 6.67)), key=f"y3_{i}", step=0.05)
                    else:
                        y3 = 0.0
                        
                with col_g2:
                    x1 = st.number_input("X1 (m)", value=float(st.session_state.get(f"x1_{i}", 2.31)), key=f"x1_{i}", step=0.05)
                    x2 = st.number_input("X2 (m)", value=float(st.session_state.get(f"x2_{i}", 2.31)), key=f"x2_{i}", step=0.05, on_change=sync_x1)
                    if num_props == 3:
                        x3 = st.number_input("X3 (m)", value=float(st.session_state.get(f"x3_{i}", 3.85)), key=f"x3_{i}", step=0.05)
                    else:
                        x3 = 0.0
                ...
                vs1 = get_valid_struts(l1_req, "wind")
                idx_s1 = vs1.index(st.session_state.get(f"s1_{i}", vs1[0])) if st.session_state.get(f"s1_{i}") in vs1 else 0
                s1 = st.selectbox(f"Lower Type ({l1_req:.2f}m)", vs1, index=idx_s1, key=f"s1_{i}")
                
                vs2 = get_valid_struts(l2_req, "wind")
                idx_s2 = vs2.index(st.session_state.get(f"s2_{i}", vs2[0])) if st.session_state.get(f"s2_{i}") in vs2 else 0
                s2 = st.selectbox(f"Middle Type ({l2_req:.2f}m)", vs2, index=idx_s2, key=f"s2_{i}")
                ...
                if num_props == 3:
                    vs3 = get_valid_struts(l3_req, "wind")
                    idx_s3 = vs3.index(st.session_state.get(f"s3_{i}", vs3[0])) if st.session_state.get(f"s3_{i}") in vs3 else 0
                    s3 = st.selectbox(f"Top Type ({l3_req:.2f}m)", vs3, index=idx_s3, key=f"s3_{i}")
                    struts_conf.append({
                        "y": y3, 
                        "x_base": x3, 
                        "type": s3, 
                ...
                btype_opts = ["Rigid Slab / Raft", "Independent Concrete Block"]
                idx_btype = btype_opts.index(st.session_state.get(f"base_type_{i}", btype_opts[0])) if st.session_state.get(f"base_type_{i}") in btype_opts else 0
                base_type = st.radio("Select Anchorage:", btype_opts, index=idx_btype, key=f"base_type_{i}", horizontal=True)
                
                block_data = None
                if base_type == "Independent Concrete Block":
                    col_b1, col_b2, col_b3, col_b4 = st.columns(4)
                    b_len = col_b1.number_input("L (m)", value=float(st.session_state.get(f"blen_{i}", 1.0)), step=0.05, key=f"blen_{i}")
                    b_wid = col_b2.number_input("W (m)", value=float(st.session_state.get(f"bwid_{i}", 1.0)), step=0.05, key=f"bwid_{i}")
                    b_ht = col_b3.number_input("H (m)", value=float(st.session_state.get(f"bht_{i}", 1.25)), step=0.05, key=f"bht_{i}")
                    b_mu = col_b4.number_input("μ", value=float(st.session_state.get(f"bmu_{i}", 0.50)), step=0.01, key=f"bmu_{i}")
                    block_data = {
                        'active': True, 
                        'L': b_len, 
                        'W': b_wid, 
