...
def parse_dxf_to_data(file_bytes):
    ...
    for idx, f in enumerate(raw_frames):
        if f['type'] == 'line':
            ...
            base_segments.append({
                ...
                'abs_c': tuple(f['c']), 'abs_r': f['r'], 'abs_sa': sa, 'abs_ea': ea, 'sweep': sweep, 'kappa': 1.0/f['r'], 'is_divided': False
            })
    ...
    for sp in raw_supports:
        # إحداثيات صريحة خالية من أي روابط مبدئية لضمان ثباتها
...
def perform_smart_division(base_segments, supports, struts):
    """
    التقطيع بيعتمد على الهندسة الفراغية المطلقة بالـ (X,Y) ضد الفريمات الأصلية 100%
    """
...
def build_chain_mesh(segments, seg_sections, loads, struts, supports, base_segments, mesh_size=0.25):
    ...
    # 🔥 اللحام الديناميكي الخارق: ربط الدعامات هندسياً بالـ Base Segments فقط 🔥
    ...
    for i, seg in enumerate(segments):
        ...
        # حقن نقط الدعامات جوه الـ Mesh المقطعة
...
def render_advanced_shape_module():
    ...
    if uploaded_dxf and st.button("Extract Data from DXF"):
        ...
        if parsed:
            ...
            # 🔥 السلاح النووي لتدمير الذاكرة الشبحية مع كل ملف DXF جديد 🔥
    ...
    dxf_v = st.session_state.dxf_version # ربط الخانات بالمفتاح النووي
    ...
    with c_in:
        ...
        for i, sup in enumerate(st.session_state.ui_supports):
            ...
            # ربط الـ Key بـ dxf_version لضمان عدم ظهور أرقام شبحية
        ...
        g_sec_type = c_sec1.selectbox("Global Frame Section", ["Soldier U100", "Custom Section"], key="g_sec")
        ...
        if g_sec_type == "Soldier U100":
            ...
            with c_sec2.expander("📝 Custom Properties", expanded=True):
                cs_A = st.number_input("Area (cm2)", value=40.0)
                cs_I = st.number_input("Inertia I (cm4)", value=800.0)
                cs_M = st.number_input("Mall (kN.m)", value=20.0)
                cs_Q = st.number_input("Qall (kN)", value=120.0)
        ...
        with st.expander("🛠️ Override specific segments section", expanded=False):
            override_segs = st.multiselect("Select segments to override:", base_seg_names)
            if override_segs:
                o_sec_type = st.radio("Override Profile", ["Custom Section", "Soldier U100"], key="o_rad", horizontal=True)
                if o_sec_type == "Soldier U100":
                     o_sec = {'name': "Soldier U100", 'E': 2100.0, 'A': 34.3 / 10000.0, 'I': 412.0 / 100000000.0, 'Mall': 13.1, 'Qall': 100.8}
                else:
                    o1, o2, o3, o4 = st.columns(4)
                    o_A = o1.number_input("A (cm2)", value=50.0, key="oa")
                    o_I = o2.number_input("I (cm4)", value=1200.0, key="oi")
                    o_M = o3.number_input("Mall (kN.m)", value=30.0, key="om")
                    o_Q = o4.number_input("Qall (kN)", value=150.0, key="oq")
        ...
        if st.button("✂️ Divide Frames & Update Topology", use_container_width=True, type="primary"):
            # بنرسل الـ ui_supports المحدثة مباشرة للمحرك
    ...
    with c_plot:
        ...
        fac_w = cc_w.number_input("Wind Factor", value=1.0, step=0.1, format="%.2f", key="cmb_w")
        ...
        for i, ld in enumerate(st.session_state.ui_loads):
            with st.expander(f"📥 Load Item {i+1}", expanded=(i==0)):
                ...
                target_mode = st.radio("Apply Load To:", ["Single Segment", "Multiple Segments", "All Segments"], index=["Single Segment", "Multiple Segments", "All Segments"].index(ld.get('target_mode', 'All Segments')), key=f"ld_mode_{i}", horizontal=True)
                ld['target_mode'] = target_mode
                
                target_segments = []
                if target_mode == "Single Segment":
                    s_choice = st.selectbox("Select Segment", working_seg_names, key=f"ld_single_{i}")
                    target_segments.append(working_seg_names.index(s_choice))
                elif target_mode == "Multiple Segments":
                    selected_segs = st.multiselect("Select Segments", working_seg_names, default=[s for s in ld.get('target_segs', []) if s in working_seg_names], key=f"ld_multi_{i}")
