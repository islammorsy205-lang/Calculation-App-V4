...
# =====================================================================
# 🧠 THE HEURISTIC OPTIMIZER ENGINE (PHYSICS-AWARE V11)
# =====================================================================
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v):
    """
    الإصدار السحري V11 (Physics-Aware):
    - يتعرف أوتوماتيكياً على أماكن نزول النهايز (Struts) على الفريم السفلي.
    - يولد الاحتمالات الرياضية ويرتبها بنظام (Score) بحيث يعطي الأولوية المطلقة للشبكات التي:
        1. تملك أقل عدد دعامات.
        2. تضع الدعامات أسفل النهايز مباشرة (لحمل الـ Concentrated Loads).
    - يختبر في حدود 120 ثانية بأقصى سرعة ممكنة دون تعليق النظام.
    """
    try:
    ...
    # 💡 1. رادار الأحمال: تحديد الإحداثيات المطلقة للـ Struts بالنسبة للسنتر
    strut_rel_xs = set()
    for ds in ui_struts:
        gx = ds.get('gx', 0.0)
        strut_rel_xs.add(round(abs(gx - center_x), 3))
    ...
    # 💡 2. المولد الرياضي السريع
    valid_half_grids = set()
    queue = deque()
    
    queue.append( (0.0, (0.0,)) ) 
    for s in spacings:
        queue.append( (s/2.0, (s/2.0,)) )
        
    gen_start = time.time()
    
    while queue and len(valid_half_grids) < 20000:
        if time.time() - gen_start > 3.0: break # حماية للتوليد
        
        curr_x, curr_grid = queue.popleft()
        cantilever = half_width - curr_x
        
        if 0.15 <= cantilever <= 1.50:
            valid_half_grids.add(curr_grid)
            
        if cantilever > 0.15:
            for s in spacings:
                next_x = curr_x + s
                if half_width - next_x >= 0.0:
                    queue.append( (next_x, curr_grid + (next_x,)) )
                    
    unique_grids = [list(g) for g in valid_half_grids]
    
    # 💡 3. نظام الترتيب والتقييم العبقري (Heuristic Scoring)
    def grid_score(grid):
        prop_count = len(grid) * 10.0 # الوزن الأساسي لعدد الدعامات
        alignment_bonus = 0.0
        
        # البحث عن دعامة قريبة من أماكن نزول النهايز
        for sx in strut_rel_xs:
            min_dist = min([abs(px - sx) for px in grid]) if grid else 99.0
            if min_dist <= 0.25:
                alignment_bonus += 5.0  # مكافأة ضخمة لو الدعامة تحت النهيز مباشرة
            elif min_dist <= 0.60:
                alignment_bonus += 2.0  # مكافأة بسيطة للتقارب
                
        return prop_count - alignment_bonus

    # الترتيب حسب السكور (الأقل سكوراً يُختبر أولاً)
    unique_grids.sort(key=grid_score)

    # 4. Sequential FEA Testing with 120s Time Limit
    ...
    for grid in unique_grids:
        ...
        if max_ry <= target_rxn and min_ry >= 0.5:
            return True, test_supps, f"✅ Optimum grid found! (0 Uplift, Perfect Strut Alignment, {len(test_supps)} Props. Tested {trials_count} combinations)"
    ...
    if best_fallback_grid:
        return True, best_fallback_grid, f"⚠️ Deep Search Timeout. Shown best fallback with NO Uplift. Tested {trials_count} combos. Max Reaction: {best_fallback_score:.2f} kN."
        
    return False, None, f"❌ Optimizer failed! Tested {trials_count} combinations. All generated UPLIFT or exceeded Target Reaction within limit."
...
def render_advanced_shape_module():
    ...
    with c_in:
        ...
        # ==========================================
        # 🤖 AI GENERATIVE DESIGN OPTIMIZER V11
        # ==========================================
        st.markdown("### 🤖 AI Auto-Shoring Optimizer")
        with st.expander("✨ Generative Design for Bridge Supports", expanded=False):
            st.info("The AI automatically detects STRUT locations to place supports underneath them, minimizing total props and completely eliminating UPLIFT forces!")
