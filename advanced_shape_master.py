...
# =====================================================================
# 🧠 THE BFS OPTIMIZER ENGINE (GENERATIVE DESIGN FOR BRIDGE SHORING) V10
# =====================================================================
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v):
    """
    الإصدار المدمر السريع (V10):
    - توليد رياضي خفيف (DFS) يجمع أقصر 15,000 شبكة محتملة فقط في أقل من ثانية.
    - ترتيب الاحتمالات تصاعدياً حسب عدد الدعامات (Least props first).
    - اختبار الـ FEA مقيد بـ 60 ثانية للبحث عن الحل المكتمل.
    - نظام إنقاذ (Fallback) يعرض أفضل شبكة بلا Uplift في حال انتهاء الوقت.
    """
    try:
    ...
    # 1. Mathematical Generator (Fast Breadth-First Memory Filter)
    ...
    # 💡 توليد الاحتمالات الرياضية بأقصى سرعة
    while queue and len(valid_half_grids) < 15000:
        if time.time() - gen_start > 5.0: break # حماية ضد تجميد الواجهة أثناء التوليد الرياضي
        
        curr_x, curr_grid = queue.popleft()
        cantilever = half_width - curr_x
        
        # إجبار الكابولي بين 15 سم و 1.5 م
    ...
    # 💡 ترتيب الشبكات حسب (أقل عدد دعامات أولاً) لضمان التوفير
    unique_grids.sort(key=lambda g: len(g))

    # 2. Sequential FEA Testing with 60s Time Limit
    ...
    for grid in unique_grids:
        ...
        # بناء الإحداثيات الحقيقية للـ DXF
        full_grid = set(grid)
        for x in grid:
        ...
        # 💡 الحل المثالي: أمان من الأحمال + أمان من الـ Uplift
        if max_ry <= target_rxn and min_ry >= 0.5:
            return True, test_supps, f"✅ Optimum grid found! (0 Uplift, {len(test_supps)} Props, Tested {trials_count} combinations)"
            
        # 💡 نظام الطوارئ (Fallback): حفظ أفضل رصة مسيفة Uplift في حال انتهاء الـ 60 ثانية
        if min_ry >= 0.1:
    ...
    if best_fallback_grid:
        return True, best_fallback_grid, f"⚠️ Deep Search Timeout (60s). Shown best fallback with NO Uplift. Tested {trials_count} combos. Max Reaction: {best_fallback_score:.2f} kN."
        
    return False, None, f"❌ Optimizer failed! Tested {trials_count} combinations. All generated UPLIFT or exceeded Target Reaction within 60s limit."
...
def render_advanced_shape_module():
    ...
    with c_in:
        ...
        # ==========================================
        # 🤖 AI GENERATIVE DESIGN OPTIMIZER V8
        # ==========================================
        ...
        with st.expander("✨ Generative Design for Bridge Supports", expanded=False):
            ...
            if st.button("✨ Run AI Optimizer & Generate Supports", type="primary", use_container_width=True):
                with st.spinner("🧠 AI is executing Deep Search (Max 60 Seconds)..."):
