...
def extract_dxf_for_interactive(file_bytes):
    ...
    try:
        ...
        # 💡 التعديل الجذري الأول: قراءة الكاد المفتوحة
        for e in msp:
            lyr = e.dxf.layer.lower()
            dxftype = e.dxftype()
            
            # 1. أي نقطة = Support (بدون شروط اللاير)
    ...
    def create_force_plot(val_key, scale, c_pos, c_neg):
        ...
        for el in elements:
            ...
            def plot_val(idx):
                ...
                if is_far(tx, ty):
                    # 💡 تم تعديل الدقة إلى رقمين عشريين هنا
...
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v):
    ...
    test_combined_loads = []
    # 💡 ربط أحمال الـ AI بالمعاملات الافتراضية الجديدة
    ...
    # 💡 الوراثة الذكية أثناء التقطيع: الاعتماد على الـ base_seg_names
...
def render_advanced_shape_module():
    ...
    with c_plot:
        ...
        # 💡 الوراثة الذكية أثناء التقطيع: القوائم تقرأ من القطاعات الأساسية فقط
        for i, ld in enumerate(st.session_state.ui_loads):
            with st.expander(f"📥 Load Item {i+1}", expanded=(i==0)):
                ...
                # الواجهة بتعرض دايماً S1, S2 (بدون تقطيع) عشان الحمل مايضيعش
                ...
                # ترجمة الحمل من القطاع الأساسي إلى القطع المقطعة أوتوماتيكياً
                target_working_indices = [w_idx for w_idx, w_seg in enumerate(working_segments) if w_seg.get('master_idx', 0) in target_base_indices]
