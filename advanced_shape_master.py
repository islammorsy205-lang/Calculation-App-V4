# ==============================================================================
# ADVANCED SHAPE MASTER - ENGINEERING CALCULATION MODULE (Ultimate V24)
# ==============================================================================
...
# and intelligent fallback mechanisms for non-converging limits.
# ==============================================================================
...
# تم إزالة مكتبة pytesseract نهائياً بناءً على طلب المهندس لتخفيف الواجهة
...
def draw_reaction_arrow(ax, node_x, node_y, force_mag, axis_nx, axis_ny, is_bridge_mode=False):
    ...
    فلتر إخفاء قوى X في الكباري لتظهر كـ Roller بصرياً رغم كونها Hinged إنشائياً.
    """
...
def solve_fea_engine(nodes, elements, nodal_loads, supports_list):
    ...
    يحل العزوم، الشير، الأكسيال، ويحسب الترخيم الموضعي (Local Deflection)
    """
    NDOF = len(nodes) * 3
...
def generate_chain_report(sys_data):
    ...
    return out
# =====================================================================
# 🧠 THE HEURISTIC OPTIMIZER ENGINES (V23 - Fallback & Deflection Relaxation)
# =====================================================================
def run_bridge_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v, is_symmetric, opt_mode, status_text, progress_bar):
    ...
    def is_safe_soldier(elems, sections):
        # 💡 الدفلكشن ليس شرطاً للنجاح (Moment & Shear Only)
    ...
    for p_count in sorted_p_keys:
        for actual_coords in grids_by_props[p_count]:
            ...
            for shift_val in shift_options:
                ...
                # 💡 حفظ أفضل محاولة حتى لو فاشلة للعرض كـ Fallback
                if min_ry >= 0.5:
    ...
    # 💡 إجبار ظهور الحل البديل دائماً للمعاينة
...
def run_single_sided_optimizer(base_segments, working_segments, active_seg_sections, ui_loads, target_rxn, view_plane, auto_mesh_size, dxf_v, status_text, progress_bar):
    ...
    status_text.empty()
    # 💡 إجبار ظهور الحل البديل للمعاينة
    if best_struts:
        spacing_data = {'cx': cx, 'cy': cy, 'struts': best_struts, 'supps': best_supps[1:]}
        return False, best_supps, best_struts, spacing_data, f"⚠️ Notice: Best safe structure yields Tie Reaction = {best_fallback_tie:.2f} kN (>180). Showing fallback."
...
def render_advanced_shape_module():
    ...
    with c_plot:
        ...
        for i, ld in enumerate(st.session_state.ui_loads):
            with st.container(border=True):
                ...
                # 💡 إضافة خانة الموقع للحمل النقطي لضمان الدقة وتفادي ضرب العزوم في الصفر
                if ld['type'] == "Point Load":
    ...
    if getattr(st.session_state, 'adv_solved', False):
        ...
        for i, sec in enumerate(fea_data['seg_sections']):
            ...
            s_stat = "SAFE" if (max_m <= sec['Mall'] and max_v <= sec['Qall'] and max_d <= all_d) else "UNSAFE"
