...
def apply_plot_styles():
    """
    تطبيق تنسيقات الرسم البياني الهندسية الدقيقة 
    """
...
def draw_reaction_arrow(ax, node_x, node_y, force_mag, axis_nx, axis_ny):
    ...
    مع تمييز قوة الشد (Uplift) باللون الأحمر
    """
    ...
    # تقريب الأرقام لخاننين عشريتين فقط حسب الطلب
    ax.text(
        start_x - 0.15 * dx, 
        start_y - 0.15 * dy, 
        f"{force_mag:+.2f}", 
...
def eval_seg_point(seg, s_val, start_data=None):
    ...
    يدعم الخطوط المستقيمة والمنحنيات
    """
...
def get_closest_segment_exact(pt, segs):
    ...
    تستخدم هذه الدالة أساساً في اللحام المغناطيسي للنهايز والدعامات
    """
    min_d = 9999.0
    best_idx = 0
    best_s = 0.0
...
def get_strut_priority(name):
    ...
    تم إعطاء الـ PPS الأولوية القصوى ثم الـ PPH بناء على طلب المستخدم
    """
    name_u = name.upper()
    score = 100
...
def extract_dxf_for_interactive(file_bytes):
    ...
    try:
        ...
        for e in msp:
            ...
            # 1. أي نقطة أو دائرة في الرسمة تعني (Support) إجبارياً
            ...
            for sub_e in entities:
                ...
                # 2. خطوط طبقة (Push / Pull) تتحول إلى نهايز Struts
                ...
                # 3. أي خط أو منحنى آخر يعتبر إطار رئيسي (Frame) بغض النظر عن اللاير
                else:
                    if sub_type == 'LINE':
                        frames.append({
                            'type': 'line', 
...
def parse_dxf_to_data(file_bytes):
    """
    تحويل البيانات الخام من الكاد إلى هياكل بيانات مفصلة (Segments)
    """
    ...
    # اللحام الفعلي: تخزين الإحداثيات المطلقة للـ Struts لربطها بالـ Mesh لاحقاً لتجنب فقدان الأحمال
    struts_mapped = []
    for s in raw_struts:
        p1 = s['p1']
        p2 = s['p2']
...
def perform_smart_division(base_segments, supports, struts):
    """
    التقطيع يتم فقط لتجميل الرسم وتوقيع الأحمال، 
    ولكن اللحام الأصلي الميكانيكي يتم في الماتريكس
    """
...
def build_chain_mesh(segments, seg_sections, loads, struts, supports, base_segments, mesh_size=0.25):
    ...
    # 4. بناء عناصر الـ Truss (النهايز) بالإحداثيات المطلقة المغناطيسية
...
def solve_fea_engine(nodes, elements, nodal_loads, supports_list):
    ...
    وتطبيق شرط (مجموع الأحمال = مجموع الريأكشنات)
    """
    NDOF = len(nodes) * 3
...
def draw_base_geometry(ax, nodes, elements, supports_list, seg_sections=None, segments=None, show_seg_names=False):
    ...
    # رسم خطوط الأبعاد الديناميكية (Dimensions) باللون الرمادي
...
# =====================================================================
# 🧠 THE HEURISTIC OPTIMIZER ENGINE (PHYSICS-AWARE V16 DUAL-CORE)
# =====================================================================
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v, is_symmetric, opt_mode):
    """
    الإصدار السحري V16 Dual-Core:
    يحتوي على مستويين للبحث بناءً على اختيار المستخدم:
    - Quick Search (180s): يجلب أسرع حل ممكن أو يعرض أفضل رصة أمان متوفرة.
    - Deep Search (900s): يرفض أي نتيجة تتجاوز الـ Target Reaction ويستمر في الترقية وزيادة عدد الدعامات حتى يجد الحل المثالي تماماً.
    """
    try:
        spacings = sorted([float(x.strip()) for x in spacings_str.split(',')], reverse=True)
    except: 
        return False, None, "❌ Format error in spacings."
    
    if not base_segments: 
        return False, None, "❌ No base segments found."
    ...
    if min_y == 9999.0: 
        return False, None, "❌ Base geometry not found."
    
    center_x = (min_x + max_x) / 2.0
    bridge_width = max_x - min_x
    half_width = bridge_width / 2.0
    
    # التقاط أماكن النهايز كـ Concentrated Loads لجذب الدعامات
    strut_xs = set()
    for ds in ui_struts:
    ...
    # 💡 فلتر المستحيلات الرياضي: نحسب إجمالي الحمل قبل بدء البحث لنعرف الحد الأدنى الفعلي لعدد الدعامات
    ...
    # 💡 التصفية: إقصاء أي رصة بها عدد دعامات أقل من المسموح رياضياً لتوفير وقت البحث
    ...
    # 💡 التطور الهرمي (Iterative Deepening): تجميع الاحتمالات حسب عدد الدعامات
    grids_by_props = {}
    ...
    # 💡 التقييم العبقري: ترتيب داخل كل مجموعة حسب تمركز الدعامات تحت النهايز لتجنب الـ Uplift والكسر
    def alignment_score(grid):
        bonus = 0.0
        for sx in strut_xs:
            if grid:
    ...
    for p_count in grids_by_props:
        grids_by_props[p_count].sort(key=alignment_score)

    start_time = time.time()
    
    # تحديد المدة بناءً على اختيار المستخدم (سريع أو عميق)
    ...
    # 💡 التصعيد الذكي: يبدأ بأقل عدد ممكن رياضياً، ولو فشل بيزود دعامة ويكمل!
    for p_count in sorted_p_counts:
        for actual_coords in grids_by_props[p_count]:
            ...
            # 💡 النجاح المطلق الذي يحترم المسموح به تماماً بدون أي Uplift
            if max_ry <= target_rxn and min_ry >= 0.5:
                sym_txt = "Symmetric" if is_symmetric else "Asymmetric"
                return True, test_supps, f"✅ BOOM! {sym_txt} Grid Found: Max Rxn = {max_ry:.2f} kN (Limit: {target_rxn}). Total Props: {p_count}. Tested {trials_count} combos."
    ...
    # 💡 التعامل مع نهاية الوقت (Timeout)
    if best_fallback_grid:
        if "Deep" in opt_mode:
            # في وضع Deep Search، نرفض عرض رصة تتجاوز المسموح لأن المستخدم طالب الدقة المتناهية
            return False, best_fallback_grid, f"❌ STRICT LIMIT FAILED: Couldn't achieve <= {target_rxn} kN within 15 mins. Best safe grid reaction is {best_fallback_score:.2f} kN. Try smaller spacings to allow more props."
        else:
            # في وضع Quick Search، نعرض أفضل رصة آمنة حتى لو تجاوزت المسموح قليلاً لتجنب تعطيل العمل
            return True, best_fallback_grid, f"⚠️ Quick Search Timeout (3 mins). Showing BEST SAFE FALLBACK. Max Reaction is {best_fallback_score:.2f} kN (Target was {target_rxn}). For strictly accurate results, run Deep Search."
...
def render_advanced_shape_module():
    ...
    # العداد السري لمنع شبح الذاكرة من إعادة الأرقام القديمة
    ...
    with c_in:
        ...
        for i, ds in enumerate(st.session_state.ui_struts):
            with st.expander(f"📏 Strut P{i+1}", expanded=True):
                ...
                for opt in strut_opts_base:
                    ...
                    if m and float(m.group(1)) <= actual_L <= float(m.group(2)): 
        ...
        # ==========================================
        # 🤖 AI GENERATIVE DESIGN OPTIMIZER V16 DUAL-CORE
        # ==========================================
        st.markdown("### 🤖 AI Auto-Shoring Optimizer")
        with st.expander("✨ Generative Design for Bridge Supports", expanded=False):
            st.info("The AI refuses to exceed the Target Load! It uses Iterative Deepening to find the absolute minimum number of props required without violating safety.")
            ...
            # 💡 الاختيار بين مستويات البحث (سريع أو عميق)
            opt_mode = st.radio(
                "Optimization Depth:", 
                ["Quick Search (Max 3 Mins)", "Deep Search (Max 15 Mins)"], 
                index=0, 
                help="Quick gives the best safe fallback if strict target is hard. Deep strictly enforces the target and tries more props."
            )
            ...
            if st.button("✨ Run AI Optimizer & Generate Supports", type="primary", use_container_width=True):
                with st.spinner("🧠 AI is executing Physics-Aware Search..."):
                    succ, res, msg = run_auto_optimizer(
                        ...
                        ai_target_rxn, ai_spc_str, view_plane, auto_mesh_size, dxf_v, is_sym, opt_mode
                    )
                    ...
                    if succ:
                        ...
                        # تنظيف الذاكرة الشبحية للـ Streamlit
    ...
    if getattr(st.session_state, 'adv_solved', False):
        ...
        for i, sec in enumerate(fea_data['seg_sections']):
            ...
            safety_data.append({
                "Component": f"{fea_data['segments'][i].get('name', f'S{i+1}')} ({get_short_name(sec['name'])})", 
