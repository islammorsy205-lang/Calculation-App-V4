...
# =====================================================================
# 🧠 THE AI OPTIMIZER ENGINE (GENERATIVE DESIGN FOR BRIDGE SHORING) V3
# =====================================================================
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v):
    ...
    # 🧠 THE NEW GRID GENERATOR (Symmetrical, Cantilever-Aware, Least Props)
    candidates = []
    for S_main in spacings:
        for center_prop in [True, False]:
            for S_edge in spacings + [None]:
                
                right_coords = []
                curr_x = 0.0 if center_prop else S_main / 2.0
                
                if curr_x >= half_width: continue
                right_coords.append(curr_x)
                
                max_k = int(half_width / S_main) + 2
                for k in range(max_k):
                    grid = right_coords + [curr_x + (i+1)*S_main for i in range(k)]
                    if grid[-1] >= half_width: break
                    
                    if S_edge is not None:
                        edge_x = grid[-1] + S_edge
                        if edge_x < half_width:
                            grid.append(edge_x)
                        else:
                            continue
                            
                    cantilever = half_width - grid[-1]
                    
                    # 💡 الكابولي (الرفرفة) إجباري لمنع الـ Uplift وتقليل الدعامات
                    if 0.15 <= cantilever <= 1.50:
                        full_grid = set(grid)
                        for x in grid:
                            if x > 1e-4: full_grid.add(-x)
                        sorted_full = sorted(list(full_grid))
                        if sorted_full not in candidates:
                            candidates.append(sorted_full)
                            
    # ترتيب الاحتمالات: الأولوية لـ "أقل عدد دعامات"، ثم "أكبر مسافة متوسطة"
    candidates.sort(key=lambda g: (len(g), -np.mean(np.diff(g)) if len(g)>1 else 0))
    
    if not candidates:
        return False, "❌ Could not generate valid symmetric grids. Try adding smaller spacings."
        
    # ================= THE ITERATIVE SOLVER =================
    for grid in candidates:
        # نقل الشبكة لتتمركز حول الـ Center الحقيقي للـ DXF
        ...
        # 💡 شرط النجاح المزدوج: الحمل أقل من المسموح + لا يوجد Uplift (Tension) نهائياً
        if max_ry <= target_rxn and min_ry > 0.5:
            return True, test_supps
            
    return False, "❌ Failed! All symmetric combinations either exceeded Target Reaction or caused UPLIFT. Please decrease Target Reaction or add smaller spacings."
...
def render_advanced_shape_module():
    ...
    with c_in:
        ...
        # ==========================================
        # 🤖 AI GENERATIVE DESIGN OPTIMIZER V3
        # ==========================================
