...
def draw_reaction_arrow(ax, node_x, node_y, force_mag, axis_nx, axis_ny):
    ...
    ax.text(
        ...
        f"{force_mag:+.2f}", # 💡 تم تعديل الدقة إلى رقمين عشريين هنا
...
def render_advanced_shape_module():
    ...
    with c_plot:
        ...
        # 💡 تم تعديل معاملات التحميل (Load Factors) الافتراضية إلى 1.00 هنا
    ...
    if getattr(st.session_state, 'adv_solved', False):
        ...
        for i, sec in enumerate(fea_data['seg_sections']):
            ...
            safety_data.append({
                ...
                "Actual": f"M={max_m:.2f}, V={max_v:.2f}", "Allowable": f"M={sec['Mall']:.2f}, V={sec['Qall']:.2f}", "Status": s_status # 💡 تم تعديل الدقة إلى رقمين عشريين هنا
            })
