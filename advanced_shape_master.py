...
def get_strut_priority(name):
    ...
    if "PPS" in name_u: return 3 if is_odd else 1
    elif "PPH" in name_u: return 4 if is_odd else 2
    elif "TILT" in name_u: return 5
    elif "MNB" in name_u or "MIB" in name_u: return 6
    return 7

# =========================================================
# 1. THE DXF PARSER (Extracts Parent Segments ONLY)
# =========================================================
        ...
        for e in msp:
            ...
            if "supp" in lyr:
                 if e.dxftype() in ['POINT', 'CIRCLE', 'INSERT']:
                     if e.dxftype() == 'POINT': supports.append({'x': e.dxf.location.x, 'y': e.dxf.location.y})
                     elif e.dxftype() == 'CIRCLE': supports.append({'x': e.dxf.center.x, 'y': e.dxf.center.y})
                     elif e.dxftype() == 'INSERT': supports.append({'x': e.dxf.insert.x, 'y': e.dxf.insert.y})
            elif "push" in lyr or "pull" in lyr:
                 entities = list(e.virtual_entities()) if e.dxftype() in ['LWPOLYLINE', 'POLYLINE'] else [e]
...
def parse_dxf_to_data(file_bytes):
    ...
    for s in raw_struts:
        ...
        d_top, b_seg, b_s = get_closest_segment_exact(top_p, base_segments)
        struts_mapped.append({'base_seg_idx': b_seg, 's_dist': b_s, 'gx': bot_p[0], 'gy': bot_p[1]})
...
def perform_smart_division(base_segments, supports, struts):
    ...
    for st in struts:
        b_seg = st.get('base_seg_idx', 0)
        b_s = st.get('s_dist', 0.0)
        if b_seg < len(base_segments):
            cut_points_dict[b_seg].add(min(max(b_s, 0.0), base_segments[b_seg]['L']))
...
def build_chain_mesh(segments, seg_sections, loads, struts, supports, base_segments, mesh_size=0.25):
    ...
    for sup in supports:
        sx, sy = sup['x'], sup['y']
        # Forcing the node exactly at the exact DXF intersection
    ...
    for i, seg in enumerate(segments):
        ...
        for st_item in struts:
            st_m_idx = st_item.get('base_seg_idx')
            if st_m_idx == seg_m_idx:
                s_local = st_item.get('s_dist', 0.0) - parent_off
        ...
        for ld in loads:
            if ld.get('seg_idx') == i and ld.get('type') == 'Point Load':
                try:
                    idx_k = keys.index(round(ld['start'], 4))
                    nid = node_indices[idx_k]
                    dir_str = ld.get('dir', '')
                    if 'Global Z' in dir_str or 'Global Y' in dir_str:
                        ...
                        _, _, th_pt = eval_seg_point(seg, ld['start'])
    ...
    for st_idx, st_item in enumerate(struts):
        b_seg = st_item.get('base_seg_idx', 0)
        dist = round(st_item.get('s_dist', 0.0), 4)
        
        if b_seg < len(base_segments):
            nx, ny, _ = eval_seg_point(base_segments[b_seg], dist)
        else:
            nx, ny = 0.0, 0.0
...
# =====================================================================
# 🧠 THE AI OPTIMIZER ENGINE (GENERATIVE DESIGN FOR BRIDGE SHORING) V4
# =====================================================================
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v):
    """
    الإصدار الخارق V4: 
    - إجبار الكابولي (لا يقل عن 0.15 م ولا يزيد عن 1.20 م) لوزن العزوم وتجنب الـ Uplift.
    - البحث يبدأ بالشبكات التي تحتوي على *أقل عدد من الدعامات*.
    - الرفض الفوري لأي شبكة تنتج قوة شد (Uplift) في أي دعامة.
    - تحديد السنتر الحقيقي للكوبري من الـ DXF وتوزيع الدعامات بالتناظر التام حوله.
    """
    try:
        spacings = sorted([float(x.strip()) for x in spacings_str.split(',')], reverse=True)
    except: return False, "❌ Format error in spacings."
    
    if not base_segments: return False, "❌ No base segments found."
    
    # 💡 استنتاج الحدود الهندسية والمركز الحقيقي للرسمة من الـ DXF
    min_y = 9999.0
    max_x = -9999.0
    min_x = 9999.0
    ...
    if min_y == 9999.0: return False, "❌ Base geometry not found."
    
    center_x = (min_x + max_x) / 2.0
    half_width = (max_x - min_x) / 2.0
    
    test_combined_loads = []
    fac_d = float(st.session_state.get("cmb_d", 1.2))
    fac_l = float(st.session_state.get("cmb_l", 1.6))
    fac_w = float(st.session_state.get("cmb_w", 1.0))
    combo_facs = {'Dead Load': fac_d, 'Live Load': fac_l, 'Wind Load': fac_w}
    ...
    for i, ld in enumerate(ui_loads):
        ...
        if t_mode == "Single Segment":
            s_choice = st.session_state.get(f"ld_single_{i}", working_seg_names[0])
            target_segments = [working_seg_names.index(s_choice)] if s_choice in working_seg_names else [0]
        elif t_mode == "Multiple Segments":
            raw_def_segs = st.session_state.get(f"ld_multi_{i}", ld.get('target_segs', []))
            safe_def_segs = [s for s in raw_def_segs if s in working_seg_names]
            target_segments = [working_seg_names.index(s) for s in safe_def_segs]
        else:
            target_segments = list(range(len(working_segments)))
        ...
        for s_idx_num in target_segments:
            ...
            test_combined_loads.append({
                ...
                'w1': w1 * combo_facs[ld['category']], 'w2': w2 * combo_facs[ld['category']]
            })
    ...
    def run_trial(test_supps):
        ...
        U, R = solve_fea_engine(nodes_t, elems_t, nloads_t, slist_t)
        ...
        for sup in slist_t:
            n = sup['node']
            ry = R[3*n + 1] # القوة الرأسية (موجب = ضغط، سالب = شد Uplift)
            if ry > max_ry: max_ry = ry
            if ry < min_ry: min_ry = ry
                
        return max_ry, min_ry

    # 🧠 THE NEW GRID GENERATOR (Symmetrical, Cantilever-Aware, Least Props Priority)
    valid_half_grids = []
    
    # دالة فرعية تولد نصف الكوبري وتجبر البرنامج على عمل كابولي
    def generate_half_grids(current_grid, is_center):
        cantilever = half_width - current_grid[-1]
        
        # 💡 إجبار الرفرفة (Cantilever) لمنع الـ Uplift
        if 0.15 <= cantilever <= 1.20:
            full_set = set(current_grid)
            for x in current_grid:
                if x > 1e-4: full_set.add(-x)
            valid_half_grids.append(sorted(list(full_set)))
            
        if cantilever < 0.15:
            return # إذا لم يتبقى كابولي، تراجع ولا تكمل هذا الفرع
            
        # إضافة المسافات המتاحة للاستمرار في التوليد
        for s in spacings:
            next_x = current_grid[-1] + s
            # تجنب إضافة دعامة قريبة جداً من الحافة
            if half_width - next_x >= 0.15: 
                generate_half_grids(current_grid + [next_x], is_center)

    # بدء التوليد: إما بدعامة في السنتر، وإما بدون دعامة في السنتر
    generate_half_grids([0.0], True)
    for s in spacings:
        generate_half_grids([s / 2.0], False)

    # 💡 السر في توفير الدعامات: ترتيب الشبكات حسب (أقل عدد دعامات) ثم (أوسع مسافات)
    unique_grids = []
    for g in valid_half_grids:
        if g not in unique_grids:
            unique_grids.append(g)
    unique_grids.sort(key=lambda g: (len(g), -np.mean(np.diff(g)) if len(g)>1 else 0))

    if not unique_grids:
        return False, "❌ Cannot form a valid symmetric grid with proper overhangs (0.15m - 1.20m). Adjust your allowed spacings."

    # ================= THE ITERATIVE FEA SOLVER =================
    for grid in unique_grids:
        # 💡 تحويل الإحداثيات الوهمية (حول الصفر) إلى الإحداثيات الحقيقية للـ DXF
        actual_coords = [center_x + x for x in grid]
        
        test_supps = []
        for idx, gx in enumerate(actual_coords):
            stype = 'Hinged' if idx == len(actual_coords)//2 else 'Roller'
            test_supps.append({'x': round(gx, 3), 'y': round(min_y, 3), 'type': stype, 'angle': 0.0})
            
        max_ry, min_ry = run_trial(test_supps)
        
        # 💡 شرط النجاح المزدوج الصارم: أقصى قوة تحت المسموح + ممنوع الـ UPLIFT تماماً
        if max_ry <= target_rxn and min_ry >= 0.5:
            return True, test_supps

    return False, "❌ Optimizer failed! All combinations generated UPLIFT or exceeded Target Reaction. Increase Target Reaction or add smaller spacings."
...
def render_advanced_shape_module():
    ...
    with c_in:
        # ==========================================
        # 1. SUPPORTS (Dynamic UI with Absolute Mapping)
        # ==========================================
        ...
        for i, sup in enumerate(st.session_state.ui_supports):
            ...
            sup['x'] = cc1.number_input(f"J{i+1} X (m)", value=float(sup.get('x', 0.0)), format="%.4f", step=0.1, key=f"sx_{i}_{dxf_v}")
            sup['y'] = cc2.number_input(f"J{i+1} Y (m)", value=float(sup.get('y', 0.0)), format="%.4f", step=0.1, key=f"sy_{i}_{dxf_v}")
            
            type_opts = ["Hinged", "Roller", "Fixed"]
            idx_type = type_opts.index(sup.get('type', 'Hinged')) if sup.get('type') in type_opts else 0
            sup['type'] = cc3.selectbox(f"J{i+1} Type", type_opts, index=idx_type, key=f"sp_{i}_{dxf_v}")
            sup['angle'] = cc4.number_input(f"J{i+1} Angle(°)", value=float(sup.get('angle', 0.0)), step=15.0, key=f"sa_{i}_{dxf_v}")
            
            cc5.markdown("<br>", unsafe_allow_html=True)
            if cc5.button("❌", key=f"del_sup_{i}"):
        ...
        with st.expander("🛠️ Override specific segments section", expanded=False):
            # 💡 فلتر الحماية ضد الإيرور الأحمر
        ...
        # ==========================================
        # 3. STRUTS (Dynamic UI with Absolute Mapping)
        # ==========================================
        ...
        for i, ds in enumerate(st.session_state.ui_struts):
            with st.expander(f"📏 Strut P{i+1}", expanded=True):
                cc1, cc2, cc3, cc4, cc5 = st.columns([1.5, 1.2, 1.2, 1.5, 0.4])
                
                s_name_guess = base_seg_names[min(ds.get('base_seg_idx', 0), len(base_seg_names)-1)]
                s_idx_str = cc1.selectbox("Snap to Base Seg", base_seg_names, index=base_seg_names.index(s_name_guess), key=f"st_s_{i}_{dxf_v}")
                ds['base_seg_idx'] = base_seg_names.index(s_idx_str)
                
                max_s_strut = base_segments[ds['base_seg_idx']].get('L', 0.0) if base_segments else 3.0
                ds['s_dist'] = cc2.number_input("Dist from Start (m)", 0.0, max_s_strut, value=min(float(ds.get('s_dist', 1.0)), max_s_strut), format="%.3f", step=0.1, key=f"st_d_{i}_{dxf_v}")
                ds['gx'] = cc3.number_input("Base X (rel 0,0)", value=float(ds.get('gx', 3.0)), format="%.3f", step=0.1, key=f"st_gx_{i}_{dxf_v}")
                
                abs_gx = ds['gx']
                nx, ny = get_approx_xy(base_segments, ds['base_seg_idx'], ds['s_dist'])
                ...
                ds['sec'] = cc4.selectbox(f"Type (L={actual_L:.3f}m)", valid_opts, index=idx_sec, key=f"st_sec_{i}_{dxf_v}")
                
                cc5.markdown("<br>", unsafe_allow_html=True)
                if cc5.button("❌", key=f"del_st_{i}"):
                    st.session_state.ui_struts.pop(i)
                    st.rerun()

        if st.button("➕ Add Strut", use_container_width=True):
            st.session_state.ui_struts.append({'base_seg_idx': 0, 's_dist': 1.0, 'gx': 3.0, 'sec': strut_opts_base[0]})
            st.rerun()
        ...
        # ==========================================
        # 🤖 AI GENERATIVE DESIGN OPTIMIZER V4
        # ==========================================
        st.markdown("### 🤖 AI Auto-Shoring Optimizer")
        with st.expander("✨ Generative Design for Bridge Supports", expanded=False):
            st.info("The AI prioritizes grids with the LEAST number of props, enforcing overhangs (cantilevers) to eliminate UPLIFT forces and balance moments, all while maintaining perfect symmetry!")
            ...
            if st.button("✨ Run AI Optimizer & Generate Supports", type="primary", use_container_width=True):
                with st.spinner("🧠 AI is calculating the ultimate symmetric & stable grid..."):
                    succ, res = run_auto_optimizer(
                        base_segments, working_segments, active_seg_sections, 
                        st.session_state.ui_struts, st.session_state.ui_loads, 
                        ai_target_rxn, ai_spc_str, view_plane, auto_mesh_size, dxf_v
                    )
                    if succ:
                        st.session_state.ui_supports = res
                        st.success("✅ Optimum symmetric grid found and applied successfully! (UPLIFT ELIMINATED)")
                        st.rerun()
                    else:
                        st.error(res)
    ...
    with c_plot:
        ...
        fac_d = cc_d.number_input("Dead Factor", value=float(st.session_state.get("cmb_d", 1.2)), step=0.1, format="%.2f", key="cmb_d")
        fac_l = cc_l.number_input("Live Factor", value=float(st.session_state.get("cmb_l", 1.6)), step=0.1, format="%.2f", key="cmb_l")
        fac_w = cc_w.number_input("Wind Factor", value=float(st.session_state.get("cmb_w", 1.0)), step=0.1, format="%.2f", key="cmb_w")
        ...
        for i, ld in enumerate(st.session_state.ui_loads):
            with st.expander(f"📥 Load Item {i+1}", expanded=(i==0)):
                ...
                if target_mode == "Single Segment":
                    idx_schoice = working_seg_names.index(st.session_state.get(f"ld_single_{i}", working_seg_names[0])) if st.session_state.get(f"ld_single_{i}") in working_seg_names else 0
                    s_choice = st.selectbox("Select Segment", working_seg_names, index=idx_schoice, key=f"ld_single_{i}")
                    target_segments.append(working_seg_names.index(s_choice))
                elif target_mode == "Multiple Segments":
                    # 💡 فلتر الحماية ضد خطأ Multiselect 
                    raw_def_segs = st.session_state.get(f"ld_multi_{i}", ld.get('target_segs', []))
                    safe_def_segs = [s for s in raw_def_segs if s in working_seg_names]
                    
                    selected_segs = st.multiselect("Select Segments", working_seg_names, default=safe_def_segs, key=f"ld_multi_{i}")
                    ld['target_segs'] = selected_segs
                    target_segments = [working_seg_names.index(s) for s in selected_segs]
                else:
                    target_segments = list(range(len(working_segments)))
        ...
        if st.button("🚀 Run Advanced FEA & Generate Diagrams", type="primary", use_container_width=True):
            ...
            with st.spinner("Solving Finite Element Matrix..."):
                U_full, R_full = solve_fea_engine(nodes_base, elements_base, nodal_loads_base, supports_list_base) 
    ...
    if getattr(st.session_state, 'adv_solved', False):
        ...
        for i, sec in enumerate(fea_data['seg_sections']):
            ...
            safety_data.append({
                ...
                "Actual": f"M={max_m:.3f}, V={max_v:.3f}", "Allowable": f"M={sec['Mall']:.3f}, V={sec['Qall']:.3f}", "Status": s_status
            })
