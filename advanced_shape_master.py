...
# =====================================================================
# 🧠 THE AI OPTIMIZER ENGINE (GENERATIVE DESIGN FOR BRIDGE SHORING)
# =====================================================================
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v):
    ...
    # 💡 التعديل الجوهري: استنتاج الحدود الهندسية والمركز الحقيقي للرسمة من الـ DXF
    min_y = 9999.0
    max_x = -9999.0
    min_x = 9999.0
    ...
    for center_prop in [True, False]:
        current_segs = [spacings[0]] * int(math.ceil(half_width / spacings[-1]) + 2)
        
        for _ in range(30):
            coords = [0.0] if center_prop else []
            curr_x = current_segs[0]/2.0 if not center_prop else 0.0
            for seg in current_segs:
                curr_x += seg
                if curr_x <= half_width + 0.15: coords.append(curr_x)
            
            full_grid = set(coords)
            for cx in coords:
                if cx > 0: full_grid.add(-cx)
                
            # 💡 المركز الحقيقي: التعديل الذي يضبط الدعامات على مكان الكوبري الفعلي
            sorted_grid = sorted([center_x + x for x in full_grid])
            
            test_supps = []
            for idx, gx in enumerate(sorted_grid):
                stype = 'Hinged' if idx == 0 else 'Roller'
                test_supps.append({'x': round(gx, 3), 'y': round(min_y, 3), 'type': stype, 'angle': 0.0})
                
            max_ry, worst_x = run_trial(test_supps)
            
            if max_ry <= target_rxn:
                if len(test_supps) < min_count:
                    min_count = len(test_supps)
                    best_grid = test_supps
                break
                
            demoted = False
            rel_worst_x = abs(worst_x - center_x)
            cx_track = current_segs[0]/2.0 if not center_prop else 0.0
            
            for s_idx, seg in enumerate(current_segs):
                cx_track += seg
                if cx_track >= rel_worst_x - (seg * 1.5):
                    try:
                        s_val_idx = spacings.index(seg)
                        if s_val_idx < len(spacings) - 1:
                            current_segs[s_idx] = spacings[s_val_idx + 1]
                            demoted = True
                    except: pass
            
            if not demoted:
                for s_idx in range(len(current_segs)):
                    try:
                        s_val_idx = spacings.index(current_segs[s_idx])
                        if s_val_idx < len(spacings) - 1:
                            current_segs[s_idx] = spacings[s_val_idx + 1]
                            demoted = True
                    except: pass
                if not demoted: break
            
    if best_grid: return True, best_grid
    return False, "❌ Failed! Loads are too high even with minimum spacing."
...
def render_advanced_shape_module():
    ...
    with c_in:
        ...
        with st.expander("🛠️ Override specific segments section", expanded=False):
            override_segs = st.multiselect("Select segments to override:", base_seg_names, default=st.session_state.get("override_segs_adv", []), key="override_segs_adv")
            if override_segs:
        ...
        # ==========================================
        # 🤖 AI GENERATIVE DESIGN OPTIMIZER
        # ==========================================
        st.markdown("### 🤖 AI Auto-Shoring Optimizer")
        with st.expander("✨ Generative Design for Bridge Supports", expanded=False):
            st.info("The AI will run multiple FEA combinations in the background to find the most efficient symmetric support grid.")
            ...
            if st.button("✨ Run AI Optimizer & Generate Supports", type="primary", use_container_width=True):
                with st.spinner("🧠 AI is calculating optimal symmetric grid..."):
                    ...
                    if succ:
                        ...
                        st.success("✅ Optimum symmetric grid found and applied successfully!")
                        st.rerun()
                    else:
                        st.error(res)
        ...
        for i, ld in enumerate(st.session_state.ui_loads):
            with st.expander(f"📥 Load Item {i+1}", expanded=(i==0)):
                ...
                if target_mode == "Single Segment":
                    ...
                    # 💡 الحل الجذري لمشكلة Multiselect: فلترة الأسماء القديمة بصرامة
                    raw_def_segs = st.session_state.get(f"ld_multi_{i}", ld.get('target_segs', []))
                    safe_def_segs = [s for s in raw_def_segs if s in working_seg_names]
