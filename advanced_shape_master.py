# ==============================================================================
# ADVANCED SHAPE MASTER - ENGINEERING CALCULATION MODULE (Ultimate V25)
# ==============================================================================
...
def safe_render_fig(fig, ax=None, nodes=None):
    ...
    تم تعديلها لتعمل بخاصية margins بدلاً من set_ylim لتفادي الانهيار 
    عند وجود قيم متناهية الصغر في دياجرام الدفلكشن.
    """
    try:
        if ax is not None:
            ax.axis('equal')
...
def draw_reaction_arrow(ax, node_x, node_y, force_mag, axis_nx, axis_ny, is_bridge_mode=False):
    ...
    تمت إضافة فلتر لإخفاء قوى X في الكباري لتظهر كـ Roller بصرياً.
    """
        ...
        if m:
            ...
            if min_L <= req_length <= max_L:
                ...
                if allowable >= abs(req_axial_force):
                    valid_struts.append(
                        (s_name, allowable, get_strut_priority(s_name))
                    )
...
def generate_chain_report(sys_data):
    ...
    return out
# =====================================================================
# 🧠 THE HEURISTIC OPTIMIZER ENGINES (V24 - Fallback & Deflection Relaxation)
# =====================================================================
def run_bridge_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v, is_symmetric, opt_mode, status_text, progress_bar):
    ...
    # وضع الدعامات على Y الدنيا لضمان بقائها في السولجر السفلي الأفقي
    dummy_supps = [
    ...
    for p_count in sorted_p_keys:
        for actual_coords in grids_by_props[p_count]:
            ...
            for idx, gx in enumerate(actual_coords):
                # 💡 كلها Hinged وثابتة على min_y السولجر السفلي
                test_supps.append({'x': gx, 'y': min_y, 'type': 'Hinged', 'angle': 0.0})
...
def render_advanced_shape_module():
    ...
    with c_plot:
        ...
        # 💡 تم مسح OCR واستخدام الإكسيل لضمان الثبات
    ...
    if getattr(st.session_state, 'adv_solved', False):
        ...
        # 💡 الجداول الشاملة للأمان بصيغتها الشفافة
        safety_data = []
