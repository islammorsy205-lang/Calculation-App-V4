...
def build_chain_mesh(segments, seg_sections, loads, struts, supports, base_segments, mesh_size=0.25):
    ...
    # 💡 مسافة اللحام المغناطيسي الصارمة (تضمن اتصال النهايات بالفريمات)
    node_tol = 0.05 
...
def solve_fea_engine(nodes, elements, nodal_loads, supports_list):
    ...
    # 💡 حساب إجمالي الحمل الرأسي الواقع على الكوبري (للتأكد من الاتزان)
...
# =====================================================================
# 🧠 THE HEURISTIC OPTIMIZER ENGINE (PHYSICS-AWARE V13)
# =====================================================================
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v, is_symmetric):
    ...
    # تحديد الإحداثيات المطلقة لمسار النهايز لعمل جذب للسبورتات 
    strut_xs = set()
    for ds in ui_struts:
    ...
    # 💡 التصفية: إعدام أي رصة لا تمتلك الحد الأدنى من الدعامات المطلوبة فيزيائياً
    filtered_grids = [list(g) for g in unique_grids if len(g) >= min_required_props]

    if not filtered_grids:
        return False, None, f"❌ Requires at least {min_required_props} props to achieve {target_rxn} kN (Total Load is {total_system_load:.1f} kN), but spacings can't fit them! Decrease spacings."
    ...
    if best_fallback_grid:
        return True, best_fallback_grid, f"⚠️ Deep Search Timeout (60s). Shown best fallback with NO Uplift. Tested {trials_count} combos. Max Reaction: {best_fallback_score:.2f} kN (Target was {target_rxn})."
        
    return False, None, f"❌ Optimizer failed! Tested {trials_count} combinations. All generated UPLIFT."
...
def render_advanced_shape_module():
    ...
    with c_in:
        ...
        # ==========================================
        # 🤖 AI GENERATIVE DESIGN OPTIMIZER V13
        # ==========================================
        ...
        with st.expander("✨ Generative Design for Bridge Supports", expanded=False):
            ...
            if st.button("✨ Run AI Optimizer & Generate Supports", type="primary", use_container_width=True):
                with st.spinner("🧠 AI is executing Physics-Aware Deep Search (Max 60 Seconds)..."):
