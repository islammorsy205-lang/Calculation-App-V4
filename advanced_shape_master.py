...
# =========================================================
# 0. Helper Functions & Styles (دوال المساعدة والتنسيق)
# =========================================================
...
def crop_image_bbox(img_bytes):
    """قص حواف الصور الناتجة لجعلها متناسقة في التقرير"""
...
def draw_reaction_arrow(ax, node_x, node_y, force_mag, axis_nx, axis_ny):
    """رسم أسهم ردود الأفعال بدقة رقمين عشريين لمنع الزحام"""
    ...
    arr_c = 'blue' if force_mag >= 0 else 'red' # أزرق للضغط، أحمر للشد (Uplift)
    ...
    # تقريب الأرقام لخاننين عشريتين فقط
    ax.text(
...
def eval_seg_point(seg, s_val, start_data=None):
    """حساب الإحداثيات المطلقة X,Y لأي نقطة على أي فريم بناء على المسافة"""
...
def get_closest_segment_exact(pt, segs):
    """إيجاد أقرب فريم لأي نقطة مرسومة (تستخدم في اللحام المغناطيسي)"""
    min_d = 9999.0
    best_idx = 0
    best_s = 0.0
    pt = np.array(pt)
...
def extract_dxf_for_interactive(file_bytes):
    """قارئ الكاد العالمي: يستخرج الفريمات والنهايز والدعامات بدون التقيد الصارم بأسماء الطبقات"""
...
def parse_dxf_to_data(file_bytes):
    """تحويل البيانات الخام من الكاد إلى هياكل بيانات مفصلة"""
...
def perform_smart_division(base_segments, supports, struts):
    """تقسيم الفريمات لأغراض العرض فقط، ولكن اللحام الأصلي يتم في الماتريكس"""
...
def build_chain_mesh(segments, seg_sections, loads, struts, supports, base_segments, mesh_size=0.25):
    """بناء مصفوفة الـ Finite Element مع اللحام المغناطيسي المطور لربط الأحمال"""
    nodes = []
    elements = []
    nodal_loads = []
    
    # 💡 مسافة اللحام المغناطيسي 5 سم (تضمن ارتباط الهيكل المائل بالأفقي تماماً)
    node_tol = 0.05 
...
def solve_fea_engine(nodes, elements, nodal_loads, supports_list):
    """محرك المصفوفات لحل القوى واستخراج ردود الأفعال بدقة تامة"""
    NDOF = len(nodes) * 3
...
def draw_base_geometry(ax, nodes, elements, supports_list, seg_sections=None, segments=None, show_seg_names=False):
    ...
    # رسم خطوط الأبعاد الرمادية (Dimensions) أسفل الدعامات
...
# =====================================================================
# 🧠 THE HEURISTIC OPTIMIZER ENGINE (PHYSICS-AWARE V12)
# =====================================================================
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v, is_symmetric):
    ...
    for ds in ui_struts:
        bx = ds.get('bx', 0.0)
        strut_rel_xs.add(round(abs(bx - center_x), 3))
    ...
    combo_factors = {'Dead Load': fac_d, 'Live Load': fac_l, 'Wind Load': fac_w} # 💡 تصليح مشكلة الاسم هنا
    ...
    if is_symmetric:
        def build_sym_grids(current_grid):
            ...
            if 0.40 <= cantilever <= 1.50:
                ...
                valid_grids.append(actual_coords)
        ...
        def build_asym_grids(current_grid):
            ...
            if 0.40 <= cantilever <= 1.50:
                valid_grids.append(current_grid)
            if cantilever < 0.40: return
            for s in spacings:
    ...
    for g in valid_grids:
        if g not in unique_grids: unique_grids.append(g)

    def grid_score(grid):
        prop_count = len(grid) * 10.0 
        alignment_bonus = 0.0
        for sx in strut_rel_xs:
            sym_sx = center_x + sx
            min_dist = min([abs(px - sym_sx) for px in grid]) if grid else 99.0
            if min_dist <= 0.25: alignment_bonus += 5.0
            elif min_dist <= 0.60: alignment_bonus += 2.0
        return prop_count - alignment_bonus

    unique_grids.sort(key=grid_score)

    if not unique_grids:
        return False, None, "❌ Cannot form a valid grid with overhangs (0.4m - 1.5m). Adjust your allowed spacings."
    ...
    for actual_coords in unique_grids:
    ...
    if best_fallback_grid:
        return True, best_fallback_grid, f"⚠️ Deep Search Timeout (120s). Shown best fallback with NO Uplift. Tested {trials_count} combos. Max Reaction: {best_fallback_score:.2f} kN."
        
    return False, None, f"❌ Optimizer failed! Tested {trials_count} combinations. All generated UPLIFT or exceeded Target Reaction within 120s limit."
# =========================================================
...
def render_advanced_shape_module():
    ...
    with c_in:
        ...
        # ==========================================
        # 🤖 AI GENERATIVE DESIGN OPTIMIZER V12
        # ==========================================
        ...
        with st.expander("✨ Generative Design for Bridge Supports", expanded=False):
            ...
            if st.button("✨ Run AI Optimizer & Generate Supports", type="primary", use_container_width=True):
                with st.spinner("🧠 AI is executing Physics-Aware Deep Search (Max 120 Seconds)..."):
