...
def draw_base_geometry(ax, nodes, elements, supports_list, seg_sections=None, segments=None, show_seg_names=False):
    ...
    # =======================================================
    # 💡 إضافة خطوط الأبعاد (Dimensions) تحت الدعامات 💡
    # =======================================================
    if len(supports_list) > 1:
        ...
        if len(unique_xs) > 1:
            ...
            # خط الأبعاد سيكون تحت مثلث الدعامة بمسافة آمنة (y-0.9)
            dim_y = min_y - 0.9 
            
            # حساب حجم الخط ديناميكياً بناءً على عدد الدعامات (بين 4 و 7) ليكون دائماً مقروء
...
# =====================================================================
# 🧠 THE AI OPTIMIZER ENGINE (GENERATIVE DESIGN FOR BRIDGE SHORING) V5
# =====================================================================
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v):
    """
    الإصدار المدمر V5: 
    - التوليد الشامل لجميع الاحتمالات الرياضية الممكنة للمسافات للحصول على التطابق التام.
    - إجبار الكابولي (لا يقل عن 0.15 م ولا يزيد عن 1.20 م) لوزن العزوم وتجنب الـ Uplift.
    - الترتيب التصاعدي: البحث يبدأ بالشبكات التي تحتوي على *أقل عدد من الدعامات*.
    - الرفض الفوري الصارم لأي شبكة تنتج قوة شد (Uplift) في أي دعامة (رد الفعل يجب أن يكون موجب > 0.5 kN).
    """
    try:
    ...
    def run_trial(test_supps):
        ...
        for sup in slist_t:
            n = sup['node']
            ry = R[3*n + 1] # القوة الرأسية
            if ry > max_ry: max_ry = ry
            if ry < min_ry: min_ry = ry
                
        return max_ry, min_ry

    # 🧠 THE RECURSIVE GRID GENERATOR (Symmetrical, Exact Spacing, Cantilever-Aware)
    valid_half_grids = []
    
    def build_grids(current_grid):
        last_x = current_grid[-1]
        cantilever = half_width - last_x
        
        # 💡 إجبار الرفرفة (Cantilever) لمنع الـ Uplift
        if 0.15 <= cantilever <= 1.20:
            valid_half_grids.append(current_grid)
            
        # التوقف إذا تجاوزنا حافة الكوبري أو وصلنا لكابولي صغير جداً
        if cantilever < 0.15:
            return
            
        # تجربة جميع المسافات المتاحة للخطوة القادمة (Recursive Search)
        for s in spacings:
            build_grids(current_grid + [last_x + s])

    # بدء التوليد: إما بدعامة في السنتر، وإما بدون دعامة في السنتر (Straddle)
    build_grids([0.0])
    for s in spacings:
        build_grids([s / 2.0])
    ...
    # 💡 السر في توفير الدعامات: ترتيب الشبكات حسب (أقل عدد دعامات) أولاً
    unique_grids.sort(key=lambda g: len(g))
...
def render_advanced_shape_module():
    ...
    with c_in:
        ...
        # ==========================================
        # 🤖 AI GENERATIVE DESIGN OPTIMIZER V5
        # ==========================================
