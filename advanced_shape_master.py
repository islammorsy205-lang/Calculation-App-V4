...
def draw_base_geometry(ax, nodes, elements, supports_list, seg_sections=None, segments=None, show_seg_names=False):
    ...
    # 💡 خطوط الأبعاد الديناميكية (Dimensions)
...
# =====================================================================
# 🧠 THE AI OPTIMIZER ENGINE (GENERATIVE DESIGN FOR BRIDGE SHORING) V6
# =====================================================================
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v):
    ...
    - إجبار الكابولي (لا يقل عن 0.15 م ولا يزيد عن 1.50 م) لوزن العزوم وتجنب الـ Uplift.
    ...
    if not unique_grids:
        return False, "❌ Cannot form a valid symmetric grid with proper overhangs (0.15m - 1.50m). Adjust your allowed spacings."
    ...
    for grid in unique_grids:
        ...
        # حماية إضافية: التأكد التام من أن الإحداثيات داخل حدود الرسمة الفعلي
        actual_coords = [x for x in actual_coords if min_x - 0.05 <= x <= max_x + 0.05]
...
def render_advanced_shape_module():
    ...
    with c_in:
        ...
        for i, sup in enumerate(st.session_state.ui_supports):
            ...
            # 💡 السلاح النووي: إضافة opt_v للمفتاح يضمن تحديث واجهة Streamlit وعدم تداخل الأرقام القديمة
        ...
        for i, ds in enumerate(st.session_state.ui_struts):
            with st.expander(f"📏 Strut P{i+1}", expanded=True):
                ...
                s_idx_str = cc1.selectbox("Snap to Base Seg", base_seg_names, index=base_seg_names.index(s_name_guess), key=f"st_s_{i}_{dxf_v}_{opt_v}")
                ...
                ds['s_dist'] = cc2.number_input("Dist from Start (m)", 0.0, max_s_strut, value=min(float(ds.get('s_dist', 1.0)), max_s_strut), format="%.3f", step=0.1, key=f"st_d_{i}_{dxf_v}_{opt_v}")
                ds['gx'] = cc3.number_input("Base X (rel 0,0)", value=float(ds.get('gx', 3.0)), format="%.3f", step=0.1, key=f"st_gx_{i}_{dxf_v}_{opt_v}")
                ...
                ds['sec'] = cc4.selectbox(f"Type (L={actual_L:.3f}m)", valid_opts, index=idx_sec, key=f"st_sec_{i}_{dxf_v}_{opt_v}")
                
                cc5.markdown("<br>", unsafe_allow_html=True)
                if cc5.button("❌", key=f"del_st_{i}_{opt_v}"):
        ...
        # ==========================================
        # 🤖 AI GENERATIVE DESIGN OPTIMIZER V6
        # ==========================================
            ...
            if st.button("✨ Run AI Optimizer & Generate Supports", type="primary", use_container_width=True):
                with st.spinner("🧠 AI is calculating the ultimate symmetric & stable grid..."):
                    ...
                    if succ:
                        ...
                        st.session_state.opt_v += 1  # 💥 السلاح السري لقتل الذاكرة الشبحية ومنع التداخل
