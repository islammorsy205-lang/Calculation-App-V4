...
def extract_dxf_for_interactive(file_bytes):
    ...
    try:
        ...
        for e in msp:
            ...
            # 1. أي نقطة = Support (بدون اشتراط اللاير)
...
def parse_dxf_to_data(file_bytes):
    ...
    # اللحام الفعلي: تخزين الإحداثيات المطلقة للـ Struts لربطها بالـ Mesh لاحقاً بدلاً من المسافات التقريبية
    struts_mapped = []
    for s in raw_struts:
        p1, p2 = s['p1'], s['p2']
...
def perform_smart_division(base_segments, supports, struts):
    ...
    # التقطيع المبدئي للرسم فقط (اللحام الحقيقي في الـ Mesh)
    for sp in supports:
...
def build_chain_mesh(segments, seg_sections, loads, struts, supports, base_segments, mesh_size=0.25):
    ...
    # 💡 مسافة اللحام المغناطيسي 5 سم (السر الأعظم لانتقال الأحمال بشكل صحيح)
    node_tol = 0.05 
    ...
    # إجبار نقاط الدعامات (Struts) لتكسير الفريمات العلوية والسفلية لضمان نقل الحمل 100%
    ...
    # لحام النهايز بالـ Nodes التي تم تكسيرها خصيصاً لها
            ...
            with st.expander(f"📥 Load Item {i+1}", expanded=(i==0)):
                ...
                for s_idx_num in target_working_indices:
                    ...
                    combined_loads.append({
                        'seg_idx': s_idx_num, 'category': ld['category'], 'type': ld['type'], 'dir': ld['dir'], 
                        'start': 0.0, 'end': max_s if ld['type'] != 'Point Load' else 0.0, 
                        'w1': ld['w1'] * combo_facs[ld['category']], 'w2': ld['w2'] * combo_facs[ld['category']]
                    })
