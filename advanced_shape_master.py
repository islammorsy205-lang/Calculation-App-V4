# ==============================================================================
# ADVANCED SHAPE MASTER - ENGINEERING CALCULATION MODULE (Ultimate Version)
# ==============================================================================
...
# manual parametric generation, exact FEA matrix solving, and dual-core generative 
# AI optimization (Bridge & Single-Sided L-Frame), plus OCR automated load assignment.
# ==============================================================================
...
def safe_render_fig(fig, ax=None, nodes=None):
    ...
    💡 تم إضافة العدسة الذكية (Auto-Scaling) لضبط الـ Zoom بناءً على الإحداثيات!
    """
    try:
        if ax is not None and nodes is not None and len(nodes) > 0:
            ...
            # بادينج ذكي يتناسب مع أبعاد الموديل
            pad_x = max(1.5, dx * 0.15)
            pad_y = max(1.5, dy * 0.15)
...
def get_strut_priority(name):
    ...
    if "PPS" in name_u: 
        ...
        score = 90 # 💡 Deprioritized totally based on constraints
    elif "MMP" in name_u: 
        score = 90 # 💡 Deprioritized totally based on constraints
    elif "MNB" in name_u: 
        score = 95
...
def get_optimal_strut_section(req_length, req_axial_force, system_type="Bridge"):
    """
    الدالة الذكية لاختيار أنسب قطاع نهيز (In-Place Upgrade) بناءً على الطول والقوة.
    ترفض الـ TILT والـ MMP تماماً، وتبدأ بـ PPS، وإذا فشل تنتقل لـ PPH.
    """
    valid_struts = []
        ...
        if m:
            ...
            if min_L <= req_length <= max_L:
                ...
                if allowable >= abs(req_axial_force):
                    valid_struts.append((s_name, allowable, get_strut_priority(s_name)))
                    
    if not valid_struts: 
        return None
        
    # Sort by priority (PPS first), then capacity
    valid_struts.sort(key=lambda x: (x[2], x[1])) 
    return valid_struts[0][0]
...
def plot_sap2000_diagrams(nodes, elements, R_reactions, scales, display_nodes, supports_list, seg_sections, loads, segments=None, ss_spacings=None):
    apply_plot_styles()
    figs_dict = {}
...
def generate_chain_report(sys_data):
    ...
    add_line("1. Safety Checks:", bold=True)
    ...
    return out
# =====================================================================
# 🧠 THE HEURISTIC OPTIMIZER ENGINES (V19 - Generative AI & Dual Core)
# =====================================================================
def run_bridge_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v, is_symmetric, opt_mode, status_text, progress_bar):
    ...
    if is_symmetric:
        ...
        cantilever_opts = np.arange(0.40, 1.51, 0.10)
    ...
    for p_count in sorted_p_keys:
        for actual_coords in grids_by_props[p_count]:
            ...
            if is_symmetric:
                ...
                cantilever_L = max_x - max(actual_coords)
                
            excluded_zone_start = max_x - (cantilever_L / 3.0)
            excluded_zone_start_left = min_x + (cantilever_L / 3.0)
            
            test_supps = []
            for idx, gx in enumerate(actual_coords):
                if idx == 0:
                    test_supps.append({'x': gx, 'y': round(min_y, 3), 'type': 'Hinged', 'angle': 0.0})
                else:
                    test_supps.append({'x': gx, 'y': round(min_y, 3), 'type': 'Roller', 'angle': 0.0})
            ...
            for shift_val in shift_options:
                ...
                for strut in ui_struts:
                    ...
                    if bx_val >= center_x:
                        new_bx = bx_val + shift_val
                    else:
                        new_bx = bx_val - shift_val
                    
                    if new_bx > excluded_zone_start: 
                        new_bx = excluded_zone_start - 0.1
                    if new_bx < excluded_zone_start_left: 
                        new_bx = excluded_zone_start_left + 0.1
                ...
                if min_ry >= 0.5 and soldier_safe and struts_safe:
    ...
    if best_fallback_grid:
        if "Deep" in opt_mode: 
            return False, best_fallback_grid, best_fallback_struts, f"❌ Target {target_rxn} kN FAILED. Best safe grid reaction is {best_fallback_score:.2f} kN."
        else: 
            return True, best_fallback_grid, best_fallback_struts, f"⚠️ Quick Timeout. BEST SAFE FALLBACK: {best_fallback_score:.2f} kN."
            
    return False, None, None, f"❌ Failed! Cannot satisfy Soldier safety and Uplift."


def run_single_sided_optimizer(base_segments, working_segments, active_seg_sections, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v, status_text, progress_bar):
    try: 
        spacings_raw = spacings_str.split(',')
        spacings = []
        for x in spacings_raw:
        ...
        return False, None, None, None, "❌ Format error in spacings."
    ...
    for k in strut_counts:
        ...
        for h_grid in valid_h_grids:
            test_supps = []
            test_supps.append({'x': cx, 'y': cy, 'type': 'Hinged', 'angle': -45.0}) 
            for hx in h_grid: 
                test_supps.append({'x': round(hx, 3), 'y': cy, 'type': 'Roller', 'angle': 0.0})
            ...
            for y_tup in y_combos:
                for x_tup in x_combos:
                    ...
                    status_text.markdown(f"**⏳ Single-Sided Search:** Testing {k} Struts | Horizontal Supports: {len(test_supps)-1} | Lowest Tie Rxn: **{best_fallback_tie:.2f} kN**")
    ...
    if best_struts:
        ...
        return False, best_supps, best_struts, spacing_data, f"❌ Failed to reach 180kN. Best safe structure yields Tie Reaction = {best_fallback_tie:.2f} kN. Consider extending base."
    
    return False, None, None, None, "❌ Optimizer failed completely."
...
def render_advanced_shape_module():
    ...
    with c_in:
        ...
        with opt_tab1:
            st.info("The AI optimizes bridge supports. Checks Soldier safety, prevents struts in outer 1/3, and upgrades sections automatically.")
        ...
        with opt_tab2:
            st.info("The AI builds Single-Sided Strongbacks dynamically. Enforces 180kN Max Tie-Rod Reaction (Corner Gatekeeper) and optimizes horizontal supports.")
            ai_spc_str_ss = st.text_input("Allowed Horizontal Support Spacings (m) [Comma Sep]", value="2.40, 2.10, 1.80, 1.50, 1.20, 0.90, 0.60", key='ss_spc')
            ...
            if st.button("✨ Run Single-Sided L-Frame AI Optimizer", type="primary", use_container_width=True):
                ...
                with st.spinner("🧠 AI is building L-Frame Matrix & Deploying 180kN Gatekeeper..."):
                    succ, res_supps, res_struts, spc_data, msg = run_single_sided_optimizer(
                        base_segments, working_segments, active_seg_sections, 
                        st.session_state.ui_loads, 180.0, ai_spc_str_ss, view_plane, 
    ...
    with c_plot:
        ...
        with st.expander("📸 Smart Auto-Load Assigner (Excel & OCR)", expanded=False):
            ...
            st.markdown("---")
            img_file = st.file_uploader("🖼️ Or Upload Snipping Image:", type=['png', 'jpg', 'jpeg'], key="ocr_img")
            if img_file and st.button("🔍 Read Image with OCR"):
                if pytesseract is None: 
                    st.error("⚠️ 'pytesseract' is missing.")
                else:
                    with st.spinner("Analyzing image..."):
                        try:
                            found_loads = 0
                            img_obj = Image.open(img_file)
                            text_out = pytesseract.image_to_string(img_obj)
                            ...
                            if found_loads > 0: 
                                st.success(f"✅ OCR Mapped {found_loads} Loads.")
                                st.rerun()
                        except Exception as e: 
        ...
        for i, ld in enumerate(st.session_state.ui_loads):
            with st.expander(f"📥 Load Item {i+1}", expanded=(i==0)):
                ...
                ld['type'] = col_l2.selectbox("Type", type_opts, index=idx_typ, key=f"ld_t_{i}_{dxf_v}")
                ...
                for w_idx, w_seg in enumerate(working_segments):
                    if w_seg.get('master_idx', 0) in target_base_indices:
                        ...
                        e_val = L_w if ld['type'] != 'Point Load' else 0.0
    ...
    if getattr(st.session_state, 'adv_solved', False):
        ...
        # 💡 الجداول الشاملة للأمان (السولجرات، الزراجين، والنحايز)
        safety_data = []
