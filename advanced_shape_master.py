...
def plot_sap2000_diagrams(nodes, elements, R_reactions, scales, display_nodes, supports_list, seg_sections, loads, segments=None):
    ...
    for sup in supports_list:
        ...
        if t == 'Roller': draw_reaction_arrow(ax_r, x, y, R_loc_y, -s_a, c_a)
        else:
...
def generate_chain_report(sys_data):
    ...
    def add_line(text, bold=False):
        p = doc.add_paragraph(); force_ltr_left(p); p.paragraph_format.line_spacing = 1.5
        r = p.add_run(text); r.font.name = 'Arial'; r.font.size = Pt(12); r.font.bold = bold; r.font.rtl = False
    def add_large_diagram(doc, img_bytes, title):
        p_img = doc.add_paragraph(); p_img.alignment = WD_ALIGN_PARAGRAPH.CENTER
        r_img = p_img.add_run(); r_img.add_picture(io.BytesIO(img_bytes), width=Cm(15.0))
        p_title = doc.add_paragraph(); p_title.alignment = WD_ALIGN_PARAGRAPH.CENTER
        r_title = p_title.add_run(title); r_title.font.name = 'Arial'; r_title.font.size = Pt(12); r_title.bold = True
        p_line = doc.add_paragraph(); p_line.alignment = WD_ALIGN_PARAGRAPH.CENTER
        r_line = p_line.add_run("_" * 60); r_line.font.size = Pt(8); r_line.font.color.rgb = RGBColor(150, 150, 150)
        doc.add_paragraph()
    ...
    for df_row in sys_data['safety_df']: add_line(f"- {df_row['Component']} ({df_row['Force Type']}): {df_row['Actual']} vs {df_row['Allowable']} => {df_row['Status']}")
    doc.add_page_break()
...
# =====================================================================
# 🧠 THE AI ITERATIVE DEEPENING OPTIMIZER (V15 ULTIMATE PHYSICS)
# =====================================================================
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v, is_symmetric):
    try: spacings = sorted([float(x.strip()) for x in spacings_str.split(',')], reverse=True)
    ...
    # 💡 فلتر المستحيلات الرياضي: حساب إجمالي الحمل لإقصاء الشبكات الضعيفة فوراً!
    ...
    # التقييم العبقري: ترتيب داخل كل مجموعة حسب تمركز الدعامات تحت النهايز
    def alignment_score(grid):
        bonus = 0.0
        for sx in strut_xs:
            if grid:
                min_dist = min([abs(px - sx) for px in grid])
                if min_dist <= 0.15: bonus += 50.0   
                elif min_dist <= 0.40: bonus += 20.0
                elif min_dist <= 0.80: bonus += 5.0
        return -bonus  
    ...
    for p_count in sorted_p_counts:
        ...
        for actual_coords in grids_by_props[p_count]:
            ...
            # 💡 النجاح المطلق الذي يحترم المسموح به تماماً
    ...
    if best_fallback_grid:
        return False, best_fallback_grid, f"❌ STRICT LIMIT FAILED: Couldn't achieve <= {target_rxn} kN within time. Best safe grid reaction is {best_fallback_score:.2f} kN. Try smaller spacings to allow more props."
...
def render_advanced_shape_module():
    ...
    with c_in:
        ...
        # ==========================================
        # 🤖 AI GENERATIVE DESIGN OPTIMIZER V15
        # ==========================================
        ...
        with st.expander("✨ Generative Design for Bridge Supports", expanded=False):
            ...
            if st.button("✨ Run AI Optimizer & Generate Supports", type="primary", use_container_width=True):
                with st.spinner("🧠 AI is executing Iterative Deep Search..."):
