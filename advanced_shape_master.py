# ==============================================================================
# ADVANCED SHAPE MASTER - ENGINEERING CALCULATION MODULE (Ultimate V26)
# ==============================================================================
...
# تم إزالة مكتبة pytesseract نهائياً لتخفيف الواجهة وتقليل الأعطال
...
def safe_render_fig(fig, ax=None, nodes=None):
    ...
    تعتمد على margins بدلاً من set_ylim لتفادي أي انهيار بسبب القيم الصغيرة
    """
    try:
        if ax is not None:
            ax.axis('equal')
...
def draw_reaction_arrow(ax, node_x, node_y, force_mag, axis_nx, axis_ny, is_bridge_mode=False):
    ...
    فلتر لإخفاء قوى X في الكباري لتظهر كـ Roller بصرياً.
    """
...
def get_strut_priority(name):
    ...
    💡 تم تفعيل الحظر لجميع المقاسات التي تنتهي بـ 1 أو 3.
    """
    name_u = name.upper()
    score = 100
    
    # فلتر الاستبعاد القاطع للنحايز الفردية
    base_name = name.split('(')[0].strip()
    if bool(re.search(r'(1|3)$', base_name)):
        return 999  # أولوية سيئة جداً لتجاهلها تماماً
...
def get_optimal_strut_section(req_length, req_axial_force, system_type="Bridge"):
    ...
    for s_name, s_props in STRUTS_DB.items():
        ...
        if priority == 999:  # تجاهل النحايز المستبعدة 
            continue
...
def plot_sap2000_diagrams(nodes, elements, R_reactions, scales, display_nodes, supports_list, seg_sections, loads, segments=None, ss_spacings=None, is_bridge_opt=False):
    ...
    # لم نعد نرسم دياجرام الدفلكشن بناءً على أوامر المهندس لتخفيف الواجهة
...
def generate_chain_report(sys_data):
    ...
    return out
# =====================================================================
# 🧠 THE HEURISTIC OPTIMIZER ENGINES (V25 - Consultant Force Fallback)
# =====================================================================
def run_bridge_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v, is_symmetric, opt_mode, status_text, progress_bar):
    ...
    def is_safe_soldier(elems, sections):
        # 💡 الدفلكشن ليس شرطاً للنجاح (Moment & Shear Only) لمرونة البحث
    ...
    # وضع الدعامات على Y الدنيا لضمان بقائها في السولجر السفلي الأفقي فقط
    dummy_supps = [
        ...
        for actual_coords in grids_by_props[p_count]:
            ...
            for shift_val in shift_options:
                ...
                for strut in ui_struts:
                    ...
                    if STRUTS_DB:
                        # 💡 منع استخدام النحايز الفردية كبداية
                ...
                # 💡 حفظ أفضل محاولة مهما كانت، لإجبار عرض الحل لاحقاً
    ...
    # 💡 إجبار ظهور الحل البديل دائماً للمعاينة حتى لو فشل
...
def run_single_sided_optimizer(base_segments, working_segments, active_seg_sections, ui_loads, target_rxn, view_plane, auto_mesh_size, dxf_v, status_text, progress_bar):
    ...
    strut_counts = [2, 3, 4, 5] # 💡 سماح بـ 5 نهايز في L-Frame لتقليل العزوم
...
def render_advanced_shape_module():
    ...
    with c_plot:
        ...
        for i, ld in enumerate(st.session_state.ui_loads):
            # 💡 استخدام Expander لطي الأحمال المتكررة ومنع الزحام
            with st.expander(f"📥 Load Item {i+1}", expanded=(i==len(st.session_state.ui_loads)-1)):
    ...
    if getattr(st.session_state, 'adv_solved', False):
        ...
        # 💡 الجداول الشاملة للأمان بعد إزالة الدفلكشن
        safety_data = []
