# ==============================================================================
# ADVANCED SHAPE MASTER - ENGINEERING CALCULATION MODULE (Ultimate V22)
# ==============================================================================
...
def safe_render_fig(fig, ax=None, nodes=None):
    """
    العدسة الذكية المحدثة (Smart Bounding Box Auto-Scaling): 
    تضمن أن الرسمة متناسقة في الأبعاد الحقيقية وتحجم الـ Bounding Box لمنع تمطيط الشاشة
    """
    try:
        if ax is not None and nodes is not None and len(nodes) > 0:
            ...
            # بادينج ذكي يتناسب مع أبعاد الموديل لمنع التصاق الأبعاد بالحواف (15% مسافة أمان)
            pad_x = max(1.5, dx * 0.15)
            pad_y = max(1.5, dy * 0.15)
            
            # إجبار التناسب الحقيقي لعدم تمطيط الرسمة
...
def draw_reaction_arrow(ax, node_x, node_y, force_mag, axis_nx, axis_ny, is_bridge_mode=False):
    ...
    تمت إضافة فلتر لإخفاء قوى X في الكباري لتظهر كـ Roller بصرياً رغم كونها Hinged إنشائياً.
    """
    if abs(force_mag) < 0.001:
        return
        
    # إذا كنا في الكباري، والقوة الأفقية مرسلة، نتجاهلها لتبسيط الرسمة!
...
def get_shifted_coords_along_segment(px, py, ds, segs):
    ...
    تأخذ إحداثيات نقطة، وتبحث عن الفريم الذي تقع عليه، 
    وتقوم بتحريك النقطة بمقدار (ds) على نفس استقامة ומيل هذا الفريم!
    """
    if abs(ds) < 1e-4: 
        return px, py
    ...
    # لو النقطة بعيدة جداً عن أي فريم، حركها أفقياً كخيار بديل
    if d_min > 0.5: 
        return px + ds, py
        
    seg = segs[best_idx]
    
    # حساب المسافة الجديدة والتأكد أنها لا تخرج خارج الفريم
    new_s = best_s + ds
...
def get_strut_priority(name):
    ...
    يتم تجاهل الـ TILT والـ MMP تماماً بناءً على أوامر المهندس.
    """
    name_u = name.upper()
    score = 100
...
def solve_fea_engine(nodes, elements, nodal_loads, supports_list):
    ...
    يحل العزوم، الشير، الأكسيال، و 💡 تم إضافة حساب الترخيم الموضعي (Local Deflection) باستخدام الـ Shape Functions.
    """
    NDOF = len(nodes) * 3
    ...
    for el in elements:
        ...
        if el['type'] == 'truss':
            ...
            for i, x in enumerate(xs):
                ...
                # 💡 معادلة الترخيم (Deflection) باستخدام Shape Functions
...
def draw_live_preview(nodes, elements, supports_list, seg_sections, loads, segments=None):
    ...
    # تفعيل العدسة الذكية لإجبار التناسق
...
def plot_sap2000_diagrams(nodes, elements, R_reactions, scales, display_nodes, supports_list, seg_sections, loads, segments=None, ss_spacings=None):
    ...
    # 💡 الكباري يتم إخفاء الـ X reaction منها، الحوائط تظهر كاملاً
    is_bridge = True if ss_spacings is None else False
    ...
    for sup in supports_list:
        ...
        if is_bridge:
            draw_reaction_arrow(ax_r, x, y, R_loc_y, -s_a, c_a, is_bridge_mode=True)
        else:
            if t == 'Roller': 
                draw_reaction_arrow(ax_r, x, y, R_loc_y, -s_a, c_a, is_bridge_mode=False)
            else:
                draw_reaction_arrow(ax_r, x, y, R_loc_x, c_a, s_a, is_bridge_mode=False)
                draw_reaction_arrow(ax_r, x, y, R_loc_y, -s_a, c_a, is_bridge_mode=False)
    ...
    # 💡 دياجرام الدفلكشن المستقل
    figs_dict['D'] = create_force_plot('D', scales.get('D', 0.015), 'purple', 'magenta')
...
def generate_chain_report(sys_data):
    ...
    add_line("1. Safety Checks (Moment, Shear, Deflection & Struts):", bold=True)
    ...
    if 'D' in bufs:
        add_large_diagram(doc, bufs['D'], "Deflection Diagram (mm)")
    
    out = io.BytesIO()
    doc.save(out)
    return out
# =====================================================================
# 🧠 THE HEURISTIC OPTIMIZER ENGINES (V22 - Generative AI & Parallel Shifting)
# =====================================================================
...
def run_bridge_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v, is_symmetric, opt_mode, status_text, progress_bar):
    ...
    # 💡 تحديد حدود السولجر السفلي المائل بدقة (لاستبعاد الأجنحة العلوية)
    ...
    def get_y_on_bottom_chord(test_x):
        """
        لصق الدعامة بشكل دقيق في السولجر السفلي حتى وإن كان مائلاً
        """
        for seg in base_segments:
            if seg.get('Shape Type') == 'Straight Line':
                p1 = seg['abs_p1']
    ...
    def is_safe_soldier(elems, sections):
        # 💡 تم إضافة فحص الـ Deflection للسولجرات بجانب العزوم والشير
        for i, sec in enumerate(sections):
            ...
            for el in elems:
                if el.get('group') == 'segment' and el.get('seg_idx') == i:
                    ...
                    d_arr = el.get('internal', {}).get('D', [0])
                    max_m = max(max_m, np.max(np.abs(m_arr)))
                    max_v = max(max_v, np.max(np.abs(v_arr)))
                    max_d = max(max_d, np.max(np.abs(d_arr)))
                    
            L_seg = float(working_segments[i].get('L', 3.0))
            all_d = (L_seg / 200.0) * 1000.0 if L_seg > 0 else 999.0
            
            if max_m > sec['Mall'] or max_v > sec['Qall'] or max_d > all_d: 
                return False
        return True
    ...
    dummy_supps = [{'x': soldier_min_x, 'y': min_y, 'type': 'Hinged'}, {'x': soldier_max_x, 'y': min_y, 'type': 'Hinged'}]
    ...
    # 💡 سماحيات الزحزحة الموازية (Parallel Shift Distances in meters)
        ...
        for actual_coords in grids_by_props[p_count]:
            ...
            for shift_val in shift_options:
                ...
                for strut in ui_struts:
                    ...
                    # 💡 زحزحة ذكية موازية لكلا النقطتين
                    ...
                    # تطبيق المنع للثلث الأخير على النقطة السفلية للكوبري
                    if nx_b > excluded_zone_start or nx_b < excluded_zone_start_left: 
                        nx_b, ny_b = strut['bx'], strut['by'] # تراجع عن الحركة إذا خالفت
                    ...
                    if STRUTS_DB:
                        new_strut['sec'] = "PPS 252 (2.14:2.84m)" # Default start
                    else:
...
def run_single_sided_optimizer(base_segments, working_segments, active_seg_sections, ui_loads, target_rxn, view_plane, auto_mesh_size, dxf_v, status_text, progress_bar):
    """
    💡 الميزة الجديدة للـ Strongback:
    خوارزمية لا تتطلب إدخال المسافات، وتقوم بزحزحة موازية للنهايز.
    """
    if not base_segments: 
                ...
                for x_tup in x_combos:
                    ...
                    for shift_val in shift_options:
                        ...
                        for idx in range(k):
                            ...
                            # نقاط الارتكاز الأساسية
                            base_tx, base_ty = cx, cy + y_tup[idx]
                            base_bx, base_by = cx + x_tup[idx], cy
                            
                            # الزحزحة الموازية الميكانيكية
                            nx_b, ny_b = get_shifted_coords_along_segment(base_bx, base_by, shift_val, base_segments)
                            nx_t, ny_t = get_shifted_coords_along_segment(base_tx, base_ty, shift_val, base_segments)
                            ...
                            # وضع الدعامة الأفقية تحت الـ bx المحدث
                            test_supps.append({'x': round(nx_b, 3), 'y': cy, 'type': 'Roller', 'angle': 0.0})
                            
                        # الدعامة الإضافية للاتزان في نهاية السولجر
...
def render_advanced_shape_module():
    ...
    with c_plot:
        ...
        # 💡 تم مسح قسم الـ OCR والـ Snipping بالكامل لتنظيف الواجهة وتثبيتها.
    ...
    if getattr(st.session_state, 'adv_solved', False):
        ...
        with st.expander("⚙️ Diagram Scale Controls", expanded=False):
            ...
            sc_d = c_s4.slider("Deflection Scale", 0.001, 0.100, float(st.session_state.get("adv_sc_d", 0.015)), step=0.001, key="adv_sc_d")
            
        img_bufs = plot_sap2000_diagrams(
            fea_data['nodes'], fea_data['elements'], fea_data['R'], 
            {'N': sc_n, 'V': sc_v, 'M': sc_m, 'D': sc_d}, 
        ...
        c_p2.image(img_bufs['D'], use_container_width=True)
        c_p2.markdown(f"<p align='center' style='font-weight:bold; border-bottom:1px solid #ccc;'>{titles['D']}</p>", unsafe_allow_html=True)
        
        c_p3, c_p4 = st.columns(2)
        c_p3.image(img_bufs['M'], use_container_width=True)
        c_p3.markdown(f"<p align='center' style='font-weight:bold; border-bottom:1px solid #ccc;'>{titles['M']}</p>", unsafe_allow_html=True)
        
        c_p4.image(img_bufs['V'], use_container_width=True)
        c_p4.markdown(f"<p align='center' style='font-weight:bold; border-bottom:1px solid #ccc;'>{titles['V']}</p>", unsafe_allow_html=True)
        
        c_p5, c_p6 = st.columns(2)
        c_p5.image(img_bufs['N'], use_container_width=True)
        c_p5.markdown(f"<p align='center' style='font-weight:bold; border-bottom:1px solid #ccc;'>{titles['N']}</p>", unsafe_allow_html=True)
        
        # 💡 الجداول الشاملة للأمان (السولجرات Moment, Shear, Deflection)
        safety_data = []
        for i, sec in enumerate(fea_data['seg_sections']):
            ...
            if max_m <= sec['Mall'] and max_v <= sec['Qall'] and max_d <= all_d:
                s_stat = "SAFE"
            else:
                s_stat = "UNSAFE"
            ...
            safety_data.append({
                ...
                "Force Type": "M, V, D", 
                "Actual": f"M={max_m:.2f}, V={max_v:.2f}, D={max_d:.2f}", 
                "Allowable": f"M={sec['Mall']:.2f}, V={sec['Qall']:.2f}, D={all_d:.2f}", 
                "Status": s_stat
            })
