# ==============================================================================
# ADVANCED SHAPE MASTER - ENGINEERING CALCULATION MODULE
# ==============================================================================
...
# mesh generation, precise FEA (Finite Element Analysis) matrix solving,
# generative AI optimization (Heuristic Dual-Core), and automated load assignment.
# ==============================================================================
...
try:
    ...
    pytesseract = None
    st.warning("⚠️ مكتبة 'pytesseract' غير موجودة. لتحليل الصور، اكتب 'pip install pytesseract' وتأكد من تثبيت برنامج Tesseract-OCR على الويندوز.")
...
def get_short_name(sec_name):
    ...
    """
    cleaned_name = re.sub(r'\s*\(.*?\)', '', sec_name)
...
def safe_render_fig(fig):
    """
    دالة تحويل الرسمة إلى صورة وحفظها بأعلى جودة ممكنة 
    مع عمل Crop للهوامش البيضاء
    """
    try:
        plt.subplots_adjust(
            left=0.01, 
...
def draw_reaction_arrow(ax, node_x, node_y, force_mag, axis_nx, axis_ny):
    """
    رسم أسهم ردود الأفعال بدقة رقمين عشريين لمنع الزحام البصري
    مع تمييز قوة الشد (Uplift) باللون الأحمر والضغط باللون الأزرق
    """
    ...
    # اللون الأزرق للضغط (آمن)، والأحمر للشد (Uplift خطير)
    if force_mag >= 0:
        arr_c = 'blue'
    else:
        arr_c = 'red'
    ...
    # تقريب الأرقام لخاننين عشريتين فقط حسب الطلب المسبق
    ax.text(
        start_x - 0.15 * dx, 
        start_y - 0.15 * dy, 
        f"{force_mag:+.2f}", 
...
def eval_seg_point(seg, s_val, start_data=None):
    ...
    تُستخدم في توقيع الأحمال والتقطيع الذكي للفريمات
    """
...
def get_closest_segment_exact(pt, segs):
    """
    إيجاد أقرب فريم لأي نقطة مرسومة
    تستخدم هذه الدالة أساساً في اللحام المغناطيسي للنهايز والدعامات لضمان عدم فصل الموديل
    """
    min_d = 9999.0
    best_idx = 0
    best_s = 0.0
...
def get_strut_priority(name):
    ...
    الـ PPS له الأولوية القصوى بناءً على طلب المهندس
    ثم الـ PPH في المرتبة الثانية، ثم الباقي
    """
    name_u = name.upper()
    score = 100
    ...
    is_odd_regex_1 = bool(re.search(r'(1|3)\s*\(', name))
    is_odd_regex_2 = bool(re.search(r'(1|3)$', name.split('(')[0].strip()))
    
    if is_odd_regex_1 or is_odd_regex_2: 
        score += 5
        
    return score

# =========================================================
# 1. THE DXF PARSER (Universal Rules with Absolute Struts)
# =========================================================
def extract_dxf_for_interactive(file_bytes):
    """
    قارئ الكاد العالمي (Universal DXF Parser): 
    يستخرج الفريمات والنهايز والدعامات بدون التقيد الصارم بأسماء الطبقات،
    بحيث يتم التعرف على العناصر هندسياً.
    """
    if ezdxf is None: 
        return None
    ...
    try:
        ...
        for e in msp:
            ...
            # 1. Any point or circle acts as a mandatory SUPPORT
            if dxftype in ['POINT', 'CIRCLE', 'INSERT']:
                if dxftype == 'POINT': 
                    ...
                    supports.append({'x': e.dxf.insert.x, 'y': e.dxf.insert.y})
                continue 
            ...
            for sub_e in entities:
                ...
                # 2. Push/Pull strings in layer name define STRUTS
                ...
                # 3. Everything else defaults to main FRAMES
                else:
                    if sub_type == 'LINE':
                        frames.append({
                            'type': 'line', 
...
def parse_dxf_to_data(file_bytes):
    """
    تحويل البيانات الخام من الكاد إلى هياكل بيانات مفصلة هندسياً (Segments Dict)
    """
    ...
    for idx, f in enumerate(raw_frames):
        if f['type'] == 'line':
            ...
            # ترتيب النقط من اليسار لليمين للتوحيد
    ...
    # اللحام الفعلي الأولي: تخزين الإحداثيات المطلقة للـ Struts لربطها بالـ Mesh لاحقاً 
    struts_mapped = []
    for s in raw_struts:
        p1 = s['p1']
        p2 = s['p2']
...
def get_approx_xy(segs, s_idx, s_val):
    """
    حساب التقريب الـ XY للفريمات المتصلة،
    ويُستخدم كـ Fallback لحساب طول النهايز بشكل تقريبي
    """
...
def perform_smart_division(base_segments, supports, struts):
    ...
    ولكن اللحام الأصلي الميكانيكي (Node Welding) يتم في الماتريكس
    """
    cut_points_dict = {}
...
def build_chain_mesh(segments, seg_sections, loads, struts, supports, base_segments, mesh_size=0.25):
    ...
    وهي الخطوة الأساسية لضمان قانون الاتزان ونقل الأحمال المركزية
    من النهايز إلى الكمرات السفلية وتلافي خطأ الـ Uplift
    """
    nodes = []
    elements = []
    nodal_loads = []
    
    # مسافة اللحام المغناطيسي 5 سم لضمان اتصال كل أجزاء الفريم ونقل الأحمال تماماً
    node_tol = 0.05 
    ...
    # 1. إجبار نقاط الدعامات (Supports) لتكسير الفريم
    for sup in supports:
        sx = sup['x']
        sy = sup['y']
    ...
    # 2. إجبار نقاط النهايز (Struts) لتكسير الفريمات العلوية والسفلية
    ...
    # 3. بناء الفريمات (Elements) وتقطيعها بناءً على النقط المحقونة
    for i, seg in enumerate(segments):
        ...
        for j in range(len(keys)-1):
            ...
            # توقيع الأحمال الموزعة على العناصر
            for ld in loads:
    ...
    # 4. بناء عناصر الـ Truss (النهايز) وتثبيتها بالإحداثيات المغناطيسية
...
def solve_fea_engine(nodes, elements, nodal_loads, supports_list):
    """
    محرك الـ FEA يعيد أيضاً إجمالي الحمل الرأسي المطبق للتأكد من الاتزان
    وتطبيق قانون نيوتن الأول بصرامة (مجموع الأحمال = مجموع الريأكشنات)
    """
    NDOF = len(nodes) * 3
    ...
    # حساب إجمالي الحمل الرأسي للتأكد من الاتزان (Equilibrium Check)
...
def draw_base_geometry(ax, nodes, elements, supports_list, seg_sections=None, segments=None, show_seg_names=False):
    """
    محرك الرسم الهندسي الدقيق للأشكال التفاعلية في واجهة Streamlit
    """
    for el in elements:
    ...
    for i, sup in enumerate(supports_list):
        ...
        if t == 'Fixed':
            p1 = rot_pt(x-0.1, y-0.1)
            p2 = rot_pt(x+0.1, y-0.1)
            p3 = rot_pt(x+0.1, y+0.1)
            p4 = rot_pt(x-0.1, y+0.1)
            ...
            l1 = rot_pt(x-w-0.05, y-h)
            l2 = rot_pt(x+w+0.05, y-h)
            ax.plot([l1[0], l2[0]], [l1[1], l2[1]], color='limegreen', lw=1.0, zorder=4)
            ...
            l1 = rot_pt(x-w-0.05, y-h-2*r)
            l2 = rot_pt(x+w+0.05, y-h-2*r)
            ax.plot([l1[0], l2[0]], [l1[1], l2[1]], color='limegreen', lw=1.0, zorder=4)
            ...
            if el['type'] == 'truss':
                ...
                if L_hyp > 1e-4:
                    ...
                    ax.text(
                        mid_x + nx_s*0.1, 
                        mid_y + ny_s*0.1, 
                        f"P{st_id}: {get_short_name(el.get('sec', ''))}", 
                        color='dimgray', 
                        fontsize=6, 
                        rotation=rot, 
                        ha='center', 
    ...
    # 💡 رسم خطوط الأبعاد الديناميكية (Dimensions) باللون الرمادي تحت الدعامات 
    ...
    for ld in loads:
        ...
        if segments:
            ...
            for sv in s_vals:
                ...
                w_curr = w1 + (w2 - w1) * (sv - ld.get('start', 0)) / L_load
    ...
    def create_force_plot(val_key, scale, c_pos, c_neg):
        ...
        for el in elements:
            ...
            def plot_val(idx):
                ...
                if is_far(tx, ty):
                    # 💡 تقريب القوى لرقمين عشريين كما طلب المستخدم
                    ax_f.text(
                        tx, 
                        ty, 
                        f"{v_disp:+.2f}", 
...
def generate_chain_report(sys_data):
    ...
    for cat in ['Dead Load', 'Live Load', 'Wind Load']:
        key = f'Load_{cat}'
        if key in bufs: 
            add_large_diagram(doc, bufs[key], f"{cat} Distribution Diagram")
...
# =====================================================================
# 🧠 THE HEURISTIC OPTIMIZER ENGINE (V17 - Strut Micro-Shifting)
# =====================================================================
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v, is_symmetric, opt_mode, status_text, progress_bar):
    """
    الإصدار السحري V17:
    يحتوي على فكرة (سماحية انزلاق النهايز - Strut Micro-shifting):
    - يختبر إزاحة قاعدة النهيز بمقدار 5، 10، 15 سم يمين ويسار.
    - يقوم بهذا الإجراء لامتصاص قوى القص والعزوم التي تسبب الريأكشنات العالية بدلاً من زيادة عدد الدعامات بشكل مفرط.
    """
    try:
    ...
    if min_y == 9999.0: 
        return False, None, None, "❌ Base geometry not found."
    ...
    for i, ld in enumerate(ui_loads):
        ...
        if t_mode == "Single Segment":
            ...
            for s in safe_def_segs:
                target_base_indices.append(base_seg_names.index(s))
        else:
            for idx in range(len(base_segments)):
                target_base_indices.append(idx)
        ...
        for s_idx_num in target_working_indices:
            ...
            test_combined_loads.append({
                ...
                'dir': ld['dir'], 
                'start': 0.0, 
                'end': max_s if ld['type'] != 'Point Load' else 0.0, 
    ...
    # 1. فلتر المستحيلات الرياضي: حساب إجمالي الحمل لتحديد أقل عدد دعامات ممكن رياضياً
    dummy_supps = [{'x': min_x, 'y': min_y, 'type': 'Hinged'}, {'x': max_x, 'y': min_y, 'type': 'Roller'}]
    _, _, total_system_load = run_trial(dummy_supps, ui_struts)
    ...
    # 2. بناء الاحتمالات المكانية للدعامات (Grids)
    valid_grids = []
    ...
    if not filtered_grids:
        return False, None, None, f"❌ Impossible! Requires at least {min_required_props} props to handle {total_system_load:.1f} kN safely, but your spacings can't fit them. Decrease spacings."
    ...
    # 3. مصفوفة الإزاحات (Strut Shift Tolerances) كما اقترح المستخدم تماماً
    shift_options = [0.0, 0.05, -0.05, 0.10, -0.10, 0.15, -0.15]
    ...
    sorted_p_counts = sorted(list(grids_by_props.keys()))
    total_estimated_trials = len(filtered_grids) * len(shift_options)
    
    # الحلقة الأساسية للبحث الهرمي مع الانزلاق السحري للنهايز
    for p_count in sorted_p_counts:
        for actual_coords in grids_by_props[p_count]:
            ...
            # تجربة إزاحات النهايز لتقليل الريأكشن عن طريق العزوم المضادة
            for shift_val in shift_options:
                ...
                for strut in ui_struts:
                    ...
                    bx_val = new_strut.get('bx', 0.0)
                    
                    if bx_val > center_x + 0.05:
                        new_strut['bx'] = bx_val + shift_val
                    elif bx_val < center_x - 0.05:
                        new_strut['bx'] = bx_val - shift_val
                    else:
                        new_strut['bx'] = bx_val + shift_val
                ...
                max_ry, min_ry, _ = run_trial(test_supps, shifted_struts)
                trials_count += 1
                
                # تحديث الواجهة التفاعلية بصرياً للمستخدم
                if trials_count % 15 == 0:
                    prog_pct = min(1.0, trials_count / float(total_estimated_trials))
                    progress_bar.progress(prog_pct)
                    status_text.markdown(f"**⏳ AI Search in Progress:** Testing Grid with **{p_count} Props** | Strut Shift: **{shift_val*100:+.0f} cm** | Lowest Rxn Found: **{best_fallback_score:.2f} kN**")
                ...
                if max_ry <= target_rxn and min_ry >= 0.5:
                    ...
                    return True, test_supps, shifted_struts, f"✅ BOOM! {sym_txt} Grid Found: Max Rxn = {max_ry:.2f} kN. Props = {p_count}. Struts Shifted by {shift_val*100:+.0f} cm. Tested {trials_count} combos."
    ...
    if best_fallback_grid:
        if "Deep" in opt_mode:
            return False, best_fallback_grid, best_fallback_struts, f"❌ STRICT LIMIT FAILED: Couldn't achieve <= {target_rxn} kN within time limit. Best safe grid reaction is {best_fallback_score:.2f} kN. Try smaller spacings."
        else:
            return True, best_fallback_grid, best_fallback_struts, f"⚠️ Quick Search Timeout. Showing BEST SAFE FALLBACK. Max Reaction is {best_fallback_score:.2f} kN. For exactly {target_rxn} kN, run Deep Search."
        
    return False, None, None, f"❌ Optimizer failed completely! All {trials_count} tested grids generated UPLIFT (Instability)."
# =========================================================
...
def render_advanced_shape_module():
    ...
    if uploaded_dxf and st.button("Extract Data from DXF"):
        ...
        if parsed:
            ...
            # التفريغ هنا لضمان عمل قارئ الـ OCR بفعالية
    ...
    with c_in:
        ...
        if STRUTS_DB:
            strut_opts_base = list(STRUTS_DB.keys())
        else:
        ...
        with st.expander("✨ Generative Design for Bridge Supports", expanded=False):
            st.info("The AI refuses to exceed the Target Load! It uses Strut Micro-Shifting and Iterative Deepening to balance moments and eliminate UPLIFT.")
            ...
            opt_mode = st.radio(
                "Optimization Depth:", 
                ["Quick Search (Max 3 Mins)", "Deep Search (Max 15 Mins)"], 
                index=0, 
                help="Quick gives the best safe fallback. Deep strictly enforces the target and uses Strut micro-shifting."
            )
            ...
            if st.button("✨ Run AI Optimizer & Generate Supports", type="primary", use_container_width=True):
                ...
                with st.spinner("🧠 AI is executing Physics-Aware Search..."):
                    succ, res_supps, res_struts, msg = run_auto_optimizer(
                        base_segments, working_segments, active_seg_sections, 
                        st.session_state.ui_struts, st.session_state.ui_loads, 
                        ai_target_rxn, ai_spc_str, view_plane, auto_mesh_size, dxf_v, is_sym, opt_mode, status_text, progress_bar
                    )
                    ...
                    if res_supps:
                        ...
                        for idx, sp in enumerate(res_supps):
                        ...
                        st.error(msg)
                        if res_supps: 
                            st.info("⚠️ A fallback grid was calculated but REJECTED because it violated Target Reaction in Deep Search mode.")
    ...
    with c_plot:
        ...
        # ==========================================
        # 5. LOADS & SMART AUTO ASSIGNER
        # ==========================================
        st.markdown("### 5. Applied Loads")
        
        # 💡 نافذة السلاح السري: OCR & Excel Parser
        with st.expander("📸 Smart Auto-Load Assigner (Excel & Snipping Tool)", expanded=False):
            st.info("💡 **Pro Tip:** Paste your table from Excel below for 100% accuracy, OR upload a snipping tool image to let the AI OCR read it automatically.")
            
            # S1: Excel Direct Paste Method
            excel_text = st.text_area("📋 Paste Directly from Excel (Segment Names & Loads):", placeholder="S1 \t 25.5\nS2 \t 30.0")
            if st.button("⚡ Assign Loads from Text"):
                    ...
                    if s_match and nums:
                        ...
                        if vals:
                            ...
                            st.session_state.ui_loads.append({
                                'category': 'Dead Load',
                                'type': 'Uniform',
                                'dir': 'Global Z (Vertical)' if "XZ" in view_plane else 'Global Y (Vertical)',
                                'target_mode': 'Single Segment',
                                'target_segs': [],
                                'w1': -abs(val), # 💡 الإجبار على أن يكون الحمل سالباً (لأسفل)
                                'w2': -abs(val),
                                '_auto_s_name': s_name
                            })
                            found_loads += 1
                ...
                if found_loads > 0:
                    st.success(f"✅ Awesome! Successfully mapped {found_loads} Loads automatically downwards (-Z).")
                    st.rerun()
                else:
                    st.warning("⚠️ Couldn't find valid format. Use format like 'S1 45.5'.")
                    
            st.markdown("---")
            
            # S2: OCR Image Reader Method
            img_file = st.file_uploader("🖼️ Or Upload/Paste Snipping Image here (Ctrl+V):", type=['png', 'jpg', 'jpeg'], key="ocr_img")
            if img_file and st.button("🔍 Read Image with OCR"):
                if pytesseract is None:
                    st.error("⚠️ Library 'pytesseract' is missing. Please install it using 'pip install pytesseract' or use the Excel Paste method above.")
                else:
                    with st.spinner("Analyzing image using Optical Character Recognition..."):
                        try:
                                ...
                                if s_match and nums:
                                    ...
                                    if vals:
                                        ...
                                        st.session_state.ui_loads.append({
                                            ...
                                            'target_mode': 'Single Segment',
                                            'target_segs': [],
                                            'w1': -abs(val), # 💡 الحمل سالب إجبارياً
                                            'w2': -abs(val),
                                            '_auto_s_name': s_name
                                        })
                                        found_loads += 1
                            ...
                            if found_loads > 0:
                                st.success(f"✅ OCR Success! Mapped {found_loads} Loads downwards (-Z).")
                                st.rerun()
                            else:
                                st.warning("⚠️ OCR couldn't read valid data from the image. Try the Excel paste method instead.")
                                
                        except Exception as e:
                            st.error(f"OCR Error: {e}")

        # 💡 الإدخال اليدوي للأحمال (Manual Load Assigner)
        ...
        for i, ld in enumerate(st.session_state.ui_loads):
            with st.expander(f"📥 Load Item {i+1}", expanded=(i==0)):
                ...
                ld['category'] = col_l1.selectbox("Category", cat_opts, index=idx_cat, key=f"ld_cat_{i}_{dxf_v}")
                
                type_opts = ["Uniform", "Trapezoidal", "Point Load"]
                idx_type = type_opts.index(ld.get('type', 'Uniform'))
                ld['type'] = col_l2.selectbox("Type", type_opts, index=idx_type, key=f"ld_t_{i}_{dxf_v}")
                ...
                if target_mode == "Single Segment":
                    ...
                    safe_auto_name = auto_name if auto_name in base_seg_names else base_seg_names[0]
                    idx_schoice = base_seg_names.index(st.session_state.get(f"ld_single_{i}", safe_auto_name))
                ...
                if ld['type'] == "Trapezoidal":
                    ld['w2'] = sc2.number_input("Value W2", value=float(ld.get('w2', 0.0)), format="%.3f", key=f"ld_w2_{i}_{dxf_v}")
                else:
                    ld['w2'] = ld['w1']
    ...
    if getattr(st.session_state, 'adv_solved', False):
        ...
        c_p2.image(img_bufs['N'], use_container_width=True)
        c_p2.markdown(f"<p align='center' style='font-weight:bold; border-bottom:1px solid #ccc;'>{titles['N']}</p>", unsafe_allow_html=True)
        ...
        c_p4.image(img_bufs['M'], use_container_width=True)
        c_p4.markdown(f"<p align='center' style='font-weight:bold; border-bottom:1px solid #ccc;'>{titles['M']}</p>", unsafe_allow_html=True)
        ...
        for i, sec in enumerate(fea_data['seg_sections']):
            ...
            if max_m <= sec['Mall'] and max_v <= sec['Qall']:
                s_status = "SAFE"
            else:
                s_status = "UNSAFE"
                
            seg_name_disp = fea_data['segments'][i].get('name', f'S{i+1}')
            short_sec = get_short_name(sec['name'])
            
            safety_data.append({
                "Component": f"{seg_name_disp} ({short_sec})", 
