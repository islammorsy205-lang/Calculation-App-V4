...
def run_auto_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, view_plane, auto_mesh_size, dxf_v):
    ...
    for i, ld in enumerate(ui_loads):
        ...
        if t_mode == "Single Segment":
            ...
            selected_segs = st.session_state.get(f"ld_multi_{i}", ld.get('target_segs', []))
            target_segments = [working_seg_names.index(s) for s in selected_segs if s in working_seg_names]
        else:
    ...
    for center_prop in [True, False]:
        current_segs = [spacings[0]] * int(math.ceil(max_x / spacings[-1]) + 2)
        
        for _ in range(25):
            coords = [0.0] if center_prop else []
            curr_x = spacings[0]/2.0 if not center_prop else 0.0
            for seg in current_segs:
                curr_x += seg
                if curr_x <= max_x + 0.15: coords.append(curr_x)
            
            full_grid = set(coords)
            for cx in coords:
                if cx > 0: full_grid.add(-cx)
            sorted_grid = sorted(list(full_grid))
            ...
            if max_ry <= target_rxn:
                if len(sorted_grid) < min_count:
                    min_count = len(sorted_grid)
                    best_grid = test_supps
                break
                
            demoted = False
            abs_worst_x = abs(worst_x)
            cx_track = spacings[0]/2.0 if not center_prop else 0.0
            
            for s_idx, seg in enumerate(current_segs):
                cx_track += seg
                if cx_track >= abs_worst_x - (seg * 1.5):
                    try:
        ...
        for i, ld in enumerate(st.session_state.ui_loads):
            with st.expander(f"📥 Load Item {i+1}", expanded=(i==0)):
                ...
                if target_mode == "Single Segment":
                    ...
                    def_segs = st.session_state.get(f"ld_multi_{i}", [s for s in ld.get('target_segs', []) if s in working_seg_names])
                    selected_segs = st.multiselect("Select Segments", working_seg_names, default=def_segs, key=f"ld_multi_{i}")
