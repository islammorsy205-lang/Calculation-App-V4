...
# Full calculation tables for Word Reports, and Isolated Load Diagrams.
# ==============================================================================
...
def crop_image_bbox(img_bytes):
    ...
    bbox = diff.getbbox()
    if bbox:
        # إضافة هامش بسيط جداً (Padding) لعدم قطع الخطوط
        padding = 5
        bbox = (
...
def safe_render_fig(fig):
    """دالة لحفظ وإخراج الصورة بأمان وبأعلى جودة"""
    try:
...
def get_closest_segment_exact(pt, segs):
    """إيجاد أقرب Segment لأي نقطة مرسومة (تُستخدم للالتحام المغناطيسي Snapping)"""
    min_d = 9999.0
    best_idx = 0
    best_s = 0.0
    px = pt[0]
...
def get_shifted_coords_along_segment(px, pz, ds, segs):
    """الزحزحة الموازية تُستخدم في الأوبتيميزر لتحريك النهايز على نفس خط السولجر"""
    if abs(ds) < 1e-4: 
        return px, pz
...
def get_strut_priority(name):
    """إعطاء الأولوية للـ Push-Pulls وفقاً لطلب الاستشاري"""
    name_u = name.upper()
...
def get_optimal_strut_section(req_length, req_axial_force):
    """البحث عن أفضل قطاع آمن للنهايز"""
    valid_struts = []
...
def parse_dxf_bridge_cases(file_bytes, loaded_width, conc_density):
    """
    مُحلل الـ DXF المتطور: يبحث عن المستطيلات في لاير TABLE_ANALYSIS
    ويُعامل كل مستطيل كمشروع/سكشن مستقل، وينقل إحداثياته لنقطة الصفر في المركز.
    """
    if ezdxf is None: 
        return None
    ...
    try:
        ...
        # 1. استخراج إطارات السكاشن (Bounding Boxes)
        for e in msp:
        ...
        # الحماية إذا لم يتم إيجاد لاير TABLE_ANALYSIS
        if not table_boxes:
            all_xs = []
            all_zs = []
            for e in msp:
                if e.dxftype() == 'LINE' and e.dxf.layer.upper() == layer_frame:
                    all_xs.extend([e.dxf.start.x / 1000.0, e.dxf.end.x / 1000.0])
                    all_zs.extend([e.dxf.start.y / 1000.0, e.dxf.end.y / 1000.0])
            
            if all_xs and all_zs:
                table_boxes.append({
                    'min_x': min(all_xs) - 1.0, 
                    'max_x': max(all_xs) + 1.0,
                    'min_z': min(all_zs) - 1.0, 
                    'max_z': max(all_zs) + 1.0,
        ...
        # 2. تفريغ محتويات الـ DXF داخل الصناديق المناسبة مع الإزاحة
        for e in msp:
            ...
            for s_txt in c['s_texts']:
                ...
                if min_d < 2.0: 
                    ...
                    if a_txt and s_txt['val'] > 1e-4:
                        area_val = a_txt['val']
                        length_val = s_txt['val']
                        w_val = (area_val * conc_density * loaded_width) / length_val
...
def solve_fea_engine(nodes, elements, nodal_loads, supports_list):
    ...
    يستخرج القوى الداخلية والانحراف بدقة عالية.
    """
    NDOF = len(nodes) * 3
...
# =====================================================================
# 🧠 THE BRIDGE HEURISTIC OPTIMIZER (Z-Snapping & Micro-Shifting)
# =====================================================================
def run_bridge_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, auto_mesh_size, is_symmetric, opt_mode, combo_factors, status_text, progress_bar):
    """
    محسن الكباري الذكي: يعتمد مسافات الإسناد وتعديلات النهايز ويطبق (Z-Snapping).
    """
    try: 
    ...
    def get_z_on_bottom_chord(test_x):
        """إسقاط ذكي رأسي لإيجاد قيمة Z على أسفل نقطة منشأ (لالتحام الركائز بالميول)"""
        for seg in base_segments:
            if seg.get('Shape Type') == 'Straight Line':
                p1 = seg['abs_p1']
                p2 = seg['abs_p2']
...
def plot_sap2000_diagrams(nodes, elements, R_reactions, scales, supports_list, loads, segments):
    """توليد وتغليف كافة الرسومات التحليلية المطلوبة مع عزل كل فئة."""
    apply_plot_styles()
    figs_dict = {}
    ...
    draw_base_geometry(ax_r, nodes, elements, supports_list, segments, show_dimensions=True)
...
def generate_multi_case_report(cases_data, proj_info):
    """توليد النوتة الحسابية المعتمدة بالشكل القياسي للاستشاري"""
    ...
    # ----------------------------------------------------
    # الهيكل القياسي للنوتة الحسابية
    # ----------------------------------------------------
    
    # 1. Cover Page
    add_line("BRIDGE FORMWORK CALCULATION NOTE", bold=True, size=24, align='center')
    add_line(f"Project: {proj_info.get('Project Name', 'Acrow Bridge Project')}", bold=True, size=16, align='center')
    add_line(f"Date: {proj_info.get('Date', '')}", size=14, align='center')
    doc.add_page_break()
    
    # 2. Index Page
    add_line("INDEX", bold=True, size=18)
    add_line("1. Data Sheets", size=14)
    add_line("2. Design Loads of Bridge deck Slab", size=14)
    add_line("3. Design Data of Formwork", size=14)
    doc.add_page_break()
    
    # 3. Data Sheets
    add_line("1. DATA SHEETS", bold=True, size=16)
    add_line("Refer to the attached technical specifications...", size=12)
    doc.add_page_break()
    
    # 4. Design Loads
    add_line("2. DESIGN LOADS OF BRIDGE DECK SLAB", bold=True, size=16)
    add_line("Calculations based on standard formwork design codes...", size=12)
    doc.add_page_break()
    
    # 5. Design Data of Formwork (The Core Tables)
    add_line("3. DESIGN DATA OF FORMWORK", bold=True, size=16)
    
    for case in cases_data:
        add_line(f"TABLE: {case['title'].upper()}", bold=True, size=14)
        doc.add_paragraph()
        
        # جدول الأحمال التفصيلي (Load Calculations Proof)
        if 'calc_details' in case and len(case['calc_details']) > 0:
            ...
            # الهيدر الأزرق (Blue Header)
            hdr_cells = table_ld.rows[0].cells
            headers = ["Segment", "Length (m)", "Area (m2)", "Load W (kN/m)"]
            for i, text in enumerate(headers):
                hdr_cells[i].text = text
                set_cell_bg_and_font(hdr_cells[i], '1F497D', RGBColor(255,255,255), bold=True)
            
            # الخلايا الرمادية (Grey Data Cells)
            for i, r_data in enumerate(case['calc_details']):
                ...
                for cell in row_cells:
                    set_cell_bg_and_font(cell, 'F2F2F2', RGBColor(0,0,0), bold=False)
                    
            doc.add_paragraph()
            
            # حساب وكتابة سطر الـ Live Load
            ll_w = 0.0
            loaded_w = proj_info.get('loaded_width', 1.30)
            ...
            if ll_w > 0:
                base_ll = ll_w / loaded_w if loaded_w > 0 else 0.0
                add_line("Live Load:", bold=True, size=12)
                add_line(f"- W1 = live load x Loaded Width for Load Calculation = {base_ll:.2f} x {loaded_w:.2f} = {ll_w:.3f} kN/m.", size=12)
            
            doc.add_paragraph()

        # ترتيب ومسميات الدياجرامات المخصصة باللون الأحمر (Underlined & Red)
        diagrams_map = [
        ...
        for key, label in diagrams_map:
            if key in case.get('img_bufs', {}):
                # وضع العنوان باللون الأحمر ووضع خط تحته
                p_title = add_line(label, bold=True, size=12, color=RGBColor(192, 0, 0))
...
def render_bridge_module(proj_info):
    ...
    if "DXF" in mode:
        ...
        conc_density = c2.number_input("Concrete Density (kN/m3)", value=25.0, step=0.5)
        
        # 💡 تمرير عرض التحميل لبروفايل المشروع لطباعته في الوورد لاحقاً
        proj_info['loaded_width'] = loaded_width
        ...
        if 'bridge_cases' in st.session_state:
            ...
            # 💡 توليد التابات (Tabs) الديناميكية المعزولة لكل Table
            ...
            for c_idx, tab in enumerate(tabs):
                ...
                with tab:
                    ...
                    with c_edit:
                        ...
                        # 💡 استخدام c_idx في الـ key لعزل الحالة تماماً عن باقي التابات
                        ...
                        with st.expander(f"⬇️ Applied Loads ({len(case['loads'])})", expanded=True):
                                ...
                                for line in excel_text.split('\n'):
                                    ...
                                    if s_m and nums:
                                        ...
                                        if vals:
                                            ...
                                            if s_name_extracted in seg_names:
                                                s_idx_assigned = seg_names.index(s_name_extracted)
                                            else:
                                                s_idx_assigned = 0
                            ...
                            for i, ld in enumerate(case['loads']):
                                # 💡 طي الأحمال كـ Expander مستقل لتنظيم الشاشة
                    ...
                    with c_view:
                        ...
                        if st.button(f"🚀 Run FEA Analysis for {case['title']}", type="primary", use_container_width=True, key=f"btn_run_{c_idx}"):
                            with st.spinner(f"Solving FEA Matrix..."):
                                ...
                                # تخزين في الحاوية المعزولة لكي يعمل الإسكيل في الأسفل
                        ...
                        # عرض النتائج والإسكيلات المعزولة
                        if f'fea_cache_{c_idx}' in st.session_state:
                            ...
                            # عرض رسومات الأحمال المعزولة إن وجدت
                            if 'DL' in img_bufs:
                                st.image(img_bufs['DL'], caption="Dead Load Distribution")
                            if 'LL' in img_bufs:
                                st.image(img_bufs['LL'], caption="Live Load Distribution")
                            if 'WL' in img_bufs:
                                st.image(img_bufs['WL'], caption="Wind Load Distribution")
        ...
        with c_in:
            ...
            for i, sup in enumerate(st.session_state.man_sups):
                ...
                sup['type'] = c3.selectbox("Type", t_opts, index=idx_t, key=f"st_{i}")
            ...
            for i, ld in enumerate(st.session_state.man_lds):
                with st.expander(f"📥 Load {i+1} ({ld.get('category', 'Dead Load')})", expanded=False):
                    ...
                    if ld['target_mode'] == "Single Segment":
                        ...
                        st.info("Load applied to ALL segments.")
        ...
        with c_plot:
            ...
            if 'man_fea_cache' in st.session_state:
                ...
                if 'DL' in img_bufs: st.image(img_bufs['DL'], caption="Dead Load Distribution")
                if 'LL' in img_bufs: st.image(img_bufs['LL'], caption="Live Load Distribution")
                if 'WL' in img_bufs: st.image(img_bufs['WL'], caption="Wind Load Distribution")
