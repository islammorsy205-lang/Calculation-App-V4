...
def run_bridge_optimizer(base_segments, working_segments, active_seg_sections, ui_struts, ui_loads, target_rxn, spacings_str, auto_mesh_size, is_symmetric, opt_mode, combo_factors, status_text, progress_bar):
    try: 
        ...
        spacings = sorted([float(x.strip()) for x in spacings_raw], reverse=True)
    except Exception as e: 
    ...
    soldier_min_x, soldier_max_x = (min(bottom_xs), max(bottom_xs)) if bottom_xs else (min_x, max_x)
    ...
    # 💡 معالجة الأحمال للمحسن (بما فيها الأنواع والـ Mode)
    test_combined_loads = []
    for i, ld in enumerate(ui_loads):
        ...
        else: target_base_indices = list(range(len(base_segments)))
            
        w1 = ld.get('w1', 0.0)
        w2 = ld.get('w2', w1) if ld.get('type') == 'Trapezoidal' else w1
        ...
        for s_idx_num in target_working_indices:
            ...
            if ld.get('type') == 'Point Load': start_val = end_val = min(loc_m, w_len)
            else: start_val, end_val = 0.0, w_len
                
            test_combined_loads.append({
                'seg_idx': s_idx_num, 'category': ld.get('category', 'Dead Load'), 'type': ld.get('type', 'Uniform'), 'dir': ld.get('dir', 'Global Z (Vertical)'), 
                'start': start_val, 'end': end_val, 
                'w1': w1 * combo_factors.get(ld.get('category', 'Dead Load'), 1.0), 'w2': w2 * combo_factors.get(ld.get('category', 'Dead Load'), 1.0)
            })
    ...
    def run_trial(test_supps, dynamic_struts):
        # 💡 تم إصلاح الاستدعاء هنا للتعامل مع 4 مخرجات فقط (Error Fix)
    ...
    dummy_supps = [{'x': soldier_min_x, 'z': min_z, 'type': 'Hinged', 'angle': 0.0}, {'x': soldier_max_x, 'z': min_z, 'type': 'Hinged', 'angle': 0.0}]
    ...
    grids_by_props = {}
    for g in filtered_grids: grids_by_props.setdefault(len(g), []).append(g)
    ...
    for ld in loads:
        ...
        if len(poly_pts) > 2:
            ...
            for k in range(0, len(top_pts), step):
                dx, dz = poly_pts[k][0] - top_pts[k][0], poly_pts[k][1] - top_pts[k][1]
                ax_ld.arrow(top_pts[k][0], top_pts[k][1], dx, dz, length_includes_head=True, head_width=0.06, head_length=0.1, fc=ld_color, ec=ld_color, lw=0.6, zorder=4)
...
def generate_multi_case_report(cases_data, proj_info):
    ...
    for case in cases_data:
        ...
        if 'safety_df' in case:
            ...
            # 💡 معالجة خطأ الـ Table Grid بأمان تام
            try:
                table.style = 'Table Grid'
            except KeyError:
                pass # تجاهل الخطأ واستخدام ستايل الوورد الافتراضي
    ...
    if "DXF" in mode:
            ...
            for c_idx, tab in enumerate(tabs):
                ...
                with tab:
                    ...
                    with c_edit:
                        ...
                        # 4. Edit Loads (Fully Upgraded UI with Mode & Type)
                        with st.expander(f"⬇️ Applied Loads ({len(case['loads'])})", expanded=True):
                                ...
                                for line in excel_text.split('\n'):
                                    ...
                                    if s_match and nums:
                                        ...
                                        if vals:
                                            ...
                                            case['loads'].append({'seg_idx': seg_names.index(s_match.group(1)) if s_match.group(1) in seg_names else 0, 'category': 'Live Load', 'type': 'Uniform', 'dir': 'Global Z (Vertical)', 'target_mode': 'Single Segment', 'target_segs_idx': [], 'start': 0.0, 'end': case['segments'][0]['L'], 'w1': w_v, 'w2': w_v, 'loc': 0.0})
                                st.rerun()
                            ...
                            for i, ld in enumerate(case['loads']):
                                ...
                                ld['category'] = c_l1.selectbox("Category", cat_opts, index=cat_opts.index(ld.get('category', 'Dead Load')) if ld.get('category') in cat_opts else 0, key=f"lct_{c_idx}_{i}")
                                ld['type'] = c_l2.selectbox("Type", type_opts, index=type_opts.index(ld.get('type', 'Uniform')) if ld.get('type') in type_opts else 0, key=f"ltp_{c_idx}_{i}")
                                
                                c_dir = ld.get('dir', 'Global Z (Vertical)')
                                if 'Y' in c_dir: c_dir = 'Global Z (Vertical)'
                                ld['dir'] = c_l3.selectbox("Direction", dir_opts, index=dir_opts.index(c_dir) if c_dir in dir_opts else 1, key=f"ldr_{c_idx}_{i}")
                                ...
                                # 💡 Target Mode Selection (Single, Multi, All)
                                ld['target_mode'] = st.radio("Apply Load To:", t_mode_opts, index=t_mode_opts.index(ld.get('target_mode', 'Single Segment')) if ld.get('target_mode') in t_mode_opts else 0, key=f"lmode_{c_idx}_{i}", horizontal=True)
                            ...
                            if st.button("➕ Manual Load", key=f"add_ld_{c_idx}"):
                                case['loads'].append({'seg_idx': 0, 'category': 'Live Load', 'type': 'Uniform', 'dir': 'Global Z (Vertical)', 'target_mode': 'Single Segment', 'target_segs_idx': [], 'start': 0.0, 'end': case['segments'][0]['L'], 'w1': -10.0, 'w2': -10.0, 'loc': 0.0})
                                st.rerun()
                    ...
                    with c_view:
                        ...
                        # 💡 معالجة فك الأحمال المجمعة إلى أحمال مفردة وتمريرها لمحرك الـ FEA
                        expanded_loads = []
                        for ld in case['loads']:
                            fac = combo_factors.get(ld.get('category', 'Dead Load'), 1.0)
                            t_mode = ld.get('target_mode', 'Single Segment')
                            if t_mode == 'Single Segment': t_idx_list = [ld.get('seg_idx', 0)]
                            elif t_mode == 'Multiple Segments': t_idx_list = ld.get('target_segs_idx', [])
                            else: t_idx_list = list(range(len(case['segments'])))
                            ...
                            for s_idx in t_idx_list:
                                ...
                                f_ld['w1'] *= fac
                                f_ld['w2'] *= fac
                                if f_ld['type'] == 'Point Load': f_ld['start'] = f_ld['end'] = min(f_ld.get('loc', 0.0), case['segments'][s_idx].get('L', 0.0))
                                else: f_ld['start'], f_ld['end'] = 0.0, case['segments'][s_idx].get('L', 0.0)
                                expanded_loads.append(f_ld)
                            
                        # 💡 الاستدعاء الآمن (4 مخرجات فقط)
                        ...
                        if st.button(f"🚀 Run FEA Analysis for {case['title']}", type="primary", use_container_width=True, key=f"btn_run_{c_idx}"):
                            with st.spinner(f"Solving FEA..."):
                                # 💡 الاستدعاء الآمن (3 مخرجات فقط)
                                U, R, net_load = solve_fea_engine(p_nodes, p_elems, p_nloads, p_supps)
                                img_bufs = plot_sap2000_diagrams(p_nodes, p_elems, R, {'N': sc_n, 'V': sc_v, 'M': sc_m}, p_supps, expanded_loads, case['segments'])
        ...
        with c_in:
            ...
            for i, ld in enumerate(st.session_state.man_lds):
                c_l1, c_l2, c_l3, c_l4 = st.columns([1.5, 1.5, 1.5, 0.5])
                ld['category'] = c_l1.selectbox("Category", cat_opts, index=cat_opts.index(ld.get('category', 'Dead Load')) if ld.get('category') in cat_opts else 0, key=f"mlct_{i}")
                ld['type'] = c_l2.selectbox("Type", type_opts, index=type_opts.index(ld.get('type', 'Uniform')) if ld.get('type') in type_opts else 0, key=f"mltp_{i}")
                
                c_dir = ld.get('dir', 'Global Z (Vertical)')
                if 'Y' in c_dir: c_dir = 'Global Z (Vertical)'
                ld['dir'] = c_l3.selectbox("Direction", dir_opts, index=dir_opts.index(c_dir) if c_dir in dir_opts else 1, key=f"mldr_{i}")
                ...
                ld['target_mode'] = st.radio("Apply Load To:", t_mode_opts, index=t_mode_opts.index(ld.get('target_mode', 'Single Segment')) if ld.get('target_mode') in t_mode_opts else 0, key=f"mlmode_{i}", horizontal=True)
            ...
            if st.button("➕ Add Manual Load"): st.session_state.man_lds.append({'seg_idx': 0, 'w1': -10.0, 'w2': -10.0, 'dir': 'Global Z (Vertical)', 'category': 'Live Load', 'type': 'Uniform', 'target_mode': 'Single Segment', 'target_segs_idx': [], 'loc': 0.0}); st.rerun()
        ...
        with c_plot:
            ...
            for ld in st.session_state.man_lds:
                ...
                else: t_idx_list = list(range(len(st.session_state.man_segs)))
                ...
                for s_idx in t_idx_list:
                    ...
                    f_ld['w1'] *= fac
                    f_ld['w2'] *= fac
                    if f_ld['type'] == 'Point Load': f_ld['start'] = f_ld['end'] = min(f_ld.get('loc', 0.0), st.session_state.man_segs[s_idx].get('L', 0.0))
                    else: f_ld['start'], f_ld['end'] = 0.0, st.session_state.man_segs[s_idx].get('L', 0.0)
            ...
            if st.button("🚀 Run FEA", type="primary", use_container_width=True):
                with st.spinner("Solving FEA..."):
                    U, R, net_load = solve_fea_engine(p_nodes, p_elems, p_nloads, p_supps)
                    st.session_state.man_img_bufs = plot_sap2000_diagrams(p_nodes, p_elems, R, {'N':0.01,'V':0.01,'M':0.01}, p_supps, m_expanded_loads, st.session_state.man_segs)
                    ...
                    for i_seg, sec in enumerate(active_sections):
                        ...
                        safety_data.append({"Segment": st.session_state.man_segs[i_seg]['name'], "M_max": f"{max_m:.2f} / {sec['Mall']:.2f}", "V_max": f"{max_v:.2f} / {sec['Qall']:.2f}", "Status": s_status})
                    
                    st.session_state.man_safety_df = safety_data
                    st.session_state.man_case_data = [{'title': 'Manual Case', 'img_bufs': st.session_state.man_img_bufs, 'safety_df': safety_data}]
                    
            if 'man_img_bufs' in st.session_state:
                c_p1, c_p2 = st.columns(2)
                c_p1.image(st.session_state.man_img_bufs['M'], caption="Moment"); c_p2.image(st.session_state.man_img_bufs['V'], caption="Shear")
                c_p3, c_p4 = st.columns(2)
                c_p3.image(st.session_state.man_img_bufs['N'], caption="Axial"); c_p4.image(st.session_state.man_img_bufs['R'], caption="Reactions")
