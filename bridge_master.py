...
# استدعاء ملفات التكوين والتقارير (تأكد من وجودها في نفس المسار)
...
def crop_image_bbox(img_bytes):
    """
    قص حواف الصور الناتجة لجعلها متناسقة وموفرة للمساحة في تقرير الوورد
    """
...
def safe_render_fig(fig):
    """
    دالة لحفظ وإخراج الصورة بأمان وبأعلى جودة (400 DPI)
    """
    try:
...
def get_shifted_coords_along_segment(px, pz, ds, segs):
    """
    الزحزحة الموازية (Parallel Micro-shifting) تُستخدم في الأوبتيميزر لتحريك النهايز
    على نفس خط السولجر دون تشويه الرسمة.
    """
    if abs(ds) < 1e-4: 
        return px, pz
    ...
    # إذا كانت النقطة بعيدة جداً، لا تقم بالزحزحة
    if d_min > 0.5: 
        return px + ds, pz
...
def get_strut_priority(name):
    ...
    PPS أولاً، ثم PPH. مع الاستبعاد القاطع للنهايز الفردية وأقطاب TILT/MMP.
    """
    name_u = name.upper()
...
def parse_dxf_bridge_cases(file_bytes, loaded_width, conc_density):
    ...
    try:
        ...
        # إذا لم يجد المستخدم لاير TABLE_ANALYSIS (حماية ضد الأعطال)
        if not table_boxes:
            all_xs = []
            all_zs = []
            for e in msp:
        ...
        # ترتيب السكاشن من اليسار لليمين
        ...
        # 2. تفريغ محتويات الـ DXF داخل الصناديق المناسبة مع نقل الصفر للمركز (Normalization)
        for e in msp:
        ...
        for c_idx, c in enumerate(cases_raw):
            ...
            loads = []
            calc_details = [] # 💡 تسجيل تفاصيل الحساب للوورد
            ...
            # التسمية الذكية (S30, S31, S40, S41)
            unlabeled = []
            ...
            processed_cases.append({
                ...
                'loads': loads,
                'calc_details': calc_details # 💡 مخرجات حساب الأحمال للوورد
            })
...
def draw_base_geometry(ax, nodes, elements, supports_list, segments, show_names=False, show_dimensions=False):
    ...
    # إظهار أسماء السكاشن (بدون لواحق التقطيع)
    ...
    # 💡 إضافة الأبعاد والمسافات الأفقية بين الركائز
...
def get_live_preview_image(nodes, elements, supports_list, loads, segments, cat_filter=None):
    """
    رسم الهندسة بالإضافة إلى مضلعات الأحمال.
    💡 تم إضافة `cat_filter` لعزل أنواع الأحمال (DL, LL, WL) في رسومات مستقلة.
    """
    apply_plot_styles()
...
def plot_sap2000_diagrams(nodes, elements, R_reactions, scales, supports_list, loads, segments):
    """
    توليد وتغليف كافة الرسومات التحليلية المطلوبة (عزل الأحمال والمخرجات الداخيلة).
    """
    apply_plot_styles()
    figs_dict = {}
    
    # 💡 عزل رسومات الأحمال (DL, LL, WL) لتظهر مستقلة
...
def generate_multi_case_report(cases_data, proj_info):
    """
    توليد النوتة الحسابية الشاملة مع إضافة جداول أحمال صريحة لكل حالة.
    """
    ...
    for case in cases_data:
        ...
        # 💡 1. جدول الحسابات الخاص بالأحمال (Load Calculations Proof)
        if 'calc_details' in case and len(case['calc_details']) > 0:
            ...
            # طباعة المعادلة التوضيحية للمراجع
            eq_text = "Equation Used: Load W (kN/m) = [Area (m2) × Density (kN/m3) × Loaded Width (m)] / Length (m)"
            p_eq = add_line(eq_text, size=10)
            p_eq.runs[0].font.italic = True
            ...
            hdr = table_ld.rows[0].cells
            hdr[0].text = "Segment"
            hdr[1].text = "Length (m)"
            hdr[2].text = "Area (m2)"
            hdr[3].text = "Load W (kN/m)"
            
            for i, r_data in enumerate(case['calc_details']):
                cells = table_ld.rows[i+1].cells
                cells[0].text = str(r_data['segment'])
                cells[1].text = f"{r_data['length']:.2f}"
                cells[2].text = f"{r_data['area']:.2f}"
                cells[3].text = f"{r_data['load_w']:.2f}"
            doc.add_paragraph()
        
        # 💡 2. جدول الأمان (Safety Checks)
        if 'safety_df' in case:
            add_line("SAFETY CHECKS SUMMARY", bold=True, size=12)
            table_sf = doc.add_table(rows=len(case['safety_df'])+1, cols=4)
            try: 
                table_sf.style = 'Table Grid'
            except KeyError: 
                pass 
            
            hdr_sf = table_sf.rows[0].cells
            hdr_sf[0].text = "Segment"
            hdr_sf[1].text = "Max Moment"
            hdr_sf[2].text = "Max Shear"
            hdr_sf[3].text = "Status"
            
            for i, row_data in enumerate(case['safety_df']):
                row_cells = table_sf.rows[i+1].cells
        ...
        # 💡 3. الرسومات المعزولة بالتتابع
        diagrams = [
            ('DL', "DEAD LOAD DISTRIBUTION"),
            ('LL', "LIVE LOAD DISTRIBUTION"),
            ('WL', "WIND LOAD DISTRIBUTION"),
...
def render_bridge_module(proj_info):
    ...
    وبناء التابات المستقلة، والأوبتيميزر المتوازي، ووضع الإدخال اليدوي.
    """
    ...
    if "DXF" in mode:
        ...
        if 'bridge_cases' in st.session_state:
            ...
            # 💡 توليد التابات (Tabs) الديناميكية المستقلة تماماً
        ...
        with c_plot:
            ...
            if 'man_fea_cache' in st.session_state:
                ...
                if st.button("📥 Download Word Report", type="primary", use_container_width=True):
                    man_case = [{'title': 'Manual Case', 'img_bufs': img_bufs, 'safety_df': st.session_state.man_safety_df}]
                    doc_out = generate_multi_case_report(man_case, proj_info)
                    st.download_button(
                        "💾 Save Manual Report", 
                        data=doc_out.getvalue(), 
