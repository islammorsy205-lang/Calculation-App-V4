    ...
    if "DXF" in mode:
            ...
            for c_idx, tab in enumerate(tabs):
                ...
                with tab:
                    ...
                    with c_view:
                        ...
                        # تم إصلاح الاستدعاء هنا للتعامل مع 4 مخرجات فقط
                        p_nodes, p_elems, _, p_supps = build_chain_mesh(case['segments'], case['sec_overrides'], factored_loads, case['struts'], case['supports'], case.get('cut_points', []))
                        ...
                        if st.button(f"🚀 Run FEA Analysis for {case['title']}", type="primary", use_container_width=True, key=f"btn_run_{c_idx}"):
                            with st.spinner(f"Solving FEA..."):
                                # تم إصلاح الاستدعاء هنا أيضاً للتعامل مع 4 مخرجات
        ...
        with c_in:
            ...
            for i, ld in enumerate(st.session_state.man_lds):
                ...
                ld['dir'] = c2.selectbox("Dir", ["Global X (Horizontal)", "Global Z (Vertical)"], index=1, key=f"ld_{i}")
        ...
        with c_plot:
            ...
            # تم إصلاح الاستدعاء هنا للتعامل مع 4 مخرجات
            p_nodes, p_elems, _, p_supps = build_chain_mesh(st.session_state.man_segs, active_sections, st.session_state.man_lds, st.session_state.man_strs, st.session_state.man_sups, [])
            ...
            if st.button("🚀 Run FEA", type="primary", use_container_width=True):
                with st.spinner("Solving FEA..."):
                    # تم إصلاح الاستدعاء هنا أيضاً
                    nodes, elements, nodal_loads, supports_list = build_chain_mesh(st.session_state.man_segs, active_sections, st.session_state.man_lds, st.session_state.man_strs, st.session_state.man_sups, [])
                    U, R = solve_fea_engine(nodes, elements, nodal_loads, supports_list)[:2]
                    st.session_state.man_img_bufs = plot_sap2000_diagrams(nodes, elements, R, {'N':0.01,'V':0.01,'M':0.01}, supports_list, st.session_state.man_lds, st.session_state.man_segs)
