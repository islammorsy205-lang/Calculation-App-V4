...
def generate_multi_case_report(cases_data, proj_info):
    ...
    تستخدم نفس أسلوب تقارير الأسقف (Slab Elements) مع دمج حسابات كباري الـ DXF.
    """
    import fitz  # تأكد من أن PyMuPDF مستدعاة لعد صفحات الـ PDF
    ...
    loaded_width = proj_info.get("loaded_width", 1.30)
    ...
    for case in cases_data:
        ...
        if 'calc_details' in case and len(case['calc_details']) > 0:
            ...
            if ll_w > 0:
                base_ll = ll_w / loaded_width if loaded_width > 0 else 0.0
                add_line("Live Load:", bold=True, size=12)
                add_line(f"- W1 = live load x Loaded Width for Load Calculation = {base_ll:.2f} x {loaded_width:.2f} = {ll_w:.3f} kN/m.", size=12)
            
            doc.add_paragraph()

        # 💡 طباعة الدياجرامات بتنسيق أحمر تحته خط
        diagrams_map = [
