...
# ==============================================================================
# 🌟 Features: True 2D XZ Plane, Strict DXF Parsing, Smart S-Naming, 
# Auto Gravity Loads drawn CORRECTLY ABOVE segments pointing down (-Z).
# ==============================================================================
...
def plot_sap2000_diagrams(nodes, elements, R_reactions, scales, supports_list, loads, segments):
    ...
    def create_force_plot(val_key, scale, c_pos, c_neg):
        ...
        for el in elements:
            ...
            px, py = x1 + c*xs - s*plot_vals*scale, z1 + s*xs + c*plot_vals*scale
            
            for k in range(len(px)-1):
                ax_f.plot([px[k], px[k+1]], [py[k], py[k+1]], color=c_pos if vals[k] >= 0 else c_neg, lw=0.8)
            ax_f.plot([x1, px[0]], [z1, py[0]], color=c_pos if vals[0]>=0 else c_neg, lw=0.8)
            ax_f.plot([x2, px[-1]], [z2, py[-1]], color=c_pos if vals[-1]>=0 else c_neg, lw=0.8)
