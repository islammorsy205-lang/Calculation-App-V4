...
def plot_sap2000_diagrams(nodes, elements, R_reactions, scales, supports_list, loads, segments):
    ...
    for ld in loads:
        ...
        for sv in s_vals:
            ...
            # 💡 الأحمال توقع عمودياً للأسفل (Global Vertical) مهما كان ميل الفريم
            dir_str = ld.get('dir', 'Global Y (Vertical)').upper()
...
def generate_multi_case_report(cases_data, proj_info):
    ...
    add_line("BRIDGE FORMWORK MULTI-CASE ANALYSIS (INTERACTIVE)", bold=True, size=16)
...
def render_bridge_module(proj_info):
            ...
            for c_idx, tab in enumerate(tabs):
                ...
                with tab:
                    ...
                    with c_edit:
                        ...
                        # 💡 2. تعديل وتخصيص القطاعات (Sections Override)
                        ...
                        # 💡 4. تعديل الأحمال وتوجيهها (Applied Loads)
                    ...
                    with c_view:
                        # 💡 زر التحليل الإنشائي المستقل لكل حالة
    ...
    # =========================================================
        ...
        with c_plot:
            st.markdown("### 📊 Interactive Preview & Analysis")
            global_sec = {'name': "Soldier U100", 'E': 2100.0, 'A': 34.3 / 10000.0, 'I': 412.0 / 100000000.0, 'Mall': 13.1, 'Qall': 100.8}
            ...
            if st.button("🚀 Run Advanced FEA", type="primary", use_container_width=True):
                with st.spinner("Solving FEA..."):
                    ...
                    st.session_state.man_case_data = [{
                        'title': 'Manual Case',
                        'img_bufs': img_bufs
                    }]
                    
            if 'man_img_bufs' in st.session_state:
                st.image(st.session_state.man_img_bufs['L'], caption="Loads & Geometry")
