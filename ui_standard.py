...
def render_slab_element(i, gamma_c, live_load, fw_load, def_sec, def_main):
    # 💡 Helper function to inherit defaults from Element 1
    def get_def(key_base, default_val):
        return st.session_state.get(f"{key_base}_1", default_val) if i > 1 else default_val

    etype_opts = ["Slab", "Drop Panel", "Beam"]
    
    def_etype = get_def("etype", "Slab")
    idx_etype = etype_opts.index(st.session_state.get(f"etype_{i}", def_etype)) if st.session_state.get(f"etype_{i}") in etype_opts else etype_opts.index(def_etype)
    ...
    if element_type == "Beam":
        ...
        idx_bpos = bpos_opts.index(st.session_state.get(f"bpos_{i}", def_bpos)) if st.session_state.get(f"bpos_{i}") in bpos_opts else bpos_opts.index(def_bpos)
    ...
    with st.container(border=True):
        ...
        with col_s1:
            ...
            idx_ssec = list(SECTIONS_DB.keys()).index(st.session_state.get(f"ss_{i}", def_ssec)) if st.session_state.get(f"ss_{i}") in SECTIONS_DB else list(SECTIONS_DB.keys()).index(def_ssec)
            ...
            with c_sl1:
                if cust_sL:
                    ...
                    idx_sl1 = s_l1_opts.index(st.session_state.get(f"sl1_{i}", def_sl1)) if st.session_state.get(f"sl1_{i}") in s_l1_opts else (len(s_l1_opts)-1 if s_sec in STD_LENGTHS else 0)
            ...
            auto_m_spc = 1.10
            if element_type == "Beam":
                # 💡 التعديل الخاص بمسافة كمرة السقف (Middle = b_width, Edge = b_width / 2.0)
                auto_m_spc = b_width if beam_pos == "Middle" else b_width / 2.0
            else:
        ...
        with col_s2:
            ...
            if not isinstance(saved_sdf, list): 
                # 💡 وراثة جدول الأحمال من الـ Element 1 لتوفير الوقت
                if i > 1 and isinstance(st.session_state.get("sdf_1"), list):
                    saved_sdf = st.session_state.get("sdf_1")
                else:
    ...
    with st.container(border=True):
        ...
        with col_m1:
            ...
            idx_msec = list(SECTIONS_DB.keys()).index(st.session_state.get(f"ms_{i}", def_msec)) if st.session_state.get(f"ms_{i}") in SECTIONS_DB else list(SECTIONS_DB.keys()).index(def_msec)
            ...
            with c_ml1:
                if cust_mL:
                    ...
                    idx_ml1 = m_l1_opts.index(st.session_state.get(f"wml1_{i}", def_wml1)) if st.session_state.get(f"wml1_{i}") in m_l1_opts else (len(m_l1_opts)-1 if m_sec in STD_LENGTHS else 0)
            ...
            idx_tn = SHORING_OPTIONS_SLAB.index(st.session_state.get(f"tn_{i}", def_tn)) if st.session_state.get(f"tn_{i}") in SHORING_OPTIONS_SLAB else SHORING_OPTIONS_SLAB.index(def_tn)
            ...
            if t_nm == "Shorebrace Frame":
                ...
                idx_cup = sub_opts.index(st.session_state.get(f"cup_sub_{i}", def_cup_sub)) if st.session_state.get(f"cup_sub_{i}") in sub_opts else sub_opts.index(def_cup_sub)
                ...
                idx_ring = r_opts.index(st.session_state.get(f"ring_sub_{i}", def_ring_sub)) if st.session_state.get(f"ring_sub_{i}") in r_opts else r_opts.index(def_ring_sub)
                ...
                if not valid_props:
                    ...
                    idx_psel = valid_props.index(st.session_state.get(f"prop_sel_{i}", def_prop_sel)) if st.session_state.get(f"prop_sel_{i}") in valid_props else (valid_props.index(def_prop_sel) if def_prop_sel in valid_props else 0)
        ...
        with col_m2:
            ...
            idx_meth = meth_opts.index(st.session_state.get(f"m_l_meth_{i}", def_meth)) if st.session_state.get(f"m_l_meth_{i}") in meth_opts else meth_opts.index(def_meth)
            ...
            if not isinstance(saved_mdf, list): 
                if i > 1 and isinstance(st.session_state.get("mdf_1"), list):
                    saved_mdf = st.session_state.get("mdf_1")
                else:
...
def render_vertical_element(i, element_subtype, def_sec, def_main):
    ...
    if element_subtype == "Wall":
        ...
        idx_wt = wt_opts.index(st.session_state.get(f"wt_{i}", def_wt)) if st.session_state.get(f"wt_{i}") in wt_opts else wt_opts.index(def_wt)
    ...
    idx_vsys = vsys_opts.index(st.session_state.get(f"vsys_{i}", def_vsys)) if st.session_state.get(f"vsys_{i}") in vsys_opts else (vsys_opts.index(def_vsys) if def_vsys in vsys_opts else 0)
    ...
    if is_panel_sys:
        if "Eco-form" in vert_system: 
            ...
            idx_pw = panel_w_opts.index(st.session_state.get(f"panel_w_{i}", def_pw)) if st.session_state.get(f"panel_w_{i}") in panel_w_opts else panel_w_opts.index(def_pw)
    ...
    if not is_panel_sys:
        ...
        with st.container(border=True):
            ...
            with col_s1:
                ...
                idx_wss = s_sec_opts.index(st.session_state.get(f"wss_{i}", def_wss)) if st.session_state.get(f"wss_{i}") in s_sec_opts else s_sec_opts.index(def_wss)
                ...
                with c_wsl1:
                    if cust_wsl:
                        ...
                        idx_wsl1 = wsl1_opts.index(st.session_state.get(f"wsl1_{i}", def_wsl1)) if st.session_state.get(f"wsl1_{i}") in wsl1_opts else (len(wsl1_opts)-1 if s_sec in STD_LENGTHS else 0)
            ...
            with col_s2:
                ...
                if not isinstance(saved_wsdf, list): 
                    if i > 1 and isinstance(st.session_state.get("wsdf_1"), list):
                        saved_wsdf = st.session_state.get("wsdf_1")
                    else:
        ...
        with st.container(border=True):
            ...
            with col_m1:
                ...
                idx_wms = list(SECTIONS_DB.keys()).index(st.session_state.get(f"wms_{i}", def_wms)) if st.session_state.get(f"wms_{i}") in SECTIONS_DB else list(SECTIONS_DB.keys()).index(def_wms)
                ...
                with c_wml1:
                    if cust_wml:
                        ...
                        idx_wml1_w = wml1_opts.index(st.session_state.get(f"wml1_w_{i}", def_wml1_w)) if st.session_state.get(f"wml1_w_{i}") in wml1_opts else (len(wml1_opts)-1 if m_sec in STD_LENGTHS else 0)
            ...
            with col_m2:
                ...
                meth_opts = ["Surface Pressure (W_tot × Main Spacing)", "Secondary Reaction (Max Reaction / Sec Spacing)"]
                def_meth = get_def("m_l_meth", meth_opts[0])
                idx_meth = meth_opts.index(st.session_state.get(f"wm_l_meth_{i}", def_meth)) if st.session_state.get(f"wm_l_meth_{i}") in meth_opts else meth_opts.index(def_meth)
                ...
                if not isinstance(saved_wmdf, list): 
                    if i > 1 and isinstance(st.session_state.get("wmdf_1"), list):
                        saved_wmdf = st.session_state.get("wmdf_1")
                    else:
