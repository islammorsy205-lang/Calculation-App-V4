...
def render_slab_element(i, gamma_c, live_load, fw_load, def_sec, def_main):
    ...
    # 💡 تم استبدال get_idx بـ session_state.get لضمان الاسترجاع التلقائي
    idx_etype = etype_opts.index(st.session_state.get(f"etype_{i}", "Slab")) if st.session_state.get(f"etype_{i}") in etype_opts else 0
    element_type = st.radio("Element Type:", etype_opts, key=f"etype_{i}", horizontal=True, index=idx_etype)
    
    st.markdown("### 🧱 Load Config")
    if element_type == "Beam":
        cb1, cb2 = st.columns(2)
        with cb1: 
            b_width = st.number_input("Beam Width (m)", value=float(st.session_state.get(f"bw_{i}", 0.30)), step=0.05, key=f"bw_{i}")
        with cb2: 
            ts = st.number_input("Beam Depth (m)", value=float(st.session_state.get(f"ts_beam_{i}", 0.60)), step=0.05, key=f"ts_beam_{i}")
        bpos_opts = ["Edge", "Middle"]
        idx_bpos = bpos_opts.index(st.session_state.get(f"bpos_{i}", "Edge")) if st.session_state.get(f"bpos_{i}") in bpos_opts else 0
        beam_pos = st.radio("Beam Position:", bpos_opts, key=f"bpos_{i}", horizontal=True, index=idx_bpos)
    else: 
        ts = st.number_input(f"{element_type} Thickness (m)", value=float(st.session_state.get(f"ts_thick_{i}", 0.30)), step=0.05, key=f"ts_thick_{i}")
        b_width = 0
        beam_pos = "None"
    ...
    with st.container(border=True):
        ...
        with col_s1:
            idx_ssec = list(SECTIONS_DB.keys()).index(st.session_state.get(f"ss_{i}", list(SECTIONS_DB.keys())[def_sec])) if st.session_state.get(f"ss_{i}") in SECTIONS_DB else def_sec
            s_sec = st.selectbox("Sec Section", list(SECTIONS_DB.keys()), index=idx_ssec, key=f"ss_{i}")
            
            s_spc = st.number_input("Spacing (Loaded Width) (m)", value=float(st.session_state.get(f"ssp_{i}", 0.350)), step=0.005, format="%.3f", key=f"ssp_{i}")
            ...
            with c_sl2:
                ...
                cust_sL = st.checkbox("✏️ Custom", value=st.session_state.get(f"cust_sL_{i}", False), key=f"cust_sL_{i}")
            
            with c_sl1:
                if cust_sL:
                    s_L = st.number_input("Total Length (m)", min_value=0.1, value=float(st.session_state.get(f"num_sL_{i}", s_l1_opts[-1])), step=0.1, key=f"num_sL_{i}")
                else:
                    idx_sl1 = s_l1_opts.index(st.session_state.get(f"sl1_{i}", s_l1_opts[-1])) if st.session_state.get(f"sl1_{i}") in s_l1_opts else (len(s_l1_opts)-1 if s_sec in STD_LENGTHS else 0)
                    s_L = st.selectbox("Total Length (m)", s_l1_opts, index=idx_sl1, key=f"sl1_{i}")
            
            s_cl = st.number_input("L. Cant (m)", value=float(st.session_state.get(f"scl_{i}", 0.50)), step=0.05, key=f"scl_{i}")
            s_spns = st.text_input("Spans (m) [Comma sep]", value=str(st.session_state.get(f"sspn_{i}", "1.30, 1.30")), key=f"sspn_{i}")
        ...
        with col_s2:
            ...
            # استرجاع الداتا المحفوظة للجدول لو موجودة
    ...
    with st.container(border=True):
        ...
        with col_m1:
            idx_msec = list(SECTIONS_DB.keys()).index(st.session_state.get(f"ms_{i}", list(SECTIONS_DB.keys())[def_main])) if st.session_state.get(f"ms_{i}") in SECTIONS_DB else def_main
            ...
            with c_ml2:
                st.markdown("<div style='margin-top: 32px;'></div>", unsafe_allow_html=True)
                cust_mL = st.checkbox("✏️ Custom", value=st.session_state.get(f"cust_mL_{i}", False), key=f"cust_mL_{i}")
            
            with c_ml1:
                if cust_mL:
                    m_L = st.number_input("Total Length (m)", min_value=0.1, value=float(st.session_state.get(f"num_mL_{i}", m_l1_opts[-1])), step=0.1, key=f"num_mL_{i}")
                else:
                    idx_ml1 = m_l1_opts.index(st.session_state.get(f"wml1_{i}", m_l1_opts[-1])) if st.session_state.get(f"wml1_{i}") in m_l1_opts else (len(m_l1_opts)-1 if m_sec in STD_LENGTHS else 0)
                    m_L = st.selectbox("Total Length (m)", m_l1_opts, index=idx_ml1, key=f"wml1_{i}")
            
            m_cl = st.number_input("L. Cant (m)", value=float(st.session_state.get(f"mcl_{i}", 0.50)), step=0.05, key=f"mcl_{i}")
            m_spns = st.text_input("Spans (m) [Comma sep]", value=str(st.session_state.get(f"mspn_{i}", "1.20")), key=f"mspn_{i}")
            ...
            idx_tn = SHORING_OPTIONS_SLAB.index(st.session_state.get(f"tn_{i}", SHORING_OPTIONS_SLAB[0])) if st.session_state.get(f"tn_{i}") in SHORING_OPTIONS_SLAB else 0
            ...
            if t_nm == "Shorebrace Frame":
                ...
                idx_cup = sub_opts.index(st.session_state.get(f"cup_sub_{i}", sub_opts[0])) if st.session_state.get(f"cup_sub_{i}") in sub_opts else 0
                subtype = c_sup1.selectbox("Steel Grade", sub_opts, index=idx_cup, key=f"cup_sub_{i}")
                unbraced = c_sup2.number_input("Unbraced Length (Lcr) (m)", min_value=0.5, max_value=3.0, value=float(st.session_state.get(f"cup_unb_{i}", 1.5)), step=0.5, key=f"cup_unb_{i}")
                ...
                idx_ring = r_opts.index(st.session_state.get(f"ring_sub_{i}", r_opts[0])) if st.session_state.get(f"ring_sub_{i}") in r_opts else 0
                subtype = c_sup1.selectbox("Diameter", r_opts, index=idx_ring, key=f"ring_sub_{i}")
                unbraced = c_sup2.number_input("Unbraced Length (Lcr) (m)", min_value=1.0, max_value=3.0, value=float(st.session_state.get(f"ring_unb_{i}", 1.5)), step=0.5, key=f"ring_unb_{i}")
                ...
                req_ext = c_prop1.number_input("Prop Extension (m)", min_value=0.95, max_value=5.50, value=float(st.session_state.get(f"prop_ext_{i}", 3.20)), step=0.1, key=f"prop_ext_{i}")
                ...
                if not valid_props:
                    ...
                    idx_psel = valid_props.index(st.session_state.get(f"prop_sel_{i}", valid_props[0])) if st.session_state.get(f"prop_sel_{i}") in valid_props else 0
                    sel_prop = c_prop2.selectbox("Select Valid Prop", valid_props, index=idx_psel, key=f"prop_sel_{i}")
                    inner_up = c_prop3.toggle("Inner Tube UP?", value=bool(st.session_state.get(f"prop_dir_{i}", True)), key=f"prop_dir_{i}")
                ...
                t_al = st.number_input("Allowable (kN)", value=float(st.session_state.get(f"ta_man_{i}", 20.0)), step=0.5, key=f"ta_man_{i}")
        ...
        with col_m2:
            ...
            idx_meth = meth_opts.index(st.session_state.get(f"m_l_meth_{i}", meth_opts[0])) if st.session_state.get(f"m_l_meth_{i}") in meth_opts else 0
...
def render_vertical_element(i, element_subtype, def_sec, def_main):
    ...
    if element_subtype == "Wall":
        ...
        idx_wt = wt_opts.index(st.session_state.get(f"wt_{i}", wt_opts[0])) if st.session_state.get(f"wt_{i}") in wt_opts else 0
    ...
    idx_vsys = vsys_opts.index(st.session_state.get(f"vsys_{i}", vsys_opts[0])) if st.session_state.get(f"vsys_{i}") in vsys_opts else 0
    ...
    if wall_pdf_curr:
        ...
        with col_w2: 
            wall_h = st.number_input("Pouring Height (m)", value=float(st.session_state.get(f"wall_ph_{i}", st.session_state.get(f"hp_{i}", 3.50))), step=0.05, key=f"wall_ph_{i}")
    else:
        with col_w2: 
            w_tot = st.number_input("Concrete Pressure Pmax (kN/m²)", value=float(st.session_state.get(f"wall_p_{i}", 47.16)), step=0.05, key=f"wall_p_{i}")
        with col_w3: 
            h_static = st.number_input("H static (m)", value=float(st.session_state.get(f"wall_hs_{i}", w_tot/25.0)), step=0.05, key=f"wall_hs_{i}")
        with col_w4: 
            wall_h = st.number_input("Pouring Height (m)", value=float(st.session_state.get(f"wall_ph_{i}", st.session_state.get(f"hp_{i}", 3.50))), step=0.05, key=f"wall_ph_{i}")
    ...
    if is_panel_sys:
        if "Eco-form" in vert_system: 
            ...
            idx_pw = panel_w_opts.index(st.session_state.get(f"panel_w_{i}", 0.60)) if st.session_state.get(f"panel_w_{i}") in panel_w_opts else 2
    ...
    if not is_panel_sys:
        ...
        with st.container(border=True):
            ...
            with col_s1:
                ...
                idx_wss = s_sec_opts.index(st.session_state.get(f"wss_{i}", s_sec_opts[def_s])) if st.session_state.get(f"wss_{i}") in s_sec_opts else def_s
                s_sec = st.selectbox("Sec Section", s_sec_opts, index=idx_wss, key=f"wss_{i}")
                
                s_spc = st.number_input("Spacing (Loaded Width) (m)", value=float(st.session_state.get(f"wssp_{i}", 0.310)), step=0.005, format="%.3f", key=f"wssp_{i}")
                
                wsl1_opts = STD_LENGTHS.get(s_sec, [3.0])
                c_wsl1, c_wsl2 = st.columns([3, 1])
                with c_wsl2:
                    st.markdown("<div style='margin-top: 32px;'></div>", unsafe_allow_html=True)
                    cust_wsl = st.checkbox("✏️ Custom", value=st.session_state.get(f"cust_wsl_{i}", False), key=f"cust_wsl_{i}")
                
                with c_wsl1:
                    if cust_wsl:
                        s_L = st.number_input("Total Length (m)", min_value=0.1, value=float(st.session_state.get(f"num_wsl1_{i}", wsl1_opts[-1])), step=0.1, key=f"num_wsl1_{i}")
                    else:
                        idx_wsl1 = wsl1_opts.index(st.session_state.get(f"wsl1_{i}", wsl1_opts[-1])) if st.session_state.get(f"wsl1_{i}") in wsl1_opts else (len(wsl1_opts)-1 if s_sec in STD_LENGTHS else 0)
                        s_L = st.selectbox("Total Length (m)", wsl1_opts, index=idx_wsl1, key=f"wsl1_{i}")
                
                s_cl = st.number_input("L. Cant (m)", value=float(st.session_state.get(f"wscl_{i}", 0.50)), step=0.05, key=f"wscl_{i}")
                s_spns = st.text_input("Spans (m) [Comma sep]", value=str(st.session_state.get(f"wsspn_{i}", "1.30, 1.30")), key=f"wsspn_{i}")
            ...
            with col_s2:
                ...
                with col_tog_s:
                    is_hydro_s = st.toggle("Apply Hydrostatic Load (Trapezoidal) ✅", value=bool(st.session_state.get(f"wshydro_{i}", False)), key=f"wshydro_{i}")
                
                s_top_empty = 0.0
                if is_hydro_s:
                    with col_inp_s:
                        s_top_empty = st.number_input("Top Empty Dist. (m)", min_value=0.0, max_value=float(s_L), value=float(st.session_state.get(f"s_top_empty_{i}", 0.0)), step=0.05, key=f"s_top_empty_{i}", help="المسافة الفاضية من أعلى الخشبة")
                ...
                saved_wsdf = st.session_state.get(f"wsdf_{i}", s_loads_data)
        ...
        with st.container(border=True):
            ...
            col_m1, col_m2 = st.columns([1, 1.2])
            with col_m1:
                idx_wms = list(SECTIONS_DB.keys()).index(st.session_state.get(f"wms_{i}", list(SECTIONS_DB.keys())[def_main])) if st.session_state.get(f"wms_{i}") in SECTIONS_DB else def_main
                ...
                with c_wml2:
                    st.markdown("<div style='margin-top: 32px;'></div>", unsafe_allow_html=True)
                    cust_wml = st.checkbox("✏️ Custom", value=st.session_state.get(f"cust_wml_{i}", False), key=f"cust_wml_{i}")
                
                with c_wml1:
                    if cust_wml:
                        m_L = st.number_input("Total Length (m)", min_value=0.1, value=float(st.session_state.get(f"num_wml1_{i}", wml1_opts[-1])), step=0.1, key=f"num_wml1_{i}")
                    else:
                        idx_wml1_w = wml1_opts.index(st.session_state.get(f"wml1_w_{i}", wml1_opts[-1])) if st.session_state.get(f"wml1_w_{i}") in wml1_opts else (len(wml1_opts)-1 if m_sec in STD_LENGTHS else 0)
                        m_L = st.selectbox("Total Length (m)", wml1_opts, index=idx_wml1_w, key=f"wml1_w_{i}")
                
                m_cl = st.number_input("L. Cant (m)", value=float(st.session_state.get(f"wmcl_w_{i}", 0.50)), step=0.05, key=f"wmcl_w_{i}")
                m_spns = st.text_input("Spans (m) [Comma sep]", value=str(st.session_state.get(f"wmspn_w_{i}", "1.32, 1.32")), key=f"wmspn_w_{i}")
            ...
            with col_m2:
                ...
                with col_tog_m:
                    is_hydro_m = st.toggle("Apply Hydrostatic Load (Trapezoidal) ✅", value=bool(st.session_state.get(f"wmhydro_{i}", False)), key=f"wmhydro_{i}")
                
                m_top_empty = 0.0
                if is_hydro_m:
                    with col_inp_m:
                        m_top_empty = st.number_input("Top Empty Dist. (m)", min_value=0.0, max_value=float(m_L), value=float(st.session_state.get(f"m_top_empty_{i}", 0.0)), step=0.05, key=f"m_top_empty_{i}", help="المسافة الفاضية من أعلى الخشبة")
                ...
                saved_wmdf = st.session_state.get(f"wmdf_{i}", m_loads_data)
    ...
    if is_panel_sys and not is_single_sided:
        ...
        if vert_system != "Circular Steel Panel System":
            with col_t:
                ...
                tie_h = st.number_input("Tie Rod Horiz. Spacing (m)", value=float(st.session_state.get(f"tie_h_{i}", 1.20)), step=0.05, key=f"tie_h_{i}")
                tie_v = st.number_input("Tie Rod Vert. Spacing (m)", value=float(st.session_state.get(f"tie_v_{i}", 1.20)), step=0.05, key=f"tie_v_{i}")
                st.info("**Tie Rod Allowable Load:** 90.00 kN")
        with col_b:
            st.markdown("#### 🔩 Acrow Bolts Configuration")
            bolt_h = st.number_input("Bolt Horiz. Spacing (m)", value=float(st.session_state.get(f"bolt_h_{i}", 0.30)), step=0.05, key=f"bolt_h_{i}")
            bolt_v = st.number_input("Bolt Vert. Spacing (m)", value=float(st.session_state.get(f"bolt_v_{i}", 1.20)), step=0.05, key=f"bolt_v_{i}")
