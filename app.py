...
def secure_login_portal():
    """
    بوابة الحماية الحديدية: تسأل عن الإيميل والباسوورد، وتحفظ البصمة لمنع الخروج عند التحديث.
    تتجاهل حالة الأحرف (كابيتال/سمول) وتمسح المسافات المنسوخة بالخطأ لسهولة الدخول.
    """
    ...
    if saved_email:
        # 💡 تنظيف الداتا المحفوظة وقاعدة البيانات لضمان التطابق
        ...
        if saved_email_clean in status_db_clean:
            if status_db_clean[saved_email_clean] == "approved":
                ...
                # لو الأدمن عمل إيقاف للحساب، يتم مسح البصمة تلقائياً
    ...
    with col2:
        with st.form("login_form"):
            ...
            if submit_btn:
                try:
                    # 💡 الحل الجذري: تنظيف الإيميل (حروف صغيرة ومسح المسافات)، وتنظيف الباسوورد (مسح المسافات فقط)
                    input_email_clean = email_input.strip().lower()
                    input_pass_clean = pass_input.strip() 
                    
                    # تنظيف قاعدة بيانات السيرفر أثناء القراءة لتفادي أي خطأ في ملف الـ Secrets
                    passwords_db_clean = {str(k).strip().lower(): str(v).strip() for k, v in st.secrets["passwords"].items()}
...
# ==========================================
# لن يتم تنفيذ أي كود أسفل هذا السطر إلا إذا كان المستخدم معتمداً
