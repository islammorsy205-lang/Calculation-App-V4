...
def secure_login_portal():
    ...
    تتجاهل حالة الأحرف (كابيتال/سمول) لسهولة الدخول.
    """
    ...
    if saved_email:
        # تحويل الإيميل المحفوظ وقاعدة البيانات لحروف صغيرة للتأكد
        saved_email_lower = saved_email.strip().lower()
        status_db_lower = {k.lower(): v for k, v in st.secrets.get("status", {}).items()}
        roles_db_lower = {k.lower(): v for k, v in st.secrets.get("roles", {}).items()}
        
        if saved_email_lower in status_db_lower:
            if status_db_lower[saved_email_lower] == "approved":
                st.session_state["logged_in"] = True
                st.session_state["user_email"] = saved_email_lower
                st.session_state["user_role"] = roles_db_lower.get(saved_email_lower, "user")
                return True
            else:
    ...
    with col2:
        with st.form("login_form"):
            ...
            if submit_btn:
                try:
                    # 💡 الحل الجذري: تحويل الإدخال وقاعدة بيانات السيرفر بالكامل لحروف صغيرة (Lowercase)
                    input_email_lower = email_input.strip().lower()
                    
                    passwords_db_lower = {k.lower(): v for k, v in st.secrets["passwords"].items()}
                    status_db_lower = {k.lower(): v for k, v in st.secrets["status"].items()}
                    roles_db_lower = {k.lower(): v for k, v in st.secrets["roles"].items()}
                    
                    if input_email_lower in passwords_db_lower:
                        # ملاحظة: التحويل لحروف صغيرة يطبق على الإيميل فقط، الباسوورد يظل كما هو لضمان الأمان!
                        if pass_input == passwords_db_lower[input_email_lower]:
                            if status_db_lower.get(input_email_lower, "declined") == "approved":
                                
                                # حفظ البصمة لمدة 365 يوم
                                cookie_manager.set("acrow_auth_email", input_email_lower, max_age=365*24*60*60)
                                
                                st.session_state["logged_in"] = True
                                st.session_state["user_email"] = input_email_lower
                                st.session_state["user_role"] = roles_db_lower.get(input_email_lower, "user")
        ...
        try:
            ...
            for email_acc, status_val in st.secrets["status"].items():
                ...
                users_data.append({
                    "Email": email_acc, 
                    "Role": role_val.upper(), 
                    "Status": "🟢 APPROVED" if status_val == "approved" else "🔴 DECLINED"
                })
