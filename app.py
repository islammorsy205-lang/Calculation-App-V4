...
# ==========================================
# 🛡️ 2. SECURITY SYSTEM & AUTHENTICATION (SECRETS MODE)
# ==========================================
def secure_login_portal():
    """
    بوابة الحماية الحديدية: تسأل عن الإيميل والباسوورد، وتقرأ الصلاحيات من Secrets.
    """
    ...
    # إذا كان المستخدم مسجل دخوله بالفعل، يتجاوز هذه البوابة
    ...
    with col2:
        with st.form("login_form"):
            ...
            if submit_btn:
                try:
                    # قراءة بيانات الحماية من إعدادات السيرفر السحابية
                    ...
                    st.error("⚠️ Server Configuration Error! Missing Secrets Section. Please contact Administrator.")
                    
    # إيقاف تحميل باقي البرنامج إذا لم يسجل الدخول
    return False

# ==========================================
# 🚀 3. MAIN APPLICATION EXECUTION
# ==========================================
# لن يتم تنفيذ أي كود أسفل هذا السطر إلا إذا كان المستخدم معتمداً (True)
...
if not st.session_state.get("is_admin", False) and "welcome_msg_shown" not in st.session_state:
    st.info("💡 **IMPORTANT:** Bookmark this page now. Your browser will save your password for future auto-logins!")
    st.session_state["welcome_msg_shown"] = True

# تعيين صلاحيات النظام بناءً على الدخول السري
is_admin_user = (st.session_state.get("user_role") == "admin")
st.session_state["is_admin"] = is_admin_user

if is_admin_user:
    # 💡 لوحة تحكم الأدمن الجديدة (تقرأ من الـ Secrets مباشرة بدون إنترنت أو قواعد بيانات)
...
def is_safe_key(k):
    ...
    # منع مفاتيح الـ Data Editors وأزرار التنزيل من جذورها
    forbidden_starts = (
    ...
    # منع الكلمات المحظورة التي تخص وظائف لا تقبل التخزين
    forbidden_contains = [
        "download", "upload", "button", "btn", "dxf", "fea_data", "formsubmitter"
    ]
