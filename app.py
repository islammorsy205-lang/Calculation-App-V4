...
# ==========================================
# 💡 درع الحماية النووي: يمنع الجداول، الأزرار، والملفات من حقن الذاكرة لتجنب أي Crash
def is_safe_key(k):
    kl = str(k).lower()
    
    # 1. منع مفاتيح الـ Data Editors (جداول الأحمال) وأزرار التنزيل من جذورها
    forbidden_starts = (
        "sdf_", "mdf_", "wsdf_", "wmdf_", "sb_loads_", # Data editors
        "dwn_", "dl_", "dw_", "dn_", "dr_", "s2k_",    # Download buttons
        "del_", "wall_pdf"                             # Buttons and uploaders
    )
    if kl.startswith(forbidden_starts):
        return False
    
    # 2. منع الكلمات المحظورة التي تخص وظائف لا تقبل التخزين
    forbidden_contains = [
            ...
            if st.button("🔄 Restore Project Data", type="primary", use_container_width=True):
                try:
                    ...
                    for k, v in saved_data.items():
                        # فلترة صارمة، وإذا حدث أي استثناء غير متوقع من Streamlit يتم تجاوزه بصمت
                        if is_safe_key(k):
                            try:
