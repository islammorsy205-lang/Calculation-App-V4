...
if "Slab Elements" in sys_cat:
    ...
    for i, tab in enumerate(tabs):
        # 💡 الإصلاح الجذري للاستنساخ: منع نقل جداول الأحمال لتفادي خطأ Streamlit Not Allowed
    ...
    for i, tab in enumerate(tabs):
        # 💡 الإصلاح الجذري للاستنساخ: منع نقل حقل رفع الـ PDF وجداول الأحمال
...
# 💡 تم بناء دالة أمان محلية هنا لتجنب خطأ الـ ImportError السابق نهائياً
...
if generate_doc_btn:
    ...
    if errors: 
        ...
        # 💡 استخدمنا الدالة المحلية اللي عملناها فوق بدلاً من عمل Import
        ...
        with st.spinner("🔄 Running Advanced 3-Moment Equation Solver & Building Document..."):
            ...
            # =========================================================================================
            # 💡 الجراحة الأولى: برمجة وتوليد الفهرس الديناميكي وحساب أرقام الصفحات (INDEX OF CONTENTS)
            # =========================================================================================
            ds_page_map = {}
            current_page = 4 
            ...
            def add_reference_line(doc_obj, item_name, ds_map):
                """دالة زراعة الإشارات المرجعية بخط أزرق تحت الـ Checks"""
                if not item_name or not ds_map: return
                item_clean = str(item_name).upper().replace("DOUBLE", "").strip()
                matched_page = None
                for k, v in ds_map.items():
                    k_clean = str(k).upper().replace(".PDF", "").strip()
                    if item_clean in k_clean or k_clean in item_clean:
                        matched_page = v
                        break
                if matched_page:
            ...
            for idx_c, conf in enumerate(configs):
                if conf['cat'] == 'horizontal':
                    add_eq(doc, f"   3.{idx_c+1} Elements Calculation for {conf['sub_cat']} ({conf.get('ts', 0)}m)")
                else:
                    add_eq(doc, f"   3.{idx_c+1} Elements Calculation for {conf['sub_cat']} Height ({conf.get('height', 0)}m)")
