...
# ==========================================
# 💡 دالة الفلترة الذكية لمنع انهيار Streamlit بسبب الأزرار وملفات الرفع
def is_safe_key(k):
    kl = k.lower()
    forbidden_words = ["download", "btn", "button", "upload", "wall_pdf"]
            ...
            if st.button("🔄 Restore Project Data", type="primary", use_container_width=True):
                try:
                    ...
                    for k, v in saved_data.items():
                        # فلترة المفاتيح أثناء الاسترجاع (Load)
                        if is_safe_key(k):
        ...
        def get_savable_state():
            ...
            for k, v in st.session_state.items():
                if isinstance(v, (int, float, str, list, dict, bool)) and not k.startswith("FormSubmitter"):
                    # فلترة المفاتيح أثناء الحفظ (Save)
                    if is_safe_key(k):
                        safe_state[k] = v
