...
def secure_login_portal():
    ...
    تتجاهل حالة الأحرف في الإيميل، وتمسح أي مسافات أو علامات تنصيص مخفية لسهولة الدخول 100%.
    """
    ...
    with col2:
        with st.form("login_form"):
            ...
            if submit_btn:
                try:
                    # 💡 التنظيف الذكي للإدخال (إزالة المسافات، علامات التنصيص المنسوخة بالخطأ، وتوحيد الإيميل)
                    input_email_clean = str(email_input).strip().lower()
                    input_pass_clean = str(pass_input).strip().strip("'").strip('"').strip()
                    
                    # 💡 التنظيف الذكي لقاعدة بيانات السيرفر أثناء القراءة لضمان التطابق المستحيل يفشل
                    passwords_db_clean = {}
