...
# ==========================================
# 💡 الفلتر الشامل لمنع انهيار Streamlit بسبب حماية الأزرار وملفات الرفع والجداول التفاعلية والمصفوفات
def is_safe_key(k):
    kl = k.lower()
    forbidden_words = [
        "download", "dwn_", "btn", "button", 
        "upload", "wall_pdf", "dxf_uploader",
        "sdf_", "mdf_", "wsdf_", "wmdf_", "sb_loads_",
        "fea_data", "dxf_parsed", "dxf_version"
    ]
    return not any(fw in kl for fw in forbidden_words)
            ...
            if st.button("🔄 Restore Project Data", type="primary", use_container_width=True):
                try:
                    ...
                    for k, v in saved_data.items():
                        # فحص المفاتيح وتجاهل الممنوع منها أثناء الاسترجاع
                        if is_safe_key(k):
    ...
    with c_save:
        def get_savable_state():
            ...
            for k, v in st.session_state.items():
                if not k.startswith("FormSubmitter") and is_safe_key(k):
                    # 💡 درع الحماية النهائي: تجربة تحويل القيمة، لو فشلت (مصفوفة، صورة، كائن معقد) يتجاهلها فوراً!
                    try:
                        json.dumps(v)
                        safe_state[k] = v
