...
# 💡 استدعاء ملف الـ 3D الجديد (لو موجود)
try:
...
def secure_login_portal():
    ...
    # 1. فحص البصمة (Cookies) للدخول التلقائي عند عمل Refresh
...
# ==========================================
# 4. Role Assignment & Local Admin Panel
# ==========================================
...
# ==========================================
# 💾 6. PROJECT MANAGEMENT (SAVE & LOAD)
# ==========================================
def is_safe_key(k):
    kl = str(k).lower()
    forbidden_starts = (
...
if generate_doc_btn:
    ...
    if errors: 
        ...
        with st.spinner("🔄 Running Advanced 3-Moment Equation Solver & Building Document..."):
            ...
            # =========================================================================================
            ds_page_map = {}
            current_page = 4 # Page 1: Cover, Page 2: Index, Page 3: Regulations
            ...
            for idx, conf in enumerate(configs):
                ...
                if conf.get('is_panel_system'):
                    ...
                    add_reference_line(doc, conf['sys_name'], ds_page_map) # 💡 سطر المرجعية الأزرق
                    chk_counter += 1
                    ...
                    if conf.get('tie_h') and conf['sys_name'] != "Circular Steel Panel System" and not conf.get('strongback', {}).get('active'):
                        ...
                        add_reference_line(doc, "Tie Rod", ds_page_map) # 💡 سطر المرجعية الأزرق
                        chk_counter += 1
                    ...
                    add_reference_line(doc, "Plywood", ds_page_map) # 💡 سطر المرجعية الأزرق
                    ...
                    add_reference_line(doc, conf['s_sec'], ds_page_map) # 💡 سطر المرجعية الأزرق
                    ...
                    add_reference_line(doc, conf['m_sec'], ds_page_map) # 💡 سطر المرجعية الأزرق
                    ...
                    if conf.get('t_allow') is not None and conf['t_allow'] < 900:
                        ...
                        add_reference_line(doc, display_name, ds_page_map) # 💡 سطر المرجعية الأزرق
                        chk_counter += 1
                ...
                if conf.get('strongback', {}).get('active'):
                    ...
                    add_reference_line(doc, sb['sv'], ds_page_map) # 💡 سطر المرجعية الأزرق
                    ...
                    add_reference_line(doc, sb['sh'], ds_page_map) # 💡 سطر المرجعية الأزرق
                    ...
                    for st_d in sb['diags']:
                        ...
                        add_reference_line(doc, st_d['type'].split()[0], ds_page_map) # 💡 سطر المرجعية الأزرق
                    ...
                    add_reference_line(doc, sb['waler_sec'], ds_page_map) # 💡 سطر المرجعية الأزرق
                ...
                if conf.get('cat') == 'vertical' and conf.get('tilting', {}).get('active'):
                    ...
                    add_eq(doc, "Refer to Regulations and Standards the Saudi Building Code (SBC) 2024", bold=True, color=RGBColor(192,0,0)) # 💡 تحديث كود 2024
                    ...
                    if 'struts' in td:
                        for st_c in td['struts']: 
                            ...
                            add_reference_line(doc, st_c['type'].split()[0], ds_page_map) # 💡 سطر المرجعية الأزرق
