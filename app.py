...
import extra_streamlit_components as stx # 💡 مكتبة حفظ الدخول التلقائي (Cookies)
...
def secure_login_portal():
    ...
    if saved_email and saved_email in st.secrets.get("status", {}):
        if st.secrets["status"][saved_email] == "approved":
            st.session_state["logged_in"] = True
            st.session_state["user_email"] = saved_email
            st.session_state["user_role"] = st.secrets["roles"].get(saved_email, "user")
            return True
        else:
    ...
    with col2:
        with st.form("login_form"):
            ...
            if submit_btn:
                try:
                    passwords_db = st.secrets["passwords"]
                    status_db = st.secrets["status"]
                    roles_db = st.secrets["roles"]
                    
                    if email_input in passwords_db:
                        if pass_input == passwords_db[email_input]:
                            if status_db.get(email_input, "declined") == "approved":
                                
                                # 💡 نجاح الدخول: حفظ البصمة لمدة 365 يوم
                                cookie_manager.set("acrow_auth_email", email_input, max_age=365*24*60*60)
                                
                                st.session_state["logged_in"] = True
                                st.session_state["user_email"] = email_input
                                st.session_state["user_role"] = roles_db.get(email_input, "user")
                                st.success("✅ Verification successful! Remembering device...")
                                time.sleep(1.5) # انتظار بسيط لضمان حفظ البصمة
                                st.rerun()
                            else:
