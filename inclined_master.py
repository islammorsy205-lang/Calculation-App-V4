...
def render_inclined_module():
    ...
    # 💡 ربط بخاصية الحفظ واسترجاع الداتا
    L_tot_val = c_tot1.number_input("Total Inclined Length (m)", value=float(st.session_state.get("inc_L_tot", 5.0)), step=0.5, key="inc_L_tot", on_change=lambda: st.session_state.update(inclined_solved=False))
    X_tot_val = c_tot2.number_input("Total Base Length (m)", value=float(st.session_state.get("inc_X_tot", 3.5)), step=0.5, key="inc_X_tot", on_change=lambda: st.session_state.update(inclined_solved=False))
    ...
    idx_inc_sec = sec_list.index(st.session_state.get("inc_sec", sec_list[default_idx])) if st.session_state.get("inc_sec") in sec_list else default_idx
    inc_sec = c_p1.selectbox("Profile (Inclined Soldier)", sec_list, index=idx_inc_sec, key="inc_sec", on_change=lambda: st.session_state.update(inclined_solved=False))
    
    idx_base_sec = base_sec_list.index(st.session_state.get("inc_base_sec", base_sec_list[0])) if st.session_state.get("inc_base_sec") in base_sec_list else 0
    base_sec = c_p2.selectbox("Profile (Base Soldier)", base_sec_list, index=idx_base_sec, key="inc_base_sec", on_change=lambda: st.session_state.update(inclined_solved=False))
    
    st.markdown("#### 🔗 2. Segments & Connections")
    c_g1, c_g2, c_g3, c_g4 = st.columns(4)
    angle_deg = c_g1.number_input("Inclination Angle (°)", value=float(st.session_state.get("inc_angle", 60.0)), step=5.0, key="inc_angle", on_change=lambda: st.session_state.update(inclined_solved=False))
    angle_rad = np.radians(angle_deg)
    num_struts = c_g2.number_input("Push-Pulls Count", min_value=1, max_value=5, value=int(st.session_state.get("inc_num_struts", 2)), step=1, key="inc_num_struts", on_change=lambda: st.session_state.update(inclined_solved=False))
    ...
    for j in range(int(num_struts)):
        ...
        l_val = cl1.number_input(f"L{j+1} on Inclined (m)", value=float(st.session_state.get(f"L_{j}", 2.0)), step=0.5, key=f"L_{j}", on_change=lambda: st.session_state.update(inclined_solved=False))
        x_val = cl2.number_input(f"X{j+1} on Base (m)", value=float(st.session_state.get(f"X_{j}", 1.5)), step=0.5, key=f"X_{j}", on_change=lambda: st.session_state.update(inclined_solved=False))
        ...
        idx_st = valid_struts.index(st.session_state.get(f"st_{j}", valid_struts[0])) if st.session_state.get(f"st_{j}") in valid_struts else 0
        st_type = cl3.selectbox(f"Strut {j+1} (Req: {req_len:.2f}m)", valid_struts, index=idx_st, key=f"st_{j}", on_change=lambda: st.session_state.update(inclined_solved=False))
    ...
    corn_opts = ["2 Tie Rods", "Shoring System", "None (On Ground)"]
    idx_corn = corn_opts.index(st.session_state.get("corn_chk_opt", corn_opts[0])) if st.session_state.get("corn_chk_opt") in corn_opts else 0
    corner_check_opt = c_s1.selectbox("Corner Securing", corn_opts, index=idx_corn, key="corn_chk_opt", on_change=lambda: st.session_state.update(inclined_solved=False))
    
    sys_opts = ["None (On Ground directly)", "Acrow Prop", "Cup-lock", "Ringlock", "Shorebrace Frame"]
    idx_sys = sys_opts.index(st.session_state.get("inc_shore_sys", sys_opts[0])) if st.session_state.get("inc_shore_sys") in sys_opts else 0
    shoring_sys = c_s2.selectbox("Base Shoring", sys_opts, index=idx_sys, key="inc_shore_sys", on_change=lambda: st.session_state.update(inclined_solved=False))
    
    ctype_opts = ["Hinged", "Roller", "Fixed"]
    idx_ctype = ctype_opts.index(st.session_state.get("corn_type", ctype_opts[0])) if st.session_state.get("corn_type") in ctype_opts else 0
    corner_type = c_s3.selectbox("Corner Support Type", ctype_opts, index=idx_ctype, key="corn_type", on_change=lambda: st.session_state.update(inclined_solved=False))
    
    corner_ang = c_s4.number_input("Corner Angle (°)", value=float(st.session_state.get("corn_ang", 0.0)), step=15.0, key="corn_ang", on_change=lambda: st.session_state.update(inclined_solved=False))
    ...
    if shoring_sys == "Acrow Prop":
        ...
        idx_pt = prop_keys.index(st.session_state.get("sh_pt", prop_keys[0])) if st.session_state.get("sh_pt") in prop_keys else 0
        prop_type = cp1.selectbox("Prop Type", prop_keys, index=idx_pt, key="sh_pt", on_change=lambda: st.session_state.update(inclined_solved=False))
        prop_ext = cp2.number_input("Extension (m)", value=float(st.session_state.get("sh_pe", 3.0)), step=0.1, key="sh_pe", on_change=lambda: st.session_state.update(inclined_solved=False))
        ...
        cp1, cp2, cp3, cp4 = st.columns(4)
        cu_grade_opts = list(CUPLOCK_DB.keys()) if CUPLOCK_DB else ["S355 (st.52)"]
        idx_cg = cu_grade_opts.index(st.session_state.get("sh_cg", cu_grade_opts[0])) if st.session_state.get("sh_cg") in cu_grade_opts else 0
        cu_grade = cp1.selectbox("Grade", cu_grade_opts, index=idx_cg, key="sh_cg", on_change=lambda: st.session_state.update(inclined_solved=False))
        ...
        idx_cu = cu_opts.index(st.session_state.get("sh_cu", cu_opts[def_idx])) if st.session_state.get("sh_cu") in cu_opts else def_idx
        cu_unb = cp2.selectbox("Unbraced (m)", cu_opts, index=idx_cu, key="sh_cu", format_func=lambda x: f"{float(x):.2f}", on_change=lambda: st.session_state.update(inclined_solved=False))
        ...
        cp1, cp2, cp3, cp4 = st.columns(4)
        ri_size_opts = list(RINGLOCK_DB.keys()) if RINGLOCK_DB else ["Ringlock 1.5\""]
        idx_rs = ri_size_opts.index(st.session_state.get("sh_rs", ri_size_opts[0])) if st.session_state.get("sh_rs") in ri_size_opts else 0
        ri_size = cp1.selectbox("Size", ri_size_opts, index=idx_rs, key="sh_rs", on_change=lambda: st.session_state.update(inclined_solved=False))
        ...
        idx_ru = ri_opts.index(st.session_state.get("sh_ru", ri_opts[def_idx])) if st.session_state.get("sh_ru") in ri_opts else def_idx
        ri_unb = cp2.selectbox("Unbraced (m)", ri_opts, index=idx_ru, key="sh_ru", format_func=lambda x: f"{float(x):.2f}", on_change=lambda: st.session_state.update(inclined_solved=False))
    ...
    num_base_sups = st.number_input("Additional Base Supports Count", 0, 10, value=int(st.session_state.get("inc_num_base_sups", int(num_struts))), key="inc_num_base_sups", on_change=lambda: st.session_state.update(inclined_solved=False))
    base_sups = []
    default_xs = [sum(X_segs[:i+1]) for i in range(len(X_segs))]
    
    stype_opts = ["Hinged", "Roller", "Fixed"]
    for i in range(int(num_base_sups)):
        cs1, cs2, cs3 = st.columns(3)
        def_x = default_xs[i] if i < len(default_xs) else float((i+1)*1.5)
        sx = cs1.number_input(f"Sup {i+1} X (m)", value=float(st.session_state.get(f"sx_{i}", def_x)), step=0.5, key=f"sx_{i}", on_change=lambda: st.session_state.update(inclined_solved=False))
        idx_stype = stype_opts.index(st.session_state.get(f"stype_{i}", "Hinged")) if st.session_state.get(f"stype_{i}") in stype_opts else 0
        stype = cs2.selectbox(f"Sup {i+1} Type", stype_opts, index=idx_stype, key=f"stype_{i}", on_change=lambda: st.session_state.update(inclined_solved=False))
    ...
    with c_in:
        ...
        ltype_opts = ["Uniform", "Trapezoidal/Triangular", "Point Load"]
        idx_ltype = ltype_opts.index(st.session_state.get("inc_l_type", ltype_opts[0])) if st.session_state.get("inc_l_type") in ltype_opts else 0
        l_type = c_ld1.selectbox("Load Type", ltype_opts, index=idx_ltype, key="inc_l_type", on_change=lambda: st.session_state.update(inclined_solved=False))
        
        num_items = c_ld2.number_input(f"Count of Loads", 1, 20, value=int(st.session_state.get("inc_num_loads", 1)), key="inc_num_loads", on_change=lambda: st.session_state.update(inclined_solved=False))
        
        dir_opts = ["Gravity (Vertical ↓)", "Perpendicular (Local ↘)"]
        idx_dir = dir_opts.index(st.session_state.get("inc_ldir", dir_opts[0])) if st.session_state.get("inc_ldir") in dir_opts else 0
        ldir = c_ld3.selectbox("Direction", dir_opts, index=idx_dir, key="inc_ldir", on_change=lambda: st.session_state.update(inclined_solved=False))
        
        applied_loads = []
        for item in range(int(num_items)):
            if l_type == "Point Load":
                c1, c2 = st.columns(2)
                start_l = c1.number_input(f"P{item+1} Distance from Corner (m)", value=float(st.session_state.get(f"ls_{item}", 0.0)), step=0.5, key=f"ls_{item}", on_change=lambda: st.session_state.update(inclined_solved=False))
                w1 = c2.number_input(f"P{item+1} Value (kN)", value=float(st.session_state.get(f"w1_{item}", 15.0)), step=1.0, key=f"w1_{item}", on_change=lambda: st.session_state.update(inclined_solved=False))
                applied_loads.append({'type': l_type, 'start': start_l, 'end': start_l, 'w1': w1, 'w2': w1, 'dir': ldir})
            elif l_type == "Uniform":
                c1, c2, c3 = st.columns(3)
                start_l = c1.number_input(f"L{item+1} Start from Corner (m)", value=float(st.session_state.get(f"ls_{item}", 0.0)), step=0.5, key=f"ls_{item}", on_change=lambda: st.session_state.update(inclined_solved=False))
                end_l = c2.number_input(f"L{item+1} End from Corner (m)", value=float(st.session_state.get(f"le_{item}", float(L_tot_val))), step=0.5, key=f"le_{item}", on_change=lambda: st.session_state.update(inclined_solved=False))
                w1 = c3.number_input(f"L{item+1} W (kN/m)", value=float(st.session_state.get(f"w1_{item}", 15.0)), step=1.0, key=f"w1_{item}", on_change=lambda: st.session_state.update(inclined_solved=False))
                applied_loads.append({'type': l_type, 'start': min(start_l, end_l), 'end': max(start_l, end_l), 'w1': w1, 'w2': w1, 'dir': ldir})
            else:
                c1, c2, c3, c4 = st.columns(4)
                start_l = c1.number_input(f"L{item+1} Start from Corner (m)", value=float(st.session_state.get(f"ls_{item}", 0.0)), step=0.5, key=f"ls_{item}", on_change=lambda: st.session_state.update(inclined_solved=False))
                end_l = c2.number_input(f"L{item+1} End from Corner (m)", value=float(st.session_state.get(f"le_{item}", float(L_tot_val))), step=0.5, key=f"le_{item}", on_change=lambda: st.session_state.update(inclined_solved=False))
                w1 = c3.number_input(f"L{item+1} W1 (kN/m)", value=float(st.session_state.get(f"w1_{item}", 15.0)), step=1.0, key=f"w1_{item}", on_change=lambda: st.session_state.update(inclined_solved=False))
                w2 = c4.number_input(f"L{item+1} W2 (kN/m)", value=float(st.session_state.get(f"w2_{item}", 0.0)), step=1.0, key=f"w2_{item}", on_change=lambda: st.session_state.update(inclined_solved=False))
    ...
    if st.session_state.inclined_solved:
        ...
        with st.expander("⚙️ Diagram Scale Controls", expanded=True):
            ...
            sc_n = c_s1.slider("Axial Force Scale", 0.001, 0.100, float(st.session_state.get("inc_sc_n", 0.02)), step=0.005, key="inc_sc_n")
            sc_v = c_s2.slider("Shear Force Scale", 0.001, 0.100, float(st.session_state.get("inc_sc_v", 0.02)), step=0.005, key="inc_sc_v")
            sc_m = c_s3.slider("Moment Scale", 0.01, 0.50, float(st.session_state.get("inc_sc_m", 0.10)), step=0.01, key="inc_sc_m")
